...
class TestCommInfoSpecializedClasses:
    ...
    def test_comminfo_dc_functionality(self):
        ...
        expected_commission = abs(size) * price * params['mult'] * params['commission']  # 0.1 * 50000 * 1.0 * 0.1 = 500.0
    ...
    def test_futures_percent_functionality(self):
        ...
        expected_commission = abs(size) * price * params['mult'] * params['commission']  # 2.0 * 3000 * 10.0 * 0.02 = 1200.0
...
class TestCommInfoPerformance:
    ...
    def test_commission_calculation_performance(self):
        ...
        # Should complete 10,000 calculations in less than 0.05 seconds
        total_time = end_time - start_time
        assert total_time < 0.05, f"Commission calculation too slow: {total_time:.3f}s"
        
        ops_per_second = 10000 / total_time
        assert ops_per_second > 50000, f"Commission calculation too slow: {ops_per_second:.1f} ops/sec"
...
class TestCommInfoEdgeCases:
    ...
    def test_interest_calculation(self):
        ...
        interest = comm._get_credit_interest(None, -100.0, 50.0, 10, current_dt.date(), pos.datetime.date())
