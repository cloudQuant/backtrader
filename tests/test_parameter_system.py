...
class TestParameterDescriptor:
    ...
    def test_value_validation_mechanism(self):
        ...
        class TestClass(ParameterizedBase):
            positive_int = ParameterDescriptor(
                default=1, 
                type_=int, 
                validator=lambda x: x > 0
            )
            choice_param = ParameterDescriptor(
                default="A", 
...
class TestComplexScenarios:
    ...
    def test_parameter_validation_on_initialization(self):
        ...
        class TestClass(ParameterizedBase):
            positive_param = ParameterDescriptor(
                default=1, 
                type_=int,
                validator=lambda x: x > 0
            )
    ...
    def test_parameter_info_and_introspection(self):
        ...
        class TestClass(ParameterizedBase):
            int_param = ParameterDescriptor(
                default=10, 
                type_=int, 
                validator=Int(min_val=0),
                doc="An integer parameter"
            )
            str_param = ParameterDescriptor(
                default="test", 
                type_=str,
