"""Inlined regression test for the HMM Random Forest volatility-regime strategy (spec 0411).
...
Data Used:
    ...
    ``tests/datas/XAUUSD_1d.csv`` over 2024-09-01 to 2025-12-31 with no bar shift.
    A single data feed is used; a rich set of regime, signal, and position-sizing
    feature columns is precomputed in pandas (with an on-disk feature cache under
    ``features/``) and exposed to the strategy as extra feed lines.

Strategy Principle:
    A Gaussian Hidden Markov Model segments the market into bull, bear, and
    neutral volatility regimes from returns, volatility, and volume features. A
    per-regime Random Forest classifier then estimates the probability that the
    next bar closes higher. The strategy assumes regime-conditioned machine
    learning forecasts carry tradable edge: it goes long when the up-probability
    clears a confidence threshold, short when it falls below the symmetric lower
    bound, and stays flat otherwise, scaling position size with the model edge.

Strategy Logic:
    Models are trained on a rolling window and periodically retrained at a fixed
    interval. Each bar reads the precomputed bull/bear/neutral signal and target
    percentage: when flat it opens a long on a bull signal or a short on a bear
    signal sized by broker value, and when in a position it closes on a neutral,
    opposite, or zero-target signal. Order and trade notifications maintain entry,
    exit, retrain, win, and loss counters plus a broker-value series for the ulcer
    index. The test hooks metrics-extraction functions and runs the module ``run``
    entry point with runonce forced on, asserting each captured metric against
...
def load_mt5_csv(filepath, fromdate=None, todate=None, bar_shift_minutes=0):
    """Load a MetaTrader 5 CSV export (tab or comma separated) into an OHLCV frame.
...
def prepare_hmm_rf_features(df, params):
    """Compute HMM regime labels and Random Forest signal/sizing feature columns.

    Trains a rolling Gaussian HMM and per-regime Random Forest classifiers to
    derive predicted state, regime score, confidence, up-probability, discrete
    signal code, scaled target percent, retrain markers, and bull/bear/neutral
    flags for each bar.

    Args:
        df: OHLCV DataFrame indexed by datetime.
        params: Strategy parameter mapping (windows, model hyperparameters,
            thresholds, sizing bounds).

    Returns:
        A DataFrame with the original OHLCV columns plus the computed feature and
        signal columns, with warmup/NaN rows dropped.
    """
    out = df.copy()
...
class Mt5HMMRFFeed(bt.feeds.PandasData):
    """PandasData feed exposing OHLCV plus HMM/RF regime and signal lines."""
...
class HMMRandomForestStrategy(bt.Strategy):
    """Regime-aware strategy trading precomputed HMM/Random-Forest signals.

    Goes long on bull signals and short on bear signals sized by broker value and
    model edge, and closes on neutral, opposite, or zero-target signals while
    tracking retrain markers and a broker-value series.
    """
    ...
    def __init__(self):
        """Initialize trade/signal counters, order state, and broker-value series."""
        self.bar_num = 0
        self.buy_count = 0
        self.sell_count = 0
        self.short_count = 0
    ...
    def next(self):
        """Open, hold, or close positions per the precomputed regime signals."""
        self.bar_num += 1
    ...
    def notify_order(self, order):
        """Clear the pending-order reference once an order is no longer live.
...
def get_sharpe_analyzer_kwargs(config):
    """Build SharpeRatio analyzer kwargs scaled to the data timeframe.

    Args:
        config: Resolved configuration dictionary (its ``data.timeframe`` selects
            the annualization factor).

    Returns:
        A dictionary of keyword arguments for the SharpeRatio analyzer.
    """
...
def finite_or_none(value):
    ...
    Returns:
        The value unchanged if finite, or None for NaN/inf/None inputs.
    """
    if value is None:
        return None
...
def calculate_ulcer_index(values):
    """Compute the Ulcer Index of an equity/broker-value series.

    Args:
        values: Sequence of portfolio values ordered in time.

    Returns:
        The Ulcer Index (root mean square percentage drawdown), or 0.0 for
...
def resolve_data_file(file_value):
    ...
    Args:
        file_value: Configured data file path, absolute or relative.

    Returns:
        The first existing candidate Path for the data file.
...
def resolve_feature_cache_path(config):
    """Resolve the on-disk path for the cached feature frame.

    Args:
        config: Resolved configuration dictionary (its ``data.feature_cache_dir``
            selects the cache directory).

    Returns:
        The Path to the feature cache CSV file.
    """
    data_cfg = config.get('data', {}) if isinstance(config, dict) else {}
...
def cache_is_fresh(cache_path, dependencies):
    """Check whether a cache file is newer than all of its dependencies.

    Args:
        cache_path: Path to the cache file.
        dependencies: Iterable of dependency Paths whose mtimes are compared.

    Returns:
        True if the cache exists and is at least as new as every existing
...
def read_feature_cache(cache_path):
    """Read a cached feature frame from CSV.

    Args:
        cache_path: Path to the cached feature CSV file.

    Returns:
        A pandas DataFrame indexed by datetime and sorted ascending.
    """
...
def write_feature_cache(cache_path, frame):
    """Write a feature frame to the cache CSV, creating parent dirs as needed.

    Args:
        cache_path: Destination Path for the cache CSV file.
        frame: The feature DataFrame to persist.
    """
...
def load_backtest_frame(config):
    """Load the data and feature frame, using the on-disk cache when fresh.
    ...
    Returns:
        A dictionary with the prepared ``data`` frame, ``fromdate``/``todate``
        bounds, and the resolved strategy ``params``.

    Raises:
        ValueError: If the prepared data frame is empty.
    """
...
def build_cerebro(config, frame):
    """Assemble a Cerebro engine with broker, feed, strategy, and analyzers.

    Args:
        config: Resolved configuration dictionary with ``backtest`` and ``data``
            sections.
        frame: Dictionary returned by load_backtest_frame containing the data and
            resolved params.
...
def extract_metrics(strat, cerebro, frame, config):
    ...
    Returns:
        A dictionary of summary metrics including regime-day counts, trade
        statistics, returns, Sharpe ratio, drawdown, SQN, and ulcer index.
    """
...
def normalize(value):
    """Convert a value to a JSON-friendly scalar or ISO datetime string.

    Args:
        value: Any value, possibly a datetime-like object.

    Returns:
        The value unchanged if already a primitive, an ISO-formatted string for
        datetime-like objects, or its str() otherwise.
    """
...
def run(plot=False):
    """Run the full HMM Random Forest backtest end to end.

    Args:
        plot: Whether to display a cerebro candlestick plot after completion.
