"""Inlined regression test for tests/functional/strategies/volatility_systems/regression/0021_bollinger_band_breakout.
...
_CONFIG = {
    'strategy': {
        ...
        'source_spec': 'research_papers_gold/strategy_specs/technical_indicators/0033_Gold_Bollinger_Band_Breakout.md',
    },
    'data': {
        'symbol': 'XAUUSD',
        'timeframe': 'D1',
...
def test_21_0021_bollinger_band_breakout() -> None:
    ...
    _close(metrics.get('final_value'), 3076810.245206004, tol=3.076810e+00, key='final_value')
    _close(metrics.get('net_pnl'), 2076810.245206004, tol=2.076810e+00, key='net_pnl')
    _close(metrics.get('total_return_pct'), 207.6810245206004, tol=2.076810e-04, key='total_return_pct')
    _close(metrics.get('win_rate'), 42.857142857142854, tol=4.285714e-05, key='win_rate')
    _close(metrics.get('profit_factor'), 2.9686639470114398, tol=2.968664e-06, key='profit_factor')
    _close(metrics.get('max_drawdown'), 23.077038313843595, tol=2.307704e-05, key='max_drawdown')
    _close(metrics.get('sharpe_ratio'), 0.6615587211741462, tol=1.000000e-06, key='sharpe_ratio')
    _close(metrics.get('annual_return_pct'), 6.438509371547295, tol=6.438509e-06, key='annual_return_pct')
    _close(metrics.get('sqn'), 0.997370584087709, tol=1.000000e-06, key='sqn')
    _close(metrics.get('ulcer_index'), 14.65243950854685, tol=1.465244e-05, key='ulcer_index')
