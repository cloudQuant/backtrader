...
from strategy_volatility_hedge_fund import (
    Mt5VolatilityHedgeFundFeed, VolatilityHedgeFundStrategy, load_mt5_csv, prepare_volatility_hedge_fund_features,
)
...
def load_data(config):
    ...
    df = prepare_volatility_hedge_fund_features(raw, config.get('params', {}))
...
def build_cerebro(frame, config):
    ...
    feed = Mt5VolatilityHedgeFundFeed(dataname=frame['data'], timeframe=bt.TimeFrame.Days, compression=1)
    cerebro.adddata(feed, name=f"{config['data']['symbol']}_{config['data']['timeframe']}")
    cerebro.addstrategy(VolatilityHedgeFundStrategy, **config.get('params', {}))
