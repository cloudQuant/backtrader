...
from strategy_volatility_long_memory import (
    VolatilityLongMemoryFeed,
    VolatilityLongMemoryStrategy,
    load_mt5_csv,
    prepare_volatility_long_memory_features,
)
...
def upsert_global_summary(config, metrics):
    ...
    rows.append({
        ...
        'fromdate': metrics.get('fromdate', ''),
        'todate': metrics.get('todate', ''),
...
def print_results(metrics):
    ...
    print(f"  Regime changes:      {metrics['regime_change_count']}")
    print(f"  Trend days:          {metrics['trend_days']}")
    print(f"  Mean reversion days: {metrics['mean_reversion_days']}")
    print(f"  Neutral days:        {metrics['neutral_days']}")
    print(f"  Avg hurst:           {metrics['avg_hurst']}")
