...
from strategy_volatility_position import (
    Mt5VolatilityFeed, GoldVolatilityPositionStrategy, load_mt5_csv, prepare_volatility_features,
)
...
def load_data(config):
    ...
    df = prepare_volatility_features(raw, config.get('params', {}))
...
def build_cerebro(frame, config):
    ...
    feed = Mt5VolatilityFeed(dataname=frame['data'], timeframe=bt.TimeFrame.Days, compression=1)
    cerebro.adddata(feed, name=f"{config['data']['symbol']}_{config['data']['timeframe']}")
    cerebro.addstrategy(GoldVolatilityPositionStrategy, **config.get('params', {}))
