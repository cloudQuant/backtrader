...
class WlxBW5Zone(bt.Indicator):
    lines = ('sell', 'buy')
    params = dict(dir_mode='ON')
    ...
    def next(self):
        ...
        ao_up = all(float(self.ao[-i]) > float(self.ao[-(i + 1)]) for i in range(0, 4))
        ac_up = all(float(self.ac[-i]) > float(self.ac[-(i + 1)]) for i in range(0, 4))
        ao_dn = all(float(self.ao[-i]) < float(self.ao[-(i + 1)]) for i in range(0, 4))
        ac_dn = all(float(self.ac[-i]) < float(self.ac[-(i + 1)]) for i in range(0, 4))
        rng = float(self.atr[0]) * 3.0 / 8.0
        if not self.flag_up and ao_up and ac_up:
            if str(self.p.dir_mode).upper() == 'ON':
                self.lines.buy[0] = float(self.data.low[0]) - rng
            else:
                self.lines.sell[0] = float(self.data.high[0]) + rng
            self.flag_up = True
        if not self.flag_down and ao_dn and ac_dn:
            if str(self.p.dir_mode).upper() == 'ON':
                self.lines.sell[0] = float(self.data.high[0]) + rng
            else:
                self.lines.buy[0] = float(self.data.low[0]) - rng
            self.flag_down = True
        if float(self.ao[0]) < float(self.ao[-1]) or float(self.ac[0]) < float(self.ac[-1]):
            self.flag_up = False
        if float(self.ao[0]) > float(self.ao[-1]) or float(self.ac[0]) > float(self.ac[-1]):
            self.flag_down = False


class ExpWlxBW5ZoneStrategy(bt.Strategy):
    params = dict(
        dir_mode='ON',
        signal_bar=1,
    ...
    def __init__(self):
        ...
        self.ind = WlxBW5Zone(self.signal_data, dir_mode=self.p.dir_mode)
        self.bar_num = 0
        self.signal_count = 0
        self.trade_count = 0
        self.buy_count = 0
    ...
    def next(self):
        ...
        if len(self.signal_data) < 45 + sb + 2:
            return
