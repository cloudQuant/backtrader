...
from strategy_adaptive_vix_ma import (
    Mt5AdaptiveVixMaFeed, AdaptiveVixMaStrategy, load_mt5_csv, prepare_adaptive_vix_ma_features,
)
...
def load_data(config):
    ...
    df = prepare_adaptive_vix_ma_features(raw, config.get('params', {}))
...
def build_cerebro(frame, config):
    ...
    feed = Mt5AdaptiveVixMaFeed(dataname=frame['data'], timeframe=bt.TimeFrame.Days, compression=1)
    cerebro.adddata(feed, name=f"{config['data']['symbol']}_{config['data']['timeframe']}")
    cerebro.addstrategy(AdaptiveVixMaStrategy, **config.get('params', {}))
