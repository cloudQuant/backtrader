"""Inlined regression test for the Exp_Loco color-flip grid-trading strategy.
...
Data Used:
    ...
    bar shift. The M15 series is added as the execution feed and resampled to
    an ``H8`` indicator timeframe; the Loco indicator is computed on the H8
    feed while orders execute on the M15 feed.

Strategy Principle:
    Ports the MetaTrader EA 0948 Exp_Loco expert advisor. The Loco indicator is
    a follow-through line that tracks the applied price and assigns a binary
    color (0 = bullish, 1 = bearish) based on whether price is making
    consecutive higher or lower readings. A color flip signals a regime change,
    so the strategy buys on a flip to bullish and sells on a flip to bearish,
    optionally reversing an open position.

Strategy Logic:
    ``LocoIndicator`` produces the ``loco`` line and ``color`` flag.
    ``LocoStrategy.__init__`` wires the indicator on the resampled signal feed
    and initialises counters and risk state. ``next`` advances once per signal
    bar, checks protective stop/take exits, and opens, closes or reverses
    positions on color flips subject to the open/close permission flags.
    ``notify_order`` sets protective levels on entry fills and triggers pending
    reversals, while ``notify_trade`` counts closed trades. ``extract_metrics``
...
class LocoStrategy(bt.Strategy):
    ...
    Buys when the Loco color flips to bullish and sells when it flips to
    bearish, executing on the M15 feed while reading the resampled-timeframe
    signal. Positions carry fixed stop-loss and take-profit offsets, and an
    opposite signal can close and reverse the current position.
    """
    ...
    def next(self):
        """Advance one bar, evaluate signal flips, and place/close orders."""
        self.bar_num += 1
    ...
    def notify_order(self, order):
        """Track order lifecycle, completion, and cancellation counters."""
    ...
    def notify_trade(self, trade):
        """Increment closed-trade count and clear risk state when flat."""
...
def resolve_data_path(filename):
    """Resolve a strategy data path relative to this test file and validate it exists."""
...
def load_backtest_frame(config):
    """Load strategy data and return data frame plus configured date bounds."""
...
def build_cerebro(config, frame):
    """Build Cerebro with execution feed, optional resampled signal feed, strategy, and analyzers."""
...
def extract_metrics(strat, cerebro, frame, config):
    """Collect performance and trade statistics for assertion and reporting."""
