...
from strategy_rndtrade import Mt5PandasFeed, RndTradeStrategy, load_mt5_csv
...
def build_cerebro(config, frame):
    ...
    feed = Mt5PandasFeed(dataname=frame['data'], timeframe=bt.TimeFrame.Minutes, compression=params.get('compression_minutes', 15))
    cerebro.adddata(feed, name=f"{config['data']['symbol']}_{config['data']['timeframe']}")
    cerebro.addstrategy(RndTradeStrategy, **params)
    cerebro.addanalyzer(bt.analyzers.SharpeRatio, _name='sharpe', timeframe=bt.TimeFrame.Minutes, factor=MINUTES_PER_TRADING_YEAR, annualize=True, riskfreerate=0)
    cerebro.addanalyzer(bt.analyzers.Returns, _name='returns', timeframe=bt.TimeFrame.Minutes, compression=params.get('compression_minutes', 15), tann=MINUTES_PER_TRADING_YEAR)
