"""Inlined regression test for tests/functional/strategies/risk_management/regression/0013_0300_crossing_moving_average.
...
def test_13_0013_0300_crossing_moving_average() -> None:
    """Migrated regression test (runonce=True only).

    Originally located at tests/functional/strategies_regression/risk_management/0013_0300_crossing_moving_average.
    """
    captured = {}
    ...
    assert metrics.get('trade_num') == 97, f"trade_num: expected=97, got={metrics.get('trade_num')!r}"
    assert metrics.get('total_trades') == 97, f"total_trades: expected=97, got={metrics.get('total_trades')!r}"
    _close(metrics.get('final_value'), 1002125.6000000003, tol=1.002126e+00, key='final_value')
    _close(metrics.get('initial_cash'), 1000000.0, tol=1.000000e+00, key='initial_cash')
    _close(metrics.get('max_drawdown'), 0.07520861460901036, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('annual_return'), 0.6181340231028206, tol=1.000000e-06, key='annual_return')
    _close(metrics.get('return_rate'), 0.0021233441085033167, tol=1.000000e-06, key='return_rate')
    _close(metrics.get('sharpe_ratio'), 11.351736678395831, tol=1.135174e-05, key='sharpe_ratio')
    _close(metrics.get('sqn'), 1.5030506843205778, tol=1.503051e-06, key='sqn')
