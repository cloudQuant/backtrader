...
def prepare_risk_premium_value_features(df, params):
    ...
    cum_return = out['close'].pct_change(return_lookback)
    out['risk_premium'] = cum_return / vol.replace(0, np.inf)
    out['signal'] = (out['risk_premium'] > 0).astype(float)
    ...
    out = out[['open', 'high', 'low', 'close', 'volume', 'openinterest',
               'risk_premium', 'signal', 'rebalance_flag']].copy()
    return out.dropna()
