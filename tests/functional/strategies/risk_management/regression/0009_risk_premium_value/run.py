...
from strategy_risk_premium_value import (
    Mt5RiskPremiumValueFeed, RiskPremiumValueStrategy, load_mt5_csv, prepare_risk_premium_value_features,
)
...
def load_data(config):
    ...
    df = prepare_risk_premium_value_features(raw, config.get('params', {}))
...
def build_cerebro(frame, config):
    ...
    feed = Mt5RiskPremiumValueFeed(dataname=frame['data'], timeframe=bt.TimeFrame.Days, compression=1)
    cerebro.adddata(feed, name=f"{config['data']['symbol']}_{config['data']['timeframe']}")
    cerebro.addstrategy(RiskPremiumValueStrategy, **config.get('params', {}))
