...
tests/functional/strategies/multi_indicator_system/regression/0016_macd_stochastic.
...
Data Used:
    ...
    Compression follows ``data.compression`` and matches the strategy timeframe.

Strategy Principle:
    The strategy triggers long/short entries on MACD momentum crossovers, optionally
    filtering with stochastic confirmation and restricting signals to configured
    intraday windows.

Strategy Logic:
    It initializes MACD, Stochastic, and risk parameters, validates trading windows
    in ``next()``, submits entries with stop-loss/take-profit, applies trailing
    stop logic when enabled, and reports outcomes via callbacks and analyzers.
"""
...
def load_mt5_csv(filepath, fromdate=None, todate=None, bar_shift_minutes=0):
    """Load MT5 TSV data into a Backtrader-ready DataFrame.
...
class Mt5PandasFeed(bt.feeds.PandasData):
    """PandasData adapter for the MT5 CSV-style OHLCV series."""
    params = (
...
class MacdStochasticStrategy(bt.Strategy):
    """MACD momentum strategy with stochastic confirmation and time-window filtering."""
    params = dict(
        fast_ema_period=12,
        slow_ema_period=26,
        signal_period=9,
    ...
    def next(self):
        """Advance position management and apply entry/exit logic each bar."""
        self.bar_num += 1
    ...
    def notify_trade(self, trade):
        """Count closed trades and update win/loss statistics."""
...
def resolve_data_path(filename):
    """Resolve a dataset filename relative to this test file."""
...
def load_backtest_frame(config):
    """Load and return the strategy dataframe for the configured timeframe."""
...
def build_cerebro(config, frame):
    """Build and configure Cerebro with feed, strategy parameters and analyzers."""
...
def extract_metrics(strat, cerebro, frame, config):
    """Collect trade statistics and analyzer values for validation."""
...
def run(plot=False):
    """Execute the backtest and return result objects and computed metrics.
