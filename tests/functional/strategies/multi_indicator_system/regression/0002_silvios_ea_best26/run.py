...
from strategy_silvios_ea_best26 import Mt5PandasFeed, SilviosEABest26Strategy, load_mt5_csv
...
def print_report(metrics):
    print('\n' + '=' * 60)
    print('BACKTEST RESULTS — SilviosEAbest26')
    print('=' * 60)
    ...
    print(f"  Sell signals:      {metrics['sell_count']}")
...
def write_local_result(metrics):
    path = BASE_DIR / 'backtest_result.json'
    with open(path, 'w', encoding='utf-8') as handle:
        json.dump({key: normalize_metric(value) for key, value in metrics.items()}, handle, ensure_ascii=False, indent=2)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='SilviosEAbest26 Backtest Runner')
