...
from strategy_quant_probability_ea import Mt5PandasFeed, QuantProbabilityStrategy, load_mt5_csv
...
def print_report(metrics):
    print('\n' + '=' * 60)
    print('BACKTEST RESULTS — Quant Probability EA')
    print('=' * 60)
    ...
    print(f"  Sell entries:      {metrics['sell_count']}")
    print(f"  Closed trades:     {metrics['trade_count']}")
    print(f"  Bull probability:  {metrics['bull_probability']:.2f}%")
    print(f"  Bear probability:  {metrics['bear_probability']:.2f}%")
    print(f"  Open position:     {metrics['open_position_size']}")
    ...
    print(f"  Profit factor:     {format_metric(metrics['profit_factor'])}")
    print('-' * 60)
    print(f"  Sharpe ratio:      {format_metric(metrics['sharpe_ratio'])}")
    annual_return_text = format_metric(metrics['annual_return_pct'])
    print(f"  Annual return:     {annual_return_text if annual_return_text == 'N/A' else annual_return_text + '%'}")
    print(f"  Max drawdown:      {metrics['max_drawdown']:.2f}%")
    print(f"  SQN:               {format_metric(metrics['sqn'])}")
    print('=' * 60)
...
def write_local_result(metrics):
    output_path = BASE_DIR / 'backtest_result.json'
    with open(output_path, 'w', encoding='utf-8') as f:
        json.dump({key: normalize_metric(value) for key, value in metrics.items()}, f, indent=2, ensure_ascii=False)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Quant Probability EA Backtest Runner')
