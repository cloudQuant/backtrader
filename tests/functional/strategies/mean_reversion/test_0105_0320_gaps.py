"""Inlined regression test for tests/functional/strategies/mean_reversion/regression/0105_0320_gaps.
...
Strategy Principle:
    - This strategy ("Gaps Strategy") implements a gap-fill mean reversion trading system.
    - Market Assumptions: Gaps formed at market openings or between intraday bars are prone to be "filled" as prices revert to previous baseline levels.
    - Entry Signals:
        - Buy Entry (gap down): Current bar's Open price is lower than previous bar's Low price by at least `gap_pips` (1 pip).
        - Sell Entry (gap up): Current bar's Open price is higher than previous bar's High price by at least `gap_pips` (1 pip).
    - Exit Signals:
        - Target Exits: Fixed Stop Loss (50 pips, `stoploss_pips`) and Take Profit (50 pips, `takeprofit_pips`).
...
_CONFIG = {
    'strategy': {
        'name': 'Gaps',
        'source_ea': 'ea/0320_Gaps/gaps.mq5',
    },
    'data': {
        'symbol': 'XAUUSD',
        'base_timeframe': 'M15',
...
class GapsStrategy(bt.Strategy):
    """Strategy class implementing Gaps breakout mean reversion.
...
def test_105_0105_0320_gaps() -> None:
    """Migrated regression test (runonce=True only).

    Originally located at tests/functional/strategies_regression/mean_reversion/0105_0320_gaps.
    """
    config = load_config()
    ...
    assert metrics.get('trade_num') == 59, f"trade_num: expected=59, got={metrics.get('trade_num')!r}"
    assert metrics.get('total_trades') == 59, f"total_trades: expected=59, got={metrics.get('total_trades')!r}"
    _close(metrics.get('final_value'), 998581.1999999998, tol=9.985812e-01, key='final_value')
    _close(metrics.get('initial_cash'), 1000000.0, tol=1.000000e+00, key='initial_cash')
    _close(metrics.get('max_drawdown'), 0.17280488497540242, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('annual_return'), -0.2751649719980809, tol=1.000000e-06, key='annual_return')
    _close(metrics.get('return_rate'), -0.001419807449746049, tol=1.000000e-06, key='return_rate')
    _close(metrics.get('sharpe_ratio'), -14.703141206173143, tol=1.470314e-05, key='sharpe_ratio')
    _close(metrics.get('sqn'), -1.988587872570162, tol=1.988588e-06, key='sqn')
