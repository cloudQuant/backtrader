"""Inlined regression test for tests/functional/strategies/mean_reversion/regression/0085_0267_ft_cci.
...
_CONFIG = {
    'strategy': {
        'name': 'FT_CCI',
        'source_ea': 'ea/0267_FT_CCI/ft_cci.mq5',
    },
    'data': {
        'symbol': 'XAUUSD',
        'base_timeframe': 'M15',
...
class FTCCIStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.cci = bt.indicators.SafeCCI(self.data0_feed, period=self.p.cci_period)
...
def test_85_0085_0267_ft_cci() -> None:
    """Migrated regression test (runonce=True only).

    Originally located at tests/functional/strategies_regression/mean_reversion/0085_0267_ft_cci.
    """
    captured = {}
    ...
    assert metrics.get('trade_num') == 180, f"trade_num: expected=180, got={metrics.get('trade_num')!r}"
    assert metrics.get('total_trades') == 180, f"total_trades: expected=180, got={metrics.get('total_trades')!r}"
    _close(metrics.get('final_value'), 996918.3000000007, tol=9.969183e-01, key='final_value')
    _close(metrics.get('initial_cash'), 1000000.0, tol=1.000000e+00, key='initial_cash')
    _close(metrics.get('max_drawdown'), 0.42106593008110477, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('annual_return'), -0.5032018593911776, tol=1.000000e-06, key='annual_return')
    _close(metrics.get('return_rate'), -0.0030864582155541356, tol=1.000000e-06, key='return_rate')
    _close(metrics.get('sharpe_ratio'), -8.863281001106106, tol=8.863281e-06, key='sharpe_ratio')
    _close(metrics.get('sqn'), -1.1837441416410823, tol=1.183744e-06, key='sqn')
