"""Inlined regression test for the Exp UltraAbsolutelyNoLagLwma strategy.
...
Data Used:
    ...
    forward by 15 minutes. The signal is computed on an H4 (240-minute) frame
    resampled from the base bars; both the M15 execution feed and the H4 signal
    feed are added to cerebro, and both carry the spread column.

Strategy Principle:
    This is a port of the MT5 expert advisor ``Exp_UltraAbsolutelyNoLagLwma``.
    A low-lag weighted moving average is positioned within its recent high/low
    range to derive bulls/bears strength percentages. The dominance and slope of
    those strengths, encoded as a color state across up/down levels, signal
    momentum shifts and form the entry/exit triggers.

Strategy Logic:
    The UltraAbsolutelyNoLagLwmaColor indicator runs on the H4 signal feed and
    emits bulls, bears, and a color_idx line. Each new signal bar, after warmup,
    the strategy checks fixed stop-loss/take-profit exits against the M15
    intrabar range, then reads the current and previous color states to derive
    open/close signals. It opens long on a bullish color flip and short on a
    bearish one, closing on the opposite. Order and trade notifications maintain
    buy/sell counters and risk prices. The test forces ``runonce=True``, hooks
    any metrics extractor or derives metrics from analyzers, then asserts trade
    counts, final value, drawdown, returns, Sharpe, and SQN against
...
class ExpUltraAbsolutelyNoLagLwmaStrategy(bt.Strategy):
    """Low-lag LWMA color-flip strategy with fixed stop/take-profit risk."""
    ...
    def __init__(self):
        """Bind feeds, build the color indicator, and reset order/risk state."""
        self.data0_feed = self.datas[0]
        self.signal_feed = self.datas[1]
        self.ind = bt.indicators.UltraAbsolutelyNoLagLwmaColor(
            self.signal_feed,
    ...
    def next(self):
        """Manage risk exits then act on color-state flip signals."""
    ...
    def notify_order(self, order):
        """Track order completion/rejection and clear pending-order state.
    ...
    def notify_trade(self, trade):
        """Clear position state when a trade is closed."""
...
def resolve_data_path(filename):
    """Resolve a test-data filename against the local test directory.

    Args:
        filename: Relative or absolute path to resolve.
...
def parse_dt(value):
    """Parse an ISO date-time value into ``datetime``.

    Args:
        value: Timestamp string accepted by ``datetime.datetime.fromisoformat``.

    Returns:
        Parsed datetime, or ``None`` when value is empty.
    """
    if not value:
        return None
...
def resample_frame(df, minutes):
    """Resample OHLCV bars to a larger minute interval.
    ...
    Returns:
        Resampled dataframe with OHLCV/openinterest/spread fields.
    """
    rule = f'{minutes}min'
...
def load_backtest_frame(config):
    """Load and prepare source and signal dataframes for backtest.

    Args:
        config: Strategy configuration mapping.

    Returns:
        A dict containing raw data and resampled signal frames.
    """
...
def add_default_analyzers(cerebro):
    """Register standard analyzers used by the regression assertions.
...
def build_cerebro(config, frame):
    """Build a configured Cerebro engine for strategy execution.

    Args:
        config: Strategy/backtest configuration mapping.
        frame: Dictionary with prepared data and signal inputs.

    Returns:
        A Cerebro instance with feeds, strategy, commission, and analyzers.
    """
...
def finite_or_none(value):
    """Convert invalid floating values to ``None`` for safer printing.
...
def summarize(results, start_value):
    """Print a concise backtest summary block used by manual diagnostics.

    Args:
        results: Cerebro run results list.
        start_value: Starting portfolio value before the run.
    """
    strat = results[0]
...
def main():
    """Run the backtest from command line.

    Parses ``--plot`` and executes strategy run with loaded config, then prints
    summary output.
    """
