"""Inlined regression test for tests/functional/strategies/mean_reversion/regression/0067_0233_exp_jfatlcandle_mmrec.
...
_CONFIG = {
    'strategy': {
        'name': 'Exp JFatlCandle MMRec',
        'source_ea': 'ea/0233_Exp_JFatlCandle_MMRec/Exp_JFatlCandle_MMRec.mq5',
    },
    'data': {
        'symbol': 'XAUUSD',
        'base_timeframe': 'M15',
...
def test_67_0067_0233_exp_jfatlcandle_mmrec() -> None:
    """Migrated regression test (runonce=True only).

    Originally located at tests/functional/strategies_regression/mean_reversion/0067_0233_exp_jfatlcandle_mmrec.
    """
    captured = {}
    ...
    assert metrics.get('trade_num') == 14, f"trade_num: expected=14, got={metrics.get('trade_num')!r}"
    assert metrics.get('total_trades') == 14, f"total_trades: expected=14, got={metrics.get('total_trades')!r}"
    _close(metrics.get('final_value'), 999804.5000000002, tol=9.998045e-01, key='final_value')
    _close(metrics.get('initial_cash'), 1000000.0, tol=1.000000e+00, key='initial_cash')
    _close(metrics.get('max_drawdown'), 0.08147185281472316, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('annual_return'), -0.04319012595066065, tol=1.000000e-06, key='annual_return')
    _close(metrics.get('return_rate'), -0.00019551911261584456, tol=1.000000e-06, key='return_rate')
    _close(metrics.get('sharpe_ratio'), -2.0211039028769417, tol=2.021104e-06, key='sharpe_ratio')
    _close(metrics.get('sqn'), -0.25921024740276427, tol=1.000000e-06, key='sqn')
