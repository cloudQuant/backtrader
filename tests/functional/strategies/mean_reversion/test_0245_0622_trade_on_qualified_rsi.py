"""Inlined regression test for tests/functional/strategies/mean_reversion/regression/0245_0622_trade_on_qualified_rsi.
...
class QualifiedRSIStrategy(bt.Strategy):
    """EA 0622: Trade on Qualified bt.indicators.RSI.
...
def test_246_0245_0622_trade_on_qualified_rsi() -> None:
    """Migrated regression test (runonce=True only).

    Originally located at tests/functional/strategies_regression/mean_reversion/0245_0622_trade_on_qualified_rsi.
    """
    ...
    assert metrics.get('buy_count') == 283, f"buy_count: expected=283, got={metrics.get('buy_count')!r}"
    assert metrics.get('sell_count') == 804, f"sell_count: expected=804, got={metrics.get('sell_count')!r}"
    assert metrics.get('total_trades') == 1087, f"total_trades: expected=1087, got={metrics.get('total_trades')!r}"
    assert metrics.get('trade_count') == 1087, f"trade_count: expected=1087, got={metrics.get('trade_count')!r}"
    assert metrics.get('won') == 520, f"won: expected=520, got={metrics.get('won')!r}"
    assert metrics.get('lost') == 567, f"lost: expected=567, got={metrics.get('lost')!r}"
    _close(metrics.get('bars'), 6129.0, tol=6.129000e-03, key='bars')
    _close(metrics.get('signal_count'), 1088.0, tol=1.088000e-03, key='signal_count')
    _close(metrics.get('completed_orders'), 2174.0, tol=2.174000e-03, key='completed_orders')
    _close(metrics.get('rejected_orders'), 0.0, tol=1.000000e-06, key='rejected_orders')
    _close(metrics.get('initial_cash'), 1000000.0, tol=1.000000e+00, key='initial_cash')
    _close(metrics.get('final_value'), 999335.2999999975, tol=9.993353e-01, key='final_value')
    _close(metrics.get('net_pnl'), -664.7000000025146, tol=6.647000e-04, key='net_pnl')
    _close(metrics.get('total_return_pct'), -0.0664700000002516, tol=1.000000e-06, key='total_return_pct')
    _close(metrics.get('win_rate'), 47.83808647654094, tol=4.783809e-05, key='win_rate')
    _close(metrics.get('profit_factor'), 0.982925939625583, tol=1.000000e-06, key='profit_factor')
    _close(metrics.get('sharpe_ratio'), -1.2249563178360594, tol=1.224956e-06, key='sharpe_ratio')
    _close(metrics.get('annual_return_pct'), -3.8603160113017196, tol=3.860316e-06, key='annual_return_pct')
    _close(metrics.get('max_drawdown'), 0.5014685200524333, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('sqn'), -0.16353223293372396, tol=1.000000e-06, key='sqn')
