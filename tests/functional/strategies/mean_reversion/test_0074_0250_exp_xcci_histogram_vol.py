"""Inlined regression test for tests/functional/strategies/mean_reversion/regression/0074_0250_exp_xcci_histogram_vol.
...
_CONFIG = {
    'strategy': {
        'name': 'Exp XCCI Histogram Vol',
        'source_ea': 'ea/0250_Exp_XCCI_Histogram_Vol/Exp_XCCI_Histogram_Vol.mq5',
    },
    'data': {
        'symbol': 'XAUUSD',
        'base_timeframe': 'M15',
...
class ExpXCCIHistogramVolStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.indicator = bt.indicators.XCCIHistogramVolIndicator(
            self.signal_data,
...
def test_74_0074_0250_exp_xcci_histogram_vol() -> None:
    """Migrated regression test (runonce=True only).

    Originally located at tests/functional/strategies_regression/mean_reversion/0074_0250_exp_xcci_histogram_vol.
    """
    captured = {}
    ...
    assert metrics.get('trade_num') == 27, f"trade_num: expected=27, got={metrics.get('trade_num')!r}"
    assert metrics.get('total_trades') == 27, f"total_trades: expected=27, got={metrics.get('total_trades')!r}"
    _close(metrics.get('final_value'), 1000092.6000000006, tol=1.000093e+00, key='final_value')
    _close(metrics.get('initial_cash'), 1000000.0, tol=1.000000e+00, key='initial_cash')
    _close(metrics.get('max_drawdown'), 0.11710557259339363, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('annual_return'), 0.021129359316594737, tol=1.000000e-06, key='annual_return')
    _close(metrics.get('return_rate'), 9.259571288520917e-05, tol=1.000000e-06, key='return_rate')
    _close(metrics.get('sharpe_ratio'), 0.5035285552712665, tol=1.000000e-06, key='sharpe_ratio')
    _close(metrics.get('sqn'), 0.07565893297680384, tol=1.000000e-06, key='sqn')
