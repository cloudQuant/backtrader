"""Inlined regression test for tests/functional/strategies/mean_reversion/regression/0181_0943_colormetro_wpr.
...
_CONFIG = {
    'strategy': {
        'name': 'Exp_ColorMETRO_WPR',
        'source_ea': 'ea/0943_Exp_ColorMETRO_WPR',
    },
    'data': {
        'symbol': 'XAUUSD',
        'timeframe': 'M15',
...
def test_182_0181_0943_colormetro_wpr() -> None:
    ...
    assert metrics.get('bar_num') == 5827, f"bar_num: expected=5827, got={metrics.get('bar_num')!r}"
    ...
    _close(metrics.get('completed_order_count'), 73.0, tol=7.300000e-05, key='completed_order_count')
    _close(metrics.get('rejected_order_count'), 0.0, tol=1.000000e-06, key='rejected_order_count')
    _close(metrics.get('initial_cash'), 1000000.0, tol=1.000000e+00, key='initial_cash')
    _close(metrics.get('final_value'), 1000861.7000000007, tol=1.000862e+00, key='final_value')
    _close(metrics.get('net_pnl'), 861.7000000006519, tol=8.617000e-04, key='net_pnl')
    _close(metrics.get('total_return_pct'), 0.0861328949696363, tol=1.000000e-06, key='total_return_pct')
    _close(metrics.get('win_rate'), 38.88888888888889, tol=3.888889e-05, key='win_rate')
    _close(metrics.get('sharpe_ratio'), 1.7206070265728979, tol=1.720607e-06, key='sharpe_ratio')
    _close(metrics.get('annual_return_pct'), 5.231071340558721, tol=5.231071e-06, key='annual_return_pct')
    _close(metrics.get('max_drawdown'), 0.36700758940251077, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('sqn'), 0.28720093747413933, tol=1.000000e-06, key='sqn')
