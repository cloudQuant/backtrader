"""Inlined regression test for tests/functional/strategies/mean_reversion/regression/0193_0968_trixcandle.

This file is generated from migration artifacts and contains the full regression
test inline to keep behavior and assertions reproducible.

Data Used:
    Reads ``{repo}/tests/datas/XAUUSD_M15.csv`` and runs from
    2025-12-03 01:15:00 to 2026-03-10 09:00:00.
    The execution feed uses M15, while indicator generation can use H4 according
    to strategy configuration.

Strategy Principle:
    Exp_TriXCandle tracks transformed TRIX OHLC values and their color states to
    detect momentum transitions for opening and closing positions.

Strategy Logic:
    1. Load source bars and resample as needed for signal generation.
    2. Compute TRIX-based candle components and derive bullish/neutral/bearish
       states.
    3. Generate long/short/close signals on signal bar transitions, managing
       order flow and stop/take-profit risk limits.
...
_CONFIG = {
    'strategy': {
        'name': 'Exp_TriXCandle',
        'source_ea': 'ea/0968_Exp_TriXCandle',
    },
    'data': {
        'symbol': 'XAUUSD',
        'timeframe': 'M15',
...
class TriXCandleStrategy(bt.Strategy):
    """Strategy orchestrating TriX candle signal-based entries and exits."""
    params = dict(
        fixed_lot=0.1,
        risk_percent=0.0,
        point=0.01,
    ...
    def __init__(self):
        ...
        self.warmup = int(self.p.trix_period) + int(self.p.signal_bar) + 4

    def log(self, text):
        """Emit a timestamped strategy debug line."""
    ...
    def next(self):
        """Execute signal processing and order management on each signal bar."""
        self.bar_num += 1
    ...
    def notify_order(self, order):
        """React to order lifecycle events and set up reverse entries when needed."""
        ...
        if not self.position:
            ...
            if reverse_direction != 0:
                self._submit_entry(reverse_direction, 'reverse after TriX candle color change')
            return
        self.order = None
...
def resolve_data_path(filename):
    """Resolve fixture path against test directory and verify existence.

    Args:
        filename: Relative path provided in config.

    Returns:
        Absolute ``Path`` for the given fixture.
    """
...
def load_backtest_frame(config):
    """Load backtest input frame using configured date range.

    Args:
        config: Migration config block.

    Returns:
        Dict containing raw frame and date bounds.
    """
...
def build_cerebro(config, frame):
    """Create Backtrader engine with execution/signal feeds and analyzers.

    Args:
        config: Combined config for backtest and strategy.
        frame: Loaded frame and metadata.
    ...
    cerebro.addstrategy(TriXCandleStrategy, **params)
...
def extract_metrics(strat, cerebro, frame, config):
    """Collect strategy/analyzer metrics for deterministic assertions.

    Args:
        strat: Strategy instance after run.
        cerebro: Finished cerebro engine.
        frame: Input metadata.
...
def test_194_0193_0968_trixcandle() -> None:
    ...
    assert metrics.get('bar_num') == 5503, f"bar_num: expected=5503, got={metrics.get('bar_num')!r}"
    ...
    _close(metrics.get('completed_order_count'), 44.0, tol=4.400000e-05, key='completed_order_count')
    _close(metrics.get('rejected_order_count'), 0.0, tol=1.000000e-06, key='rejected_order_count')
    _close(metrics.get('initial_cash'), 1000000.0, tol=1.000000e+00, key='initial_cash')
    _close(metrics.get('final_value'), 999922.5999999989, tol=9.999226e-01, key='final_value')
    _close(metrics.get('net_pnl'), -77.40000000107102, tol=7.740000e-05, key='net_pnl')
    _close(metrics.get('total_return_pct'), -0.007740299553568613, tol=1.000000e-06, key='total_return_pct')
    _close(metrics.get('win_rate'), 54.54545454545454, tol=5.454545e-05, key='win_rate')
    _close(metrics.get('sharpe_ratio'), -0.27787221632347664, tol=1.000000e-06, key='sharpe_ratio')
    _close(metrics.get('annual_return_pct'), -0.4571573704847759, tol=1.000000e-06, key='annual_return_pct')
    _close(metrics.get('max_drawdown'), 0.22094771813255698, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('sqn'), -0.043099804388131345, tol=1.000000e-06, key='sqn')
