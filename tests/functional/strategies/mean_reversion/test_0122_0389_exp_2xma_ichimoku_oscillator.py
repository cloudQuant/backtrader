...
``tests/functional/strategies/mean_reversion/regression/0122_0389_exp_2xma_ichimoku_oscillator``.

Auto-generated by ``scripts/inline_regression_tests.py``.

Data Used:
    Uses ``{repo}/tests/datas/XAUUSD_M15.csv`` on base timeframe ``M15`` with
    signal timeframe ``H4``.
    Period is ``2025-12-03 01:15:00`` to ``2026-03-10 09:00:00`` and timestamps are
    shifted by 15 minutes.

Strategy Principle:
    Build two XMA-smoothed Ichimoku-style series and derive oscillator colors from
    the sign and slope of their spread.
    Colors map to buy/sell reversals under optional close/open controls.

Strategy Logic:
    Load data, create execution and signal feeds, run both indicators and strategy
...
class Exp2XmaIchimokuOscillatorStrategy(bt.Strategy):
    ...
    def next(self):
        """Evaluate color transitions and execute close/open logic."""
    ...
    def notify_order(self, order):
        """Record opening counters and clear pending order state."""
    ...
    def notify_trade(self, trade):
        """Update trade outcome counters once a trade closes."""
    ...
    def stop(self):
        """Print concise strategy summary."""
        total = self.trade_count
        wr = (self.win_count / total * 100.0) if total else 0.0
        print('========== Exp_2XMA_Ichimoku_Oscillator Strategy Finished ==========')
...
class Mt5PandasFeed(bt.feeds.PandasData):
    """Pandas feed with spread support for execution and signal data."""
    lines = ('spread',)
    params = (
        ('datetime', None),
        ('open', 0),
...
def resample_frame(df, minutes):
    """Resample minute-based bars for signal generation."""
...
def load_backtest_frame(config: dict) -> dict:
    """Build execution and signal frames from configured data range."""
...
def add_default_analyzers(cerebro):
    """Attach analyzers used by the regression asserts."""
...
def finite_or_none(value):
    """Normalize non-finite metrics to ``None``."""
    if value is None:
        return None
...
def extract_metrics(strat, start_value, frame):
    """Collect and normalize backtest metrics."""
