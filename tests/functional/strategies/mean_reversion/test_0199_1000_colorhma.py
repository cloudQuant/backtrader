"""Inlined regression test for the Exp_ColorHMA mean-reversion strategy.
...
Data Used:
    ...
    2025-12-03 01:15:00 .. 2026-03-10 09:00:00, then resampled up to the H8
    (480-minute) indicator timeframe on which the strategy runs. The single feed
    is priced as a futures-like instrument (multiplier 100, margin 0.01).

Strategy Principle:
    Built on the Hull Moving Average (HMA), a low-lag smoother formed from two
    weighted moving averages (full and half period) combined and re-smoothed
    over the square root of the period. The "color" is the HMA slope direction.
    Trading is a slope-reversal system: when the HMA turns up after falling it is
    a bullish signal and when it turns down after rising it is bearish. Signals
    are read on the resampled H8 series, and each position is bounded by fixed
    point-based stop-loss and take-profit distances.

Strategy Logic:
    ``_weighted_ma`` computes a linearly weighted average; ``ColorHMA`` builds
    the HMA line and its direction. ``ColorHMAStrategy`` derives open/close
    signals from the last three HMA values, manages protective stop/target
    exits, and opens or reverses positions on reversals. ``notify_order`` records
    fills, sets stop/target prices, and counts rejects; ``notify_trade`` tallies
    wins and losses. Module-level helpers load the CSV, resample to H8, build the
...
_CONFIG = {
    'strategy': {
        'name': 'Exp_ColorHMA',
        'source_ea': 'ea/1000_Exp_ColorHMA',
    },
    'data': {
        'symbol': 'XAUUSD',
        'timeframe': 'M15',
...
class ColorHMAStrategy(Strategy):
    """Trade ColorHMA slope reversals with fixed stop-loss and take-profit."""
    ...
    def next(self):
        """Manage protective exits, then open or reverse on HMA slope reversals."""
        self.bar_num += 1
...
def test_200_0199_1000_colorhma() -> None:
    ...
    assert metrics.get('bar_num') == 181, f"bar_num: expected=181, got={metrics.get('bar_num')!r}"
    ...
    _close(metrics.get('completed_order_count'), 48.0, tol=4.800000e-05, key='completed_order_count')
    _close(metrics.get('rejected_order_count'), 0.0, tol=1.000000e-06, key='rejected_order_count')
    _close(metrics.get('initial_cash'), 1000000.0, tol=1.000000e+00, key='initial_cash')
    _close(metrics.get('final_value'), 1002360.1, tol=1.002360e+00, key='final_value')
    _close(metrics.get('net_pnl'), 2360.0999999999767, tol=2.360100e-03, key='net_pnl')
    _close(metrics.get('total_return_pct'), 0.2357319338228817, tol=1.000000e-06, key='total_return_pct')
    _close(metrics.get('win_rate'), 41.66666666666667, tol=4.166667e-05, key='win_rate')
    _close(metrics.get('sharpe_ratio'), 33.23064444414605, tol=3.323064e-05, key='sharpe_ratio')
    _close(metrics.get('annual_return_pct'), 7421.2951363065995, tol=7.421295e-03, key='annual_return_pct')
    _close(metrics.get('max_drawdown'), 0.2259491892689791, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('sqn'), 0.6546575808272646, tol=1.000000e-06, key='sqn')
