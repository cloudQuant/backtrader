"""Inlined regression test for tests/functional/strategies/mean_reversion/regression/0174_0778_exp_mfi.
...
def test_174_0174_0778_exp_mfi() -> None:
    """Migrated regression test (runonce=True only).

    Originally located at tests/functional/strategies_regression/mean_reversion/0174_0778_exp_mfi.
    """
    ...
    _close(metrics.get('final_value'), 1001200.6000000011, tol=1.001201e+00, key='final_value')
    _close(metrics.get('net_pnl'), 1200.6000000011409, tol=1.200600e-03, key='net_pnl')
    _close(metrics.get('total_return_pct'), 0.1199879856166598, tol=1.000000e-06, key='total_return_pct')
    _close(metrics.get('win_rate'), 47.61904761904761, tol=4.761905e-05, key='win_rate')
    _close(metrics.get('max_drawdown'), 0.43759437827547604, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('sharpe_ratio'), 2.116912998277498, tol=2.116913e-06, key='sharpe_ratio')
    _close(metrics.get('annual_return_pct'), 7.355102291072822, tol=7.355102e-06, key='annual_return_pct')
    _close(metrics.get('sqn'), 0.24392296669370306, tol=1.000000e-06, key='sqn')
