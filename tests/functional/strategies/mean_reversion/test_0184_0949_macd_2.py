"""Inlined regression test for tests/functional/strategies/mean_reversion/regression/0184_0949_macd_2.
...
def test_185_0184_0949_macd_2() -> None:
    ...
    assert metrics.get('bar_num') == 5567, f"bar_num: expected=5567, got={metrics.get('bar_num')!r}"
    ...
    _close(metrics.get('final_value'), 997550.799999999, tol=9.975508e-01, key='final_value')
    _close(metrics.get('net_pnl'), -2449.200000001001, tol=2.449200e-03, key='net_pnl')
    _close(metrics.get('total_return_pct'), -0.2452204196575681, tol=1.000000e-06, key='total_return_pct')
    _close(metrics.get('win_rate'), 51.85185185185185, tol=5.185185e-05, key='win_rate')
    _close(metrics.get('sharpe_ratio'), -6.509228747966225, tol=6.509229e-06, key='sharpe_ratio')
    _close(metrics.get('annual_return_pct'), -13.51196349398548, tol=1.351196e-05, key='annual_return_pct')
    _close(metrics.get('max_drawdown'), 0.4089119696623942, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('sqn'), -1.0253899339033206, tol=1.025390e-06, key='sqn')
