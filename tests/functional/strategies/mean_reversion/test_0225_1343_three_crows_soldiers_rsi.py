"""Inlined regression test for tests/functional/strategies/mean_reversion/regression/0225_1343_three_crows_soldiers_rsi.
...
Strategy Principle:
    - This strategy implements a candlestick pattern recognition system (3 Black Crows / 3 White Soldiers) confirmed by bt.indicators.RSI.
    - Market Assumptions: Reversal candlestick patterns (like 3 Black Crows or 3 White Soldiers) signal strong trend turnarounds, especially when confirmed by overbought/oversold extremes in the Relative Strength Index (RSI).
    - Indicators:
        - RSI: Relative Strength Index (37-period, `rsi_period`).
        - SMA: Simple Moving Average (51-period, `ma_period`) of body sizes.
    - Entry Signals:
        - Buy Entry: 3 White Soldiers pattern is identified AND RSI is oversold (< 40).
        - Sell Entry: 3 Black Crows pattern is identified AND RSI is overbought (> 60).
    - Exit Signals:
        - Long Exit: RSI crosses up through 30 or up through 70.
        - Short Exit: RSI crosses down through 70 or down through 30.
"""
...
_CONFIG = {
    'strategy': {
        'name': '3 Crows/Soldiers + RSI',
        'source_ea': 'ea/1343_Three_Crows_Soldiers_RSI_Strategy/expert_abc_ws_rsi.mq5',
    },
    'data': {
        'symbol': 'XAUUSD',
        'timeframe': 'M15',
...
def test_226_0225_1343_three_crows_soldiers_rsi() -> None:
    """Migrated regression test (runonce=True only).

    Originally located at tests/functional/strategies_regression/mean_reversion/0225_1343_three_crows_soldiers_rsi.
    """
    ...
    _close(metrics.get('final_value'), 997916.5999999996, tol=9.979166e-01, key='final_value')
    _close(metrics.get('net_pnl'), -2083.4000000003725, tol=2.083400e-03, key='net_pnl')
    _close(metrics.get('total_return_pct'), -0.20834000000004016, tol=1.000000e-06, key='total_return_pct')
    ...
    _close(metrics.get('max_drawdown'), 0.2914470788598308, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('sharpe_ratio'), -11.281919822981799, tol=1.128192e-05, key='sharpe_ratio')
    _close(metrics.get('annual_return_pct'), -11.616124714112136, tol=1.161612e-05, key='annual_return_pct')
