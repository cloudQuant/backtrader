"""Inlined regression test for tests/functional/strategies/mean_reversion/regression/0226_1344_three_crows_soldiers_mfi.
...
_CONFIG = {
    'strategy': {
        'name': '3 Crows/Soldiers + MFI',
        'source_ea': 'ea/1344_Three_Crows_Soldiers_MFI_Strategy/expert_abc_ws_mfi.mq5',
    },
    'data': {
        'symbol': 'XAUUSD',
        'timeframe': 'M15',
...
def test_227_0226_1344_three_crows_soldiers_mfi() -> None:
    """Migrated regression test (runonce=True only).

    Originally located at tests/functional/strategies_regression/mean_reversion/0226_1344_three_crows_soldiers_mfi.
    """
    config = load_config()
    ...
    _close(metrics.get('final_value'), 999408.7999999999, tol=9.994088e-01, key='final_value')
    _close(metrics.get('net_pnl'), -591.2000000000698, tol=5.912000e-04, key='net_pnl')
    _close(metrics.get('total_return_pct'), -0.059120000000012496, tol=1.000000e-06, key='total_return_pct')
    _close(metrics.get('win_rate'), 42.857142857142854, tol=4.285714e-05, key='win_rate')
    _close(metrics.get('profit_factor'), 0.4494409835389243, tol=1.000000e-06, key='profit_factor')
    _close(metrics.get('max_drawdown'), 0.5463093468808874, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('sharpe_ratio'), -1.1831995116052159, tol=1.183200e-06, key='sharpe_ratio')
    _close(metrics.get('annual_return_pct'), -3.4407675405647113, tol=3.440768e-06, key='annual_return_pct')
    _close(metrics.get('sqn'), -0.9030932605535521, tol=1.000000e-06, key='sqn')
