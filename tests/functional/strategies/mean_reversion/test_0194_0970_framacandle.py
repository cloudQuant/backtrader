"""Inlined regression test for the Exp_FrAMACandle (FrAMA Candle) strategy.
...
Data Used:
    ...
    09:00:00. Execution runs on the base M15 feed while the FrAMA signal is
    computed on an H4 (240-minute) feed resampled from the same M15 source, so
    two feeds drive the strategy.

Strategy Principle:
    The strategy builds a Fractal Adaptive Moving Average (FrAMA) candle on the
    signal timeframe by applying FrAMA smoothing to each of the open, high, low,
    and close series. The relationship between the smoothed open and close
    defines a candle color (bullish, bearish, or neutral). Color transitions act
    as trend-reversal signals: the strategy enters in the new trend direction and
    closes or reverses when the color flips, with fixed stop-loss and
    take-profit distances bounding each trade.

Strategy Logic:
    On each new signal bar the strategy first manages any open position against
    its stop-loss/take-profit levels, then reads the current and prior FrAMA
    candle colors to derive buy/sell open and close flags (with fallbacks when
    indicator history is insufficient). Matching flags close opposing positions,
    optionally reverse, or open a new fixed-lot position with freshly set risk
    prices. Order notifications track entry fills, risk levels, reversals, and
    rejection counts; trade notifications tally closed trades. After
...
_CONFIG = {
    'strategy': {
        'name': 'Exp_FrAMACandle',
        'source_ea': 'ea/0970_Exp_FrAMACandle',
    },
    'data': {
        'symbol': 'XAUUSD',
        'timeframe': 'M15',
...
class Mt5PandasFeed(bt.feeds.PandasData):
    """PandasData feed mapping M15 OHLCV columns plus a spread line."""
...
class FramaCandleStrategy(bt.Strategy):
    """FrAMA candle color-change strategy with SL/TP and reversal handling."""
    ...
    def __init__(self):
        """Bind feeds and the FrAMA indicator, and init counters and state."""
        self.data0_feed = self.datas[0]
        self.signal_feed = self.datas[-1]
        self.indicator = bt.indicators.FramaLinesIndicator(self.signal_feed, period=self.p.frama_period)
        self.bar_num = 0
    ...
    def log(self, text):
        """Print a message prefixed with the current execution bar timestamp.
    ...
    def next(self):
        """Evaluate FrAMA color transitions and submit close/reverse/open orders.

        The method gates on one signal bar cadence and suppresses duplicate
        execution within the same signal timestamp. It checks protective exits,
        then derives buy/sell signals from current and prior candle color and
        triggers matching entries, closes, and reversals.
        """
        self.bar_num += 1
    ...
    def notify_order(self, order):
        """Update counters and risk state when orders complete, cancel, or reject.
    ...
    def notify_trade(self, trade):
        """Count closed trades and clear risk state once flattened.

        Args:
            trade: Trade object whose lifecycle changed.
...
def resolve_data_path(filename):
    """Resolve a relative strategy data path to an absolute filesystem path.

    Args:
        filename: Relative data filename configured under ``_CONFIG['data']['file']``.
...
def load_backtest_frame(config):
    """Load base bars and return frame metadata required by the backtest.

    Args:
        config: Loaded inline configuration dictionary.

    Returns:
        Dictionary containing loaded frame data plus ``fromdate`` and ``todate``.
    """
...
def build_cerebro(config, frame):
    """Construct a Backtrader engine for the FrAMA candle regression.
...
def extract_metrics(strat, cerebro, frame, config):
    """Extract and normalize strategy and analyzer metrics.

    Args:
        strat: Completed strategy instance.
        cerebro: Engine after running.
        frame: Backtest frame dictionary.
        config: Strategy/backtest configuration.

    Returns:
        Dictionary of metrics checked by this regression.
    """
...
def test_195_0194_0970_framacandle() -> None:
    """Migrated regression test (runonce=True only).

    Originally located at tests/functional/strategies_regression/mean_reversion/0194_0970_framacandle.
    """
    ...
    assert metrics.get('loss_count') == 114, f"loss_count: expected=114, got={metrics.get('loss_count')!r}"
    assert metrics.get('total_trades') == 240, f"total_trades: expected=240, got={metrics.get('total_trades')!r}"
    assert metrics.get('trade_count') == 240, f"trade_count: expected=240, got={metrics.get('trade_count')!r}"
    _close(metrics.get('bars'), 6129.0, tol=6.129000e-03, key='bars')
    _close(metrics.get('buy_signal_count'), 241.0, tol=2.410000e-04, key='buy_signal_count')
    _close(metrics.get('sell_signal_count'), 0.0, tol=1.000000e-06, key='sell_signal_count')
    _close(metrics.get('completed_order_count'), 481.0, tol=4.810000e-04, key='completed_order_count')
    _close(metrics.get('rejected_order_count'), 0.0, tol=1.000000e-06, key='rejected_order_count')
    _close(metrics.get('initial_cash'), 1000000.0, tol=1.000000e+00, key='initial_cash')
    _close(metrics.get('final_value'), 1007488.7000000046, tol=1.007489e+00, key='final_value')
    _close(metrics.get('net_pnl'), 7488.70000000461, tol=7.488700e-03, key='net_pnl')
    _close(metrics.get('total_return_pct'), 0.7460798894913765, tol=1.000000e-06, key='total_return_pct')
    _close(metrics.get('win_rate'), 52.5, tol=5.250000e-05, key='win_rate')
    _close(metrics.get('sharpe_ratio'), 6.8511168639915265, tol=6.851117e-06, key='sharpe_ratio')
    _close(metrics.get('annual_return_pct'), 55.52865746670267, tol=5.552866e-05, key='annual_return_pct')
    _close(metrics.get('max_drawdown'), 1.094455957786889, tol=1.094456e-06, key='max_drawdown')
    _close(metrics.get('sqn'), 0.9552245385302753, tol=1.000000e-06, key='sqn')
