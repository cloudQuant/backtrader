"""Regression test for `tests/functional/strategies/mean_reversion/regression/0270_0931_mfi_histogram`.
...
Data Used:
    Uses `{repo}/tests/datas/XAUUSD_M15.csv` as the raw input.
    Data are filtered from 2025-12-03 01:15:00 to 2026-03-10 09:00:00.
    Execution feed is M15 and signal feed is resampled from H4 candles.

Strategy Principle:
    MFI histogram state changes are derived from rising/falling money-flow pressure
    and compared against high/low level thresholds.
    Color transitions drive buy/sell initiation, with optional inverse-side close flags.

Strategy Logic:
    Historical bars are loaded with `_resolve_repo_paths()`, indicator feed is built
    through `build_signal_frame()`, and Backtrader executes both base and signal data.
    `ExpMFIHistogramStrategy` interprets indicator color transitions, submits entries,
...
_CONFIG = {
    'strategy': {
        'name': 'Exp_MFI_Histogram',
        'source_ea': 'ea/0931_Exp_MFI_Histogram',
    },
    'data': {
        'symbol': 'XAUUSD',
        'timeframe': 'M15',
...
class Mt5PandasFeed(bt.feeds.PandasData):
    """Base data feed mapping for M15 OHLCV input."""
...
class ExpMFIHistogramStrategy(bt.Strategy):
    ...
    def log(self, text):
        """Log signal/trade messages with timestamp."""
        dt = bt.num2date(self.base.datetime[0])
        print(f'{dt.isoformat()}, {text}')

    def _check_exit_levels(self):
        """Close positions when stop-loss/take-profit conditions are met."""
    ...
    def next(self):
        """Evaluate color transitions and issue close/entry orders."""
        self.bar_num += 1
        if len(self.base) < 2:
            return
    ...
    def notify_trade(self, trade):
        """Count opened/closed trades and win-loss outcomes."""
...
def load_backtest_frame(config):
    """Load backtest input data frame for configured symbol and date range."""
...
def build_signal_frame(df, indicator_minutes):
    """Resample base data into indicator resolution for signal calculations."""
...
def build_cerebro(config, frame):
    """Construct and configure Backtrader engine with strategy and analyzers."""
...
def test_271_0270_0931_mfi_histogram() -> None:
    ...
    _close(metrics.get('final_value'), 997557.6000000007, tol=9.975576e-01, key='final_value')
    _close(metrics.get('net_pnl'), -2442.399999999325, tol=2.442400e-03, key='net_pnl')
    _close(metrics.get('total_return_pct'), -0.24423999999992896, tol=1.000000e-06, key='total_return_pct')
    _close(metrics.get('win_rate'), 22.22222222222222, tol=2.222222e-05, key='win_rate')
    _close(metrics.get('profit_factor'), 0.4842796511750723, tol=1.000000e-06, key='profit_factor')
    _close(metrics.get('max_drawdown'), 0.25998894774510906, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('sharpe_ratio'), -14.48050915066068, tol=1.448051e-05, key='sharpe_ratio')
    _close(metrics.get('annual_return_pct'), -13.466847487356532, tol=1.346685e-05, key='annual_return_pct')
    _close(metrics.get('sqn'), -2.0192889199813626, tol=2.019289e-06, key='sqn')
