"""Inlined regression test for tests/functional/strategies/mean_reversion/regression/0086_0268_ascv.
...
_CONFIG = {
    'strategy': {
        'name': 'ASCV',
        'source_ea': 'ea/0268_ASCV/ascv.mq5',
    },
    'data': {
        'symbol': 'XAUUSD',
        'base_timeframe': 'M15',
...
class ASCVStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.signal = bt.indicators.BrainTrendSignalProxy(self.data0_feed, atr_period=self.p.atr_period, sto_period=self.p.sto_period)
...
def test_86_0086_0268_ascv() -> None:
    """Migrated regression test (runonce=True only).

    Originally located at tests/functional/strategies_regression/mean_reversion/0086_0268_ascv.
    """
    captured = {}
    ...
    assert metrics.get('loss_count') == 231, f"loss_count: expected=231, got={metrics.get('loss_count')!r}"
    assert metrics.get('trade_num') == 319, f"trade_num: expected=319, got={metrics.get('trade_num')!r}"
    assert metrics.get('total_trades') == 319, f"total_trades: expected=319, got={metrics.get('total_trades')!r}"
    _close(metrics.get('final_value'), 997486.4000000001, tol=9.974864e-01, key='final_value')
    _close(metrics.get('initial_cash'), 1000000.0, tol=1.000000e+00, key='initial_cash')
    _close(metrics.get('max_drawdown'), 0.4400758362523234, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('annual_return'), -0.434726523961346, tol=1.000000e-06, key='annual_return')
    _close(metrics.get('return_rate'), -0.002516764396276441, tol=1.000000e-06, key='return_rate')
    _close(metrics.get('sharpe_ratio'), -9.198922291460013, tol=9.198922e-06, key='sharpe_ratio')
    _close(metrics.get('sqn'), -1.2824693308823947, tol=1.282469e-06, key='sqn')
