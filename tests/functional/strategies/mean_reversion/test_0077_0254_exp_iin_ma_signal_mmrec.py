...
Data Used:
    XAUUSD M15 bars from ``tests/datas/XAUUSD_M15.csv`` for execution and signal
    generation.
    H1 signal bars are generated by resampling M15 data.
    Backtest window: ``2025-12-03 01:15:00`` through ``2026-03-10 09:00:00``.

Strategy Principle:
    A moving-average crossover indicator generates buy/sell signals while recent
    trade outcomes adjust margin mode selection under adverse streaks.

Strategy Logic:
    Load MT5 CSV data, build execution and H1 signal feeds, compute MA crossover
    levels, send bracketed entries/exits with optional reversals, and validate
...
class ExpIinMASignalMMRecStrategy(bt.Strategy):
    """MA crossover strategy with money management adaptation and reverse flow."""
    params = dict(
        point_size=0.01,
        lot_min=0.01,
        lot_max=100.0,
    ...
    def next(self):
        """Evaluate signal matrix and dispatch entries, closes, or reversals."""
    ...
    def notify_order(self, order):
        """Update state when entry or exit orders complete or are canceled.

        Args:
            order: A Backtrader order object.
        """
    ...
    def notify_trade(self, trade):
        """Track realized trade outcomes and update rolling MM memory."""
...
def resolve_data_path(filename):
    """Resolve a repository-relative data path to an absolute path.
...
def load_backtest_frame(config):
    """Load MT5 data for the configured date range.

    Args:
        config: Inline strategy config dictionary.

    Returns:
        Mapping with ``"data"`` key containing a DataFrame.
    """
...
def resample_frame(df, minutes):
    """Resample bars to a larger period and normalize aggregated fields."""
    """
    Args:
        df: Base timeframe DataFrame.
        minutes: Target minutes for resampling.
...
def build_cerebro(config, frame):
    """Build and configure cerebro for execution and signal feeds."""
    """
    Args:
        config: Inline strategy config.
...
def finite_or_none(value):
    """Convert non-finite numeric values to ``None`` for display safety."""
    """
    Args:
...
def summarize(results, start_value):
    """Print backtest summary for local verification."""
    """
    Args:
        results: Cerebro execution results.
...
def main():
    """Run the migration-style backtest and optional visualization."""
