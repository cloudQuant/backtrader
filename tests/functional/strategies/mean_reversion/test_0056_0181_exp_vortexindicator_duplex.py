"""Inlined regression test for tests/functional/strategies/mean_reversion/regression/0056_0181_exp_vortexindicator_duplex.
...
_CONFIG = {
    'strategy': {
        'name': 'Exp_VortexIndicator_Duplex',
        'source_ea': 'ea/0181_Exp_VortexIndicator_Duplex',
    },
    'data': {
        'symbol': 'XAUUSD',
        'timeframe': 'M15',
...
def test_56_0056_0181_exp_vortexindicator_duplex() -> None:
    """Migrated regression test (runonce=True only).

    Originally located at tests/functional/strategies_regression/mean_reversion/0056_0181_exp_vortexindicator_duplex.
    """
    config = load_config()
    ...
    _close(metrics.get('final_value'), 999826.5000000005, tol=9.998265e-01, key='final_value')
    _close(metrics.get('net_pnl'), -173.49999999953434, tol=1.735000e-04, key='net_pnl')
    _close(metrics.get('total_return_pct'), -0.01734999999994935, tol=1.000000e-06, key='total_return_pct')
    _close(metrics.get('win_rate'), 36.84210526315789, tol=3.684211e-05, key='win_rate')
    _close(metrics.get('profit_factor'), 0.9291316068948493, tol=1.000000e-06, key='profit_factor')
    _close(metrics.get('max_drawdown'), 0.08057633520712482, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('sharpe_ratio'), -1.5319716082282187, tol=1.531972e-06, key='sharpe_ratio')
    _close(metrics.get('annual_return_pct'), -1.0210779286826712, tol=1.021078e-06, key='annual_return_pct')
    _close(metrics.get('sqn'), -0.2032394144399051, tol=1.000000e-06, key='sqn')
