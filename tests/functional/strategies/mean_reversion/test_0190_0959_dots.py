"""Inlined regression test for tests/functional/strategies/mean_reversion/regression/0190_0959_dots.
...
def test_191_0190_0959_dots() -> None:
    ...
    assert metrics.get('bar_num') == 5292, f"bar_num: expected=5292, got={metrics.get('bar_num')!r}"
    ...
    _close(metrics.get('buy_signal_count'), 26.0, tol=2.600000e-05, key='buy_signal_count')
    _close(metrics.get('sell_signal_count'), 22.0, tol=2.200000e-05, key='sell_signal_count')
    _close(metrics.get('completed_order_count'), 95.0, tol=9.500000e-05, key='completed_order_count')
    _close(metrics.get('rejected_order_count'), 0.0, tol=1.000000e-06, key='rejected_order_count')
    _close(metrics.get('initial_cash'), 1000000.0, tol=1.000000e+00, key='initial_cash')
    _close(metrics.get('final_value'), 1003740.5000000013, tol=1.003741e+00, key='final_value')
    _close(metrics.get('net_pnl'), 3740.5000000012806, tol=3.740500e-03, key='net_pnl')
    _close(metrics.get('total_return_pct'), 0.37335217259522446, tol=1.000000e-06, key='total_return_pct')
    _close(metrics.get('win_rate'), 57.446808510638306, tol=5.744681e-05, key='win_rate')
    _close(metrics.get('sharpe_ratio'), 10.559066524525063, tol=1.055907e-05, key='sharpe_ratio')
    _close(metrics.get('annual_return_pct'), 24.734181781649863, tol=2.473418e-05, key='annual_return_pct')
    _close(metrics.get('max_drawdown'), 0.1653087343248693, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('sqn'), 1.6799988048182006, tol=1.679999e-06, key='sqn')
