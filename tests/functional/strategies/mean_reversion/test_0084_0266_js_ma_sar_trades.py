"""Inlined regression test for tests/functional/strategies/mean_reversion/regression/0084_0266_js_ma_sar_trades.
...
_CONFIG = {
    'strategy': {
        'name': 'JS MA SAR Trades',
        'source_ea': 'ea/0266_JS_MA_SAR_Trades/js_ma_sar_trades.mq5',
    },
    'data': {
        'symbol': 'XAUUSD',
        'base_timeframe': 'M15',
...
class JSMASARTradesStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.zigzag = bt.indicators.PivotZigZagProxy(self.data0_feed, depth=self.p.zigzag_depth)
...
def test_84_0084_0266_js_ma_sar_trades() -> None:
    """Migrated regression test (runonce=True only).

    Originally located at tests/functional/strategies_regression/mean_reversion/0084_0266_js_ma_sar_trades.
    """
    captured = {}
    ...
    assert metrics.get('loss_count') == 85, f"loss_count: expected=85, got={metrics.get('loss_count')!r}"
    assert metrics.get('trade_num') == 174, f"trade_num: expected=174, got={metrics.get('trade_num')!r}"
    assert metrics.get('total_trades') == 174, f"total_trades: expected=174, got={metrics.get('total_trades')!r}"
    _close(metrics.get('final_value'), 1000748.7000000003, tol=1.000749e+00, key='final_value')
    _close(metrics.get('initial_cash'), 1000000.0, tol=1.000000e+00, key='initial_cash')
    _close(metrics.get('max_drawdown'), 0.2201275458980294, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('annual_return'), 0.18487300835233053, tol=1.000000e-06, key='annual_return')
    _close(metrics.get('return_rate'), 0.0007484198639718886, tol=1.000000e-06, key='return_rate')
    _close(metrics.get('sharpe_ratio'), 3.468426521158724, tol=3.468427e-06, key='sharpe_ratio')
    _close(metrics.get('sqn'), 0.4508924927679945, tol=1.000000e-06, key='sqn')
