"""Inlined regression test for tests/functional/strategies/mean_reversion/regression/0071_0242_autotrader_momentum.
...
Data Used:
    Uses XAUUSD M15 CSV data from `tests/datas/XAUUSD_M15.csv`.

Strategy Principle:
    Momentum entries are generated by comparing the current bar with a lagged
    comparable bar. Market risk is controlled through fixed stop-loss,
    take-profit, and optional trailing stop management per open position.

Strategy Logic:
    The script loads and filters OHLCV/ spread data, builds a one-time
    in-memory Backtrader environment with the custom feed, executes the inlined
    strategy, runs analyzers, and validates migrated regression expectations from
    the captured metric dictionary.
"""
...
_CONFIG = {
    'strategy': {
        'name': 'Autotrader Momentum',
        'source_ea': 'ea/0242_Autotrader_Momentum/autotrader_momentum.mq5',
    },
    'data': {
        'symbol': 'XAUUSD',
        'base_timeframe': 'M15',
...
def load_config(*args, **kwargs):
    """
    Inlined config (was config.yaml). Accepts any args for compatibility with
    strategies that pass a path.
    """
    import copy
...
def load_mt5_csv(filepath, fromdate=None, todate=None, bar_shift_minutes=0):
    """Load an MT5 TSV data file and return normalized OHLCV DataFrame used by tests.

    Args:
        filepath: Path to the MT5 tab-separated values file.
        fromdate: Optional start timestamp for frame slicing.
        todate: Optional end timestamp for frame slicing.
        bar_shift_minutes: Optional minute offset to shift every bar timestamp.

    Returns:
        DataFrame indexed by datetime with required Backtrader columns.
    """
...
class Mt5PandasFeed(bt.feeds.PandasData):
    """
    Backtrader data feed wrapper exposing spread as an extra data line.
    """
    lines = ('spread',)
    params = (
        ('datetime', None),
...
class AutotraderMomentumStrategy(bt.Strategy):
    """
    Momentum strategy with bracket-like exits and reverse-on-close behaviour.
    """
    params = dict(
        fixed_lot=1.0,
        point_size=0.01,
    ...
    def __init__(self):
        """
        Create strategy state used for order tracking and trailing control.
        """
    ...
    def log(self, text):
        """
        Log text with execution timestamp.
        """
    ...
    def prenext(self):
        """
        Run regular next logic before minimum startup history is available.
        """
        self.next()
    ...
    def next(self):
        """
        Apply trailing updates and evaluate momentum entry/exit/reverse signals.
        """
    ...
    def notify_order(self, order):
        """
        Track order state transitions and trigger next-step flow control.
        """
    ...
    def notify_trade(self, trade):
        """
        Reset internal side and stop/take state after a closed trade.
        """
...
def resolve_data_path(filename):
    """Resolve a data file path relative to the strategy test directory.
...
def parse_dt(value):
    """Convert ISO datetime text into a ``datetime`` object.

    Args:
        value: ISO formatted datetime string or falsy value.
...
def load_backtest_frame(config):
    """Load backtest data frame from config and validate non-empty output.

    Args:
        config: Strategy configuration map.

    Returns:
        Mapping containing prepared OHLCV DataFrame under key ``data``.
    """
...
def add_default_analyzers(cerebro):
    """Attach common analyzer suite used for migrated regression assertions."""
...
def build_cerebro(config, frame):
    ...
    Args:
        ...
        frame: Dictionary containing data frame(s) returned by loader.
...
def finite_or_none(value):
    """
    Convert non-finite numeric values into ``None`` for output safety.
    """
    if value is None:
        return None
...
def summarize(results, start_value):
    """
    Print a compact backtest summary for compatibility with migrated tests.
    """
    strat = results[0]
...
def main():
    """
    Run the inlined regression backtest and print summary output.
    """
...
def test_71_0071_0242_autotrader_momentum() -> None:
    """
    Execute the migrated regression assertions for the inlined backtest.
    """
    captured = {}
    ...
    assert metrics.get('win_count') == 1457, f"win_count: expected=1457, got={metrics.get('win_count')!r}"
    assert metrics.get('loss_count') == 1601, f"loss_count: expected=1601, got={metrics.get('loss_count')!r}"
    assert metrics.get('trade_num') == 3058, f"trade_num: expected=3058, got={metrics.get('trade_num')!r}"
    assert metrics.get('total_trades') == 3058, f"total_trades: expected=3058, got={metrics.get('total_trades')!r}"
    _close(metrics.get('final_value'), 1000765.9999999981, tol=1.000766e+00, key='final_value')
    _close(metrics.get('initial_cash'), 1000000.0, tol=1.000000e+00, key='initial_cash')
    _close(metrics.get('max_drawdown'), 5.1018560124366905, tol=5.101856e-06, key='max_drawdown')
    _close(metrics.get('annual_return'), 0.18952471084301256, tol=1.000000e-06, key='annual_return')
    _close(metrics.get('return_rate'), 0.0007657067717303949, tol=1.000000e-06, key='return_rate')
    _close(metrics.get('sharpe_ratio'), 0.3269222304479731, tol=1.000000e-06, key='sharpe_ratio')
    _close(metrics.get('sqn'), 0.012255149354735127, tol=1.000000e-06, key='sqn')
