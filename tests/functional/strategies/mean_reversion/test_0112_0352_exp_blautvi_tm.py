"""Inlined regression test for the Exp BlauTVI Tm mean-reversion strategy.
...
Strategy Principle:
    This is a port of the MT5 expert advisor ``Exp_BlauTVI_Tm``. The bt.indicators.BlauTVI(Blau Tick Volume Indicator) is a triple-EMA-smoothed oscillator built from
    up-tick and down-tick volume estimates, bounded near +/-100. The strategy
    reads the indicator on the higher H4 timeframe and treats a turn in its
    smoothed value as a mean-reversion signal: a downturn that resumes upward
    flags a long, an upturn that resumes downward flags a short, with opposing
    turns closing existing positions.

Strategy Logic:
    load_backtest_frame builds the M15 base frame and resamples it to the H4
    signal frame. On each M15 bar the strategy manages stop-loss/take-profit
    exits, enforces an optional trading-time window, and acts once per new H4
    bar. After warmup it reads the indicator value at the configured signal-bar
    shift over three points, derives buy/sell open and close flags, and submits
    fixed- or risk-sized orders. notify_order tracks fills and sets entry risk
    prices; notify_trade and summarize report trade statistics. The test forces
    runonce=True, captures metrics from analyzers, and asserts each value
...
_CONFIG = {
    'strategy': {
        'name': 'Exp BlauTVI Tm',
        'source_ea': 'ea/0352_Exp_BlauTVI_Tm/Exp_BlauTVI_Tm.mq5',
    },
    'data': {
        'symbol': 'XAUUSD',
        'base_timeframe': 'M15',
...
class ExpBlauTVITmStrategy(bt.Strategy):
    """Dual-timeframe BlauTVI mean-reversion strategy with stop/take-profit risk."""
    ...
    def __init__(self):
        ...
        self.ind = bt.indicators.BlauTVI(self.signal_feed, xlength1=self.p.xlength1, xlength2=self.p.xlength2, xlength3=self.p.xlength3)
        self.order = None
    ...
    def next(self):
        """Manage exits and the trading window, then open positions on new H4 signals."""
    ...
    def notify_order(self, order):
        """Track fills, set entry risk prices, count entries, and clear order state.
...
def parse_dt(value):
    """Parse an ISO datetime string into a datetime, or return None.

    Args:
        value: ISO-formatted datetime string, or a falsy value.

    Returns:
        A datetime.datetime if ``value`` is non-empty, otherwise None.
    """
    if not value:
        return None
...
def resample_frame(df, minutes):
    """Resample an OHLCV+spread DataFrame to a coarser bar interval.

    Args:
        df: Source DataFrame indexed by datetime with a spread column.
        minutes: Target bar size in minutes.

    Returns:
        A resampled DataFrame with aggregated OHLCV plus spread, dropping bars
        without price data and filling missing open interest and spread with zero.
    """
    rule = f'{minutes}min'
...
def load_backtest_frame(config):
    """Load the M15 execution frame and the resampled H4 signal frame.

    Args:
        config: Resolved configuration dict containing a ``data`` section.

    Returns:
        A dict with the M15 ``data`` frame and the resampled ``signal`` frame.
...
def build_cerebro(config, frame):
    """Build a Cerebro instance, add feeds, strategy, and default analyzers."""
...
def finite_or_none(value):
    """Return ``None`` when a metric is missing or not finite, else value."""
    if value is None:
        return None
...
def main():
    """CLI entry point to run the strategy backtest with optional plotting."""
...
def test_112_0112_0352_exp_blautvi_tm() -> None:
    """Migrated regression test (runonce=True only).

    Originally located at tests/functional/strategies_regression/mean_reversion/0112_0352_exp_blautvi_tm.
    """
    captured = {}
    ...
    _close(metrics.get('final_value'), 1000146.7999999996, tol=1.000147e+00, key='final_value')
    _close(metrics.get('initial_cash'), 1000000.0, tol=1.000000e+00, key='initial_cash')
    _close(metrics.get('max_drawdown'), 0.09739687708398284, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('annual_return'), 0.03370225418187728, tol=1.000000e-06, key='annual_return')
    _close(metrics.get('return_rate'), 0.00014678922593396745, tol=1.000000e-06, key='return_rate')
    _close(metrics.get('sharpe_ratio'), 1.3863387740191244, tol=1.386339e-06, key='sharpe_ratio')
    _close(metrics.get('sqn'), 0.1651006420325025, tol=1.000000e-06, key='sqn')
