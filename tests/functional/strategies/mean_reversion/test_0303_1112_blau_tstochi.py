"""Regression test for `tests/functional/strategies/mean_reversion/regression/0303_1112_blau_tstochi`.

Data Used:
    - Data file: `{repo}/tests/datas/XAUUSD_M15.csv`
    - Symbol: `XAUUSD`
    - Trading timeframe: `M15` with 15-minute bar shift
    - Indicator timeframe: `H4` through resampling for oscillator input
    - Date range: `2025-12-03 01:15:00` to `2026-03-10 09:00:00`

Strategy Principle:
    - Computes a smoothed Blau T-Stoch oscillator using highest/lowest lookbacks
      and nested EMA filters.
    - In `breakdown` mode the signal flips on zero-cross transitions; in `twist`
      mode it uses three-bar shape changes.
    - Long/short entries and closes are guarded by user flags plus stop-loss /
      take-profit distance in points.

Strategy Logic:
    - Load inline configuration and resolve repository paths.
    - Parse MT5 CSV into a normalized DataFrame and apply date filtering.
    - Build a backtest with primary feed and optional indicator-timeframe resampling.
    - Run Backtrader with analyzers, collect metrics, and assert all expected snapshots.
...
_CONFIG = {
    'strategy': {
        'name': 'Blau TStochI',
        'source_ea': 'ea/1112_Exp_BlauTStochI',
    },
    'data': {
        'symbol': 'XAUUSD',
        'timeframe': 'M15',
...
def load_mt5_csv(filepath, fromdate=None, todate=None, bar_shift_minutes=0):
    """Load MT5 TSV file to normalized OHLCV DataFrame.
...
class Mt5PandasFeed(btfeeds.PandasData):
    """Pandas feed wrapper for MT5 OHLCV column mapping."""
    params = (
...
class BlauTStochIStrategy(Strategy):
    """Backtest strategy translating Blau histogram state into position actions."""
    params = dict(
        mode='twist',
        signal_bar=1,
        xlength=20,
    ...
    def __init__(self):
        """Instantiate oscillator and initialize runtime signal/trade counters."""
    ...
    def next(self):
        """Evaluate protective exits, signal generation, and order routing each bar."""
        self.bar_num += 1
    ...
    def notify_order(self, order):
        """Track order completion/rejection and compute active entry levels."""
...
def resolve_data_path(filename):
    """Resolve the file path relative to current test directory."""
...
def load_backtest_frame(config):
    """Load the main dataframe and compute from/to datetime bounds."""
...
def build_cerebro(config, frame):
    """Create a configured `Cerebro` instance with analyzers and strategy wiring."""
...
def extract_metrics(strat, cerebro, frame, config):
    """Aggregate strategy counters and analyzer outputs into verification metrics."""
...
def test_304_0303_1112_blau_tstochi() -> None:
    """Migrated regression test (runonce=True only).

    Originally located at tests/functional/strategies_regression/mean_reversion/0303_1112_blau_tstochi.
    """
    config = load_config()
    ...
    assert metrics.get('buy_count') == 85, f"buy_count: expected=85, got={metrics.get('buy_count')!r}"
    ...
    assert metrics.get('loss_count') == 82, f"loss_count: expected=82, got={metrics.get('loss_count')!r}"
    ...
    _close(metrics.get('buy_signal_count'), 98.0, tol=9.800000e-05, key='buy_signal_count')
    _close(metrics.get('sell_signal_count'), 95.0, tol=9.500000e-05, key='sell_signal_count')
    _close(metrics.get('completed_order_count'), 346.0, tol=3.460000e-04, key='completed_order_count')
    _close(metrics.get('rejected_order_count'), 0.0, tol=1.000000e-06, key='rejected_order_count')
    _close(metrics.get('initial_cash'), 1000000.0, tol=1.000000e+00, key='initial_cash')
    _close(metrics.get('final_cash'), 1005904.6000000006, tol=1.005905e+00, key='final_cash')
    _close(metrics.get('final_value'), 1005904.6000000006, tol=1.005905e+00, key='final_value')
    _close(metrics.get('net_pnl'), 5904.600000000559, tol=5.904600e-03, key='net_pnl')
    _close(metrics.get('total_return_pct'), 0.5904600000000482, tol=1.000000e-06, key='total_return_pct')
    _close(metrics.get('win_rate'), 52.601156069364166, tol=5.260116e-05, key='win_rate')
    _close(metrics.get('profit_factor'), 1.2512713361050924, tol=1.251271e-06, key='profit_factor')
    ...
    _close(metrics.get('max_drawdown'), 0.3534502967084183, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('sharpe_ratio'), 29.12130095431269, tol=2.912130e-05, key='sharpe_ratio')
    _close(metrics.get('annual_return_pct'), 20770.059198179653, tol=2.077006e-02, key='annual_return_pct')
    _close(metrics.get('sqn'), 0.9604706387654631, tol=1.000000e-06, key='sqn')
