"""Inlined regression test for tests/functional/strategies/mean_reversion/regression/0030_rsi_mean_reversion.
...
_CONFIG = {
    'strategy': {
        ...
        'source_spec': 'research_papers_gold/strategy_specs/mean_reversion/0267_RSI_Mean_Reversion_Strategy.md',
    },
    'data': {
        'symbol': 'XAUUSD',
        'timeframe': 'D1',
...
def test_30_0030_rsi_mean_reversion() -> None:
    """Migrated regression test (runonce=True only).

    Originally located at tests/functional/strategies_regression/mean_reversion/0030_rsi_mean_reversion.
    """
    config = load_config()
    ...
    assert metrics.get('buy_count') == 680, f"buy_count: expected=680, got={metrics.get('buy_count')!r}"
    assert metrics.get('sell_count') == 680, f"sell_count: expected=680, got={metrics.get('sell_count')!r}"
    assert metrics.get('win_count') == 377, f"win_count: expected=377, got={metrics.get('win_count')!r}"
    assert metrics.get('loss_count') == 303, f"loss_count: expected=303, got={metrics.get('loss_count')!r}"
    assert metrics.get('total_trades') == 680, f"total_trades: expected=680, got={metrics.get('total_trades')!r}"
    assert metrics.get('trade_count') == 680, f"trade_count: expected=680, got={metrics.get('trade_count')!r}"
    assert metrics.get('won') == 377, f"won: expected=377, got={metrics.get('won')!r}"
    assert metrics.get('lost') == 303, f"lost: expected=303, got={metrics.get('lost')!r}"
    _close(metrics.get('bars'), 4637.0, tol=4.637000e-03, key='bars')
    _close(metrics.get('initial_cash'), 1000000.0, tol=1.000000e+00, key='initial_cash')
    _close(metrics.get('final_value'), 1840626.5013919952, tol=1.840627e+00, key='final_value')
    _close(metrics.get('net_pnl'), 840626.5013919952, tol=8.406265e-01, key='net_pnl')
    _close(metrics.get('total_return_pct'), 84.06265013919952, tol=8.406265e-05, key='total_return_pct')
    _close(metrics.get('win_rate'), 55.44117647058824, tol=5.544118e-05, key='win_rate')
    _close(metrics.get('profit_factor'), 1.1415610243584349, tol=1.141561e-06, key='profit_factor')
    _close(metrics.get('max_drawdown'), 38.627473708684896, tol=3.862747e-05, key='max_drawdown')
    _close(metrics.get('sharpe_ratio'), 0.31007058317569747, tol=1.000000e-06, key='sharpe_ratio')
    _close(metrics.get('annual_return_pct'), 3.3712307550948943, tol=3.371231e-06, key='annual_return_pct')
    _close(metrics.get('sqn'), 1.191822859583781, tol=1.191823e-06, key='sqn')
    _close(metrics.get('ulcer_index'), 20.616201440968382, tol=2.061620e-05, key='ulcer_index')
