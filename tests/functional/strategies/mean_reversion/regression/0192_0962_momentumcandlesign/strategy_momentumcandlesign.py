...
class MomentumCandleSignIndicator(bt.Indicator):
    lines = ('sell_signal', 'buy_signal', 'momentum_open', 'momentum_close')
    params = dict(period=12, atr_period=15)

    def __init__(self):
        self.addminperiod(max(int(self.p.period), int(self.p.atr_period)) + 3)
        self.momentum_open = bt.indicators.Momentum(self.data.open, period=int(self.p.period))
        self.momentum_close = bt.indicators.Momentum(self.data.close, period=int(self.p.period))
...
class MomentumCandleSignStrategy(bt.Strategy):
    params = dict(
        fixed_lot=0.1,
        risk_percent=0.0,
        point=0.01,
    ...
    def __init__(self):
        ...
        self.indicator = MomentumCandleSignIndicator(
            self.signal_feed,
        ...
        self.warmup = max(int(self.p.momentum_period), int(self.p.atr_period)) + int(self.p.signal_bar) + 4
    ...
    def _signal_present(self, line, signal_bar):
        value = self._buffer_value(line, signal_bar)
    ...
    def next(self):
        ...
        if self.position and self._check_exit_levels():
            return
        buy_open = self.p.buy_pos_open and self._signal_present(self.indicator.buy_signal, self.p.signal_bar)
        sell_open = self.p.sell_pos_open and self._signal_present(self.indicator.sell_signal, self.p.signal_bar)
        ...
        if self.position.size > 0:
            if buy_close:
                ...
                self.order = self.close()
                self.log('CLOSE long MomentumCandleSign reverse/close signal')
            return
        if self.position.size < 0:
            if sell_close:
                self.pending_reverse_direction = 1 if buy_open else 0
                self.order = self.close()
                self.log('CLOSE short MomentumCandleSign reverse/close signal')
            return
        if buy_open:
            self._submit_entry(1, 'MomentumCandleSign buy dot')
            return
        if sell_open:
            self._submit_entry(-1, 'MomentumCandleSign sell dot')
            return
    ...
    def notify_order(self, order):
        ...
        if not self.position:
            ...
            if reverse_direction != 0:
                self._submit_entry(reverse_direction, 'reverse after MomentumCandleSign signal')
            return
        self.order = None
