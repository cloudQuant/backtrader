...
def compute_mfi(frame, period=25, volume_type='VOLUME_TICK'):
    typical = (frame['high'] + frame['low'] + frame['close']) / 3.0
    volume = frame['tick_volume'] if volume_type == 'VOLUME_TICK' else frame['real_volume']
    raw_flow = typical * volume
    prev_typical = typical.shift(1)
    positive = raw_flow.where(typical > prev_typical, 0.0)
    negative = raw_flow.where(typical < prev_typical, 0.0)
    pos_sum = positive.rolling(int(period), min_periods=int(period)).sum()
    neg_sum = negative.rolling(int(period), min_periods=int(period)).sum()
    money_ratio = pos_sum / neg_sum.replace(0, np.nan)
    mfi = 100 - (100 / (1 + money_ratio))
    mfi = mfi.where(~((neg_sum == 0) & (pos_sum > 0)), 100.0)
    mfi = mfi.where(~((pos_sum == 0) & (neg_sum > 0)), 0.0)
    mfi = mfi.fillna(50.0)
    return mfi
...
def compute_cronex_mfi(frame, mfi_period=25, xma_method='MODE_SMA', fast_period=14, slow_period=25, volume_type='VOLUME_TICK'):
    out = frame.copy()
    mfi = compute_mfi(out, mfi_period, volume_type)
    ind = smooth_series(mfi, fast_period, xma_method)
...
class CronexMFIFeed(bt.feeds.PandasData):
    lines = ('ind', 'sign',)
    params = (
        ('datetime', None), ('open', 0), ('high', 1), ('low', 2), ('close', 3),
...
class CronexMFIStrategy(bt.Strategy):
    params = dict(
        mm=0.1,
        mm_mode='LOT',
        stop_loss=1000,
    ...
    def next(self):
        ...
        self.log('cronex_mfi {0} buy_open={1} sell_open={2}'.format(debug, buy_open, sell_open))
