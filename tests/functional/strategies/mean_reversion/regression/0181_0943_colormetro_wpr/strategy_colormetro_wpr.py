    ...
    def next(self):
        ...
        if self.position.size > 0:
            if buy_close:
                ...
                self.order = self.close()
                self.log('CLOSE long ColorMETRO_WPR signal')
            return
        if self.position.size < 0:
            if sell_close:
                self.pending_reverse_direction = 1 if buy_open else 0
                self.order = self.close()
                self.log('CLOSE short ColorMETRO_WPR signal')
            return
        if buy_open:
            self._submit_entry(1, 'ColorMETRO_WPR bullish cloud flip')
            return
        if sell_open:
            self._submit_entry(-1, 'ColorMETRO_WPR bearish cloud flip')
            return
    ...
    def notify_order(self, order):
        ...
        if not self.position:
            ...
            if reverse_direction != 0:
                self._submit_entry(reverse_direction, 'reverse after ColorMETRO_WPR signal')
            return
        self.order = None
