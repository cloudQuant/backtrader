...
该策略是对 MT5 EA `0795_Exp_3STO` 的 Backtrader 迁移版本。
当前版本使用 `XAUUSD_M15.csv` 回测，保留了原 EA 的核心结构：

- 三个不同周期的 `Stochastic`
- 高周期与中周期定义趋势方向
- 低周期的 `Stochastic` / signal 交叉触发入场
- 任一周期趋势反向时触发离场
- 固定 `SL/TP`

## 核心逻辑

1. 计算三个不同周期上的 `Stochastic`
2. 在高周期和中周期上，用 `main > signal` 视为多头趋势，`main < signal` 视为空头趋势
3. 在低周期上，当 `main` 上穿 `signal` 且高/中周期同向看多时做多
4. 在低周期上，当 `main` 下穿 `signal` 且高/中周期同向看空时做空
...
- 原 EA 使用 `SignalBar=1` 基于已完成 K 线取信号；当前版本同样只在已完成 bar 上计算交叉
