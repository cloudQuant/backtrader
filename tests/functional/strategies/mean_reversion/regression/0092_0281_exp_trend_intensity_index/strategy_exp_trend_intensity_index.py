...
class TrendIntensityIndexProxy(bt.Indicator):
    lines = ('tii', 'color_index')
    params = dict(length1=60, length2=30, high_level=80, low_level=20)

    def __init__(self):
        self.sma = bt.indicators.SimpleMovingAverage(self.data.close, period=self.p.length1)
        self.smooth = bt.indicators.ExponentialMovingAverage(self.data.close, period=self.p.length2)
...
class ExpTrendIntensityIndexStrategy(bt.Strategy):
    params = dict(
        fixed_lot=0.1,
        point_size=0.01,
        stoploss_pips=1000,
    ...
    def __init__(self):
        ...
        self.tii = TrendIntensityIndexProxy(
            self.data0_feed,
    ...
    def next(self):
        ...
        col0 = float(self.tii.color_index[-shift])
        col1 = float(self.tii.color_index[-shift - 1])
        ...
        if self.position.size < 0 and sell_close:
            self._submit_close('tii buy transition', reverse='long' if buy_open else None)
            return
        if self.position.size > 0 and buy_close:
            self._submit_close('tii sell transition', reverse='short' if sell_open else None)
            return
        if not self.position:
            if buy_open:
                self._submit_entry('long', 'tii left low zone')
            elif sell_open:
                self._submit_entry('short', 'tii left high zone')
