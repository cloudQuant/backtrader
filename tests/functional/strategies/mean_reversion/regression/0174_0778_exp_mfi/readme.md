...
该策略是对 MT5 EA `0778_Exp_MFI` 的 Backtrader 迁移版本。
当前版本保留了原 EA 的核心结构：
...
2. 在 `H4` 上计算 `MFI(14)`
3. 当 MFI 下穿超卖线时生成买入或反向卖出信号
4. 当 MFI 上穿超买线时生成卖出或反向买入信号
...
- 原 EA 默认在 `H4` MFI 上生成信号，当前版本保留该设计
...
在当前 `XAUUSD_M15` 数据窗口下，`H4 MFI` 交叉信号能够稳定触发并完成回测，验证了该策略在 Backtrader 中的可运行性。
