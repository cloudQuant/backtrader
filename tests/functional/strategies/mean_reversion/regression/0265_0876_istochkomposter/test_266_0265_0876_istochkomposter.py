"""Inlined regression test for tests/functional/strategies/mean_reversion/regression/0265_0876_istochkomposter.
...
_CONFIG = {
    'strategy': {
        'name': 'Exp_iStochKomposter',
        'source_ea': 'ea/0876_Exp_iStochKomposter',
    },
    'data': {
        'symbol': 'XAUUSD',
        'timeframe': 'M15',
...
def test_266_0265_0876_istochkomposter() -> None:
    ...
    assert metrics.get('win_count') == 81, f"win_count: expected=81, got={metrics.get('win_count')!r}"
    assert metrics.get('loss_count') == 107, f"loss_count: expected=107, got={metrics.get('loss_count')!r}"
    assert metrics.get('total_trades') == 188, f"total_trades: expected=188, got={metrics.get('total_trades')!r}"
    assert metrics.get('trade_count') == 188, f"trade_count: expected=188, got={metrics.get('trade_count')!r}"
    assert metrics.get('won') == 81, f"won: expected=81, got={metrics.get('won')!r}"
    assert metrics.get('lost') == 107, f"lost: expected=107, got={metrics.get('lost')!r}"
    _close(metrics.get('bars'), 6129.0, tol=6.129000e-03, key='bars')
    _close(metrics.get('signal_count'), 188.0, tol=1.880000e-04, key='signal_count')
    _close(metrics.get('initial_cash'), 1000000.0, tol=1.000000e+00, key='initial_cash')
    _close(metrics.get('final_value'), 998870.4, tol=9.988704e-01, key='final_value')
    _close(metrics.get('net_pnl'), -1129.5999999999767, tol=1.129600e-03, key='net_pnl')
    _close(metrics.get('total_return_pct'), -0.11295999999999529, tol=1.000000e-06, key='total_return_pct')
    _close(metrics.get('win_rate'), 43.08510638297872, tol=4.308511e-05, key='win_rate')
    _close(metrics.get('profit_factor'), 0.8997408314694494, tol=1.000000e-06, key='profit_factor')
    _close(metrics.get('max_drawdown'), 0.17061171688421242, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('sharpe_ratio'), -3.8007722059865405, tol=3.800772e-06, key='sharpe_ratio')
    _close(metrics.get('annual_return_pct'), -6.465678640331679, tol=6.465679e-06, key='annual_return_pct')
    _close(metrics.get('sqn'), -0.4947375598801714, tol=1.000000e-06, key='sqn')
