"""Inlined regression test for tests/functional/strategies/mean_reversion/regression/0081_0260_exp_trendmanager_tm_plus.
...
_CONFIG = {
    'strategy': {
        'name': 'Exp TrendManager Tm Plus',
        'source_ea': 'ea/0260_Exp_TrendManager_Tm_Plus/Exp_TrendManager_Tm_Plus.mq5',
    },
    'data': {
        'symbol': 'XAUUSD',
        'base_timeframe': 'M15',
...
def test_81_0081_0260_exp_trendmanager_tm_plus() -> None:
    """Migrated regression test (runonce=True only).

    Originally located at tests/functional/strategies_regression/mean_reversion/0081_0260_exp_trendmanager_tm_plus.
    """
    captured = {}
    ...
    assert metrics.get('trade_num') == 9, f"trade_num: expected=9, got={metrics.get('trade_num')!r}"
    assert metrics.get('total_trades') == 9, f"total_trades: expected=9, got={metrics.get('total_trades')!r}"
    _close(metrics.get('final_value'), 1001070.4999999999, tol=1.001070e+00, key='final_value')
    _close(metrics.get('initial_cash'), 1000000.0, tol=1.000000e+00, key='initial_cash')
    _close(metrics.get('max_drawdown'), 0.05961370479275758, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('annual_return'), 0.2732879966353228, tol=1.000000e-06, key='annual_return')
    _close(metrics.get('return_rate'), 0.0010699274234673274, tol=1.000000e-06, key='return_rate')
    _close(metrics.get('sharpe_ratio'), 7.6415724434507615, tol=7.641572e-06, key='sharpe_ratio')
    _close(metrics.get('sqn'), 0.928948581428741, tol=1.000000e-06, key='sqn')
