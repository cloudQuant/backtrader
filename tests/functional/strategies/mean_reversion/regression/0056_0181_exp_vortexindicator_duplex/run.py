...
from strategy_exp_vortexindicator_duplex import ExpVortexIndicatorDuplexStrategy, Mt5PandasFeed, VortexFeed, build_vortex_frame, load_mt5_csv
...
def build_cerebro(config, frame):
    ...
    signal_frame = build_vortex_frame(
        frame['data'],
    ...
    signal_feed = VortexFeed(dataname=signal_frame, timeframe=bt.TimeFrame.Minutes, compression=indicator_minutes)
    ...
    for key in ['indicator_minutes', 'vortex_period']:
        strategy_params.pop(key, None)
    cerebro.addstrategy(ExpVortexIndicatorDuplexStrategy, **strategy_params)
...
def print_report(metrics):
    print('\n' + '=' * 60 + '\nBACKTEST RESULTS — Exp_VortexIndicator_Duplex\n' + '=' * 60)
    for key in [
