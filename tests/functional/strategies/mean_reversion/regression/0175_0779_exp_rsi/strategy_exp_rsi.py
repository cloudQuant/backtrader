...
class ExpRsiStrategy(bt.Strategy):
    params = dict(
        mm=0.1,
        mm_mode='LOT',
        stop_loss=1000,
    ...
    def __init__(self):
        ...
        self.rsi = bt.indicators.RSI(self.h4.close, period=self.p.rsi_period)
    ...
    def _enough_history(self):
        return len(self.h4) >= self.p.rsi_period + max(3, self.p.signal_bar + 2)
    ...
    def _evaluate_signals(self):
        ...
        prev_idx = idx - 1
        rsi_now = float(self.rsi[idx])
        rsi_prev = float(self.rsi[prev_idx])

        buy_open = buy_close = sell_open = sell_close = False

        if self.p.trend == 'DIRECT':
            if rsi_prev > self.p.low_level and rsi_now <= self.p.low_level:
                if self.p.buy_pos_open:
                    buy_open = True
                if self.p.sell_pos_close:
                    sell_close = True
            if rsi_prev < self.p.high_level and rsi_now >= self.p.high_level:
            ...
            if rsi_prev < self.p.high_level and rsi_now >= self.p.high_level:
                ...
                if self.p.sell_pos_close:
                    sell_close = True
        return buy_open, buy_close, sell_open, sell_close, rsi_now, rsi_prev
    ...
    def next(self):
        ...
        buy_open, buy_close, sell_open, sell_close, rsi_now, rsi_prev = self._evaluate_signals()
        self.log(f'rsi signal prev={rsi_prev:.2f} now={rsi_now:.2f} buy_open={buy_open} sell_open={sell_open}')
