...
class MFISlowdown(bt.Indicator):
    ...
    def __init__(self):
        ...
        self.mfi = bt.indicators.MFI(self.data, period=int(self.p.mfi_period))
...
class ExpMFISlowdownStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.ind = MFISlowdown(self.signal_data, mfi_period=self.p.mfi_period, level_max=self.p.level_max, level_min=self.p.level_min, seek_slowdown=self.p.seek_slowdown)
        self.signal_count = 0
        self.trade_count = 0
        self.buy_count = 0
        self.sell_count = 0
    ...
    def next(self):
        ...
        if len(self.signal_data) < max(int(self.p.mfi_period) + 2, 18) + sb: return
