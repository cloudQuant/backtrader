...
class ASCVStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        if self.entry_order is not None or self.close_order is not None:
            return
        buy_signal = float(self.signal.buy_signal[-1]) != 0.0
        sell_signal = float(self.signal.sell_signal[-1]) != 0.0
        if self.p.reverse:
            buy_signal, sell_signal = sell_signal, buy_signal
        if buy_signal:
            if self.position.size < 0:
                self._submit_close('buy signal reverse', reverse='long')
            elif not self.position:
                self._submit_entry('long', 'braintrend buy signal')
        elif sell_signal:
            if self.position.size > 0:
                self._submit_close('sell signal reverse', reverse='short')
            elif not self.position:
