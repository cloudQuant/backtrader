...
class FiftyFiveMAStrategy(bt.Strategy):
    params = dict(
        fixed_lot=0.1,
        point_size=0.01,
        stoploss_pips=30,
    ...
    def __init__(self):
        ...
        self.ma = bt.indicators.ExponentialMovingAverage(self.data0_feed.close, period=self.p.ma_period)
    ...
    def next(self):
        if len(self.data0_feed) < self.p.ma_period + max(self.p.bar_a, self.p.bar_b) + 3:
            return
        if not self._new_bar():
            return
        current_dt = bt.num2date(self.data0_feed.datetime[0])
        if current_dt.hour < self.p.start_hour or current_dt.hour >= self.p.end_hour:
            return
        ...
        if self.entry_order is not None or self.close_order is not None:
            return
        ma_a = float(self.ma[-self.p.bar_a])
        ma_b = float(self.ma[-self.p.bar_b])
        signal = None
        if ma_a > ma_b + self.p.difference:
            signal = 'long' if not self.p.reverse else 'short'
        elif ma_a < ma_b - self.p.difference:
            signal = 'short' if not self.p.reverse else 'long'
        if signal is None:
            return
        if not self.position:
            self._submit_entry(signal, 'ma(bar_a) vs ma(bar_b) diff threshold')
            return
        if self.p.close_opposite:
            if signal == 'long' and self.position.size < 0:
                self._submit_close('close opposite on long signal', reverse='long')
            elif signal == 'short' and self.position.size > 0:
                self._submit_close('close opposite on short signal', reverse='short')
