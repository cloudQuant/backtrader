# 0398 Exp_KWAN_RDP — Backtrader 迁移

## 原始 EA
- 文件：`ea/0398_Exp_KWAN_RDP/Exp_KWAN_RDP.mq5`
- 指标：`ea/0398_Exp_KWAN_RDP/KWAN_RDP.mq5`
- 框架：`TradeAlgorithms.mqh` 单仓开/平模板

## 策略逻辑
1. **复合指标 KWAN_RDP**
   - 计算 `kwan_raw = 100 × DeMarker × MFI / Momentum`
...
| `DeMarker_Period` | `demarker_period` | 14 | DeMarker 周期 |
| `MFIPeriod` | `mfi_period` | 14 | MFI 周期 |
| `VolumeType` | `volume_type` | TICK | MFI 成交量类型 |
