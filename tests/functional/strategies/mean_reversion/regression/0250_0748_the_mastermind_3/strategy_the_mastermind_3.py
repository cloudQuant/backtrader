...
class TheMastermind3Strategy(bt.Strategy):
    params = dict(
        lots=200.0,
        stop_loss=2000,
        take_profit=0,
    ...
    def __init__(self):
        self.wpr_26 = bt.indicators.WilliamsR(self.data, period=26)
        self.wpr_27 = bt.indicators.WilliamsR(self.data, period=27)
        self.wpr_29 = bt.indicators.WilliamsR(self.data, period=29)
        self.wpr_30 = bt.indicators.WilliamsR(self.data, period=30)
    ...
    def _buy_signal(self):
        values = [float(self.wpr_26[0]), float(self.wpr_27[0]), float(self.wpr_29[0]), float(self.wpr_30[0])]
        return all(v < -99.99 for v in values)
    ...
    def next(self):
        ...
        if sell_signal and self.last_trade_side != -1:
            self.signal_count += 1
            self._set_risk('sell')
            self.order = self.sell(size=min(self.p.lots, 5.0))
            self.last_trade_side = -1
            return
        if buy_signal and self.last_trade_side != 1:
            self.signal_count += 1
            self._set_risk('buy')
            self.order = self.buy(size=min(self.p.lots, 5.0))
            self.last_trade_side = 1
