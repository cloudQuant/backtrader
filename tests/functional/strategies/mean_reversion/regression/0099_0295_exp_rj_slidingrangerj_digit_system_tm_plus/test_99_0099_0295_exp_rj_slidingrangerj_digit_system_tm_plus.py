"""Inlined regression test for tests/functional/strategies/mean_reversion/regression/0099_0295_exp_rj_slidingrangerj_digit_system_tm_plus.
...
_CONFIG = {
    'strategy': {
        'name': 'Exp Rj SlidingRangeRj Digit System Tm Plus',
        'source_ea': 'ea/0295_Exp_Rj_SlidingRangeRj_Digit_System_Tm_Plus/exp_rj_slidingrangerj_digit_system_tm_plus.mq5',
    },
    'data': {
        'symbol': 'XAUUSD',
        'base_timeframe': 'M15',
...
def test_99_0099_0295_exp_rj_slidingrangerj_digit_system_tm_plus() -> None:
    """Migrated regression test (runonce=True only).

    Originally located at tests/functional/strategies_regression/mean_reversion/0099_0295_exp_rj_slidingrangerj_digit_system_tm_plus.
    """
    captured = {}
    ...
    assert metrics.get('trade_num') == 34, f"trade_num: expected=34, got={metrics.get('trade_num')!r}"
    assert metrics.get('total_trades') == 34, f"total_trades: expected=34, got={metrics.get('total_trades')!r}"
    _close(metrics.get('final_value'), 1000637.4000000008, tol=1.000637e+00, key='final_value')
    _close(metrics.get('initial_cash'), 1000000.0, tol=1.000000e+00, key='initial_cash')
    _close(metrics.get('max_drawdown'), 0.07782479303537336, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('annual_return'), 0.15527203889029445, tol=1.000000e-06, key='annual_return')
    _close(metrics.get('return_rate'), 0.0006371969469003471, tol=1.000000e-06, key='return_rate')
    _close(metrics.get('sharpe_ratio'), 3.777839490950301, tol=3.777839e-06, key='sharpe_ratio')
    _close(metrics.get('sqn'), 0.5174588001136629, tol=1.000000e-06, key='sqn')
