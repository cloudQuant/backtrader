    ...
    def next(self):
        ...
        if self.position:
            if self.position.size > 0 and buy_close:
                self._submit_close('sell signal seen', reverse='short' if sell_open else None)
            elif self.position.size < 0 and sell_close:
                self._submit_close('buy signal seen', reverse='long' if buy_open else None)
            return
        if buy_open:
            self._submit_entry('long', 'MA crossover buy')
        elif sell_open:
