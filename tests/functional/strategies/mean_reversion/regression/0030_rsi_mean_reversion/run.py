...
from strategy_rsi_mean_reversion import (
    Mt5RSIMeanReversionFeed, RSIMeanReversionStrategy, load_mt5_csv, prepare_rsi_mean_reversion_features,
)
...
def load_data(config):
    ...
    df = prepare_rsi_mean_reversion_features(raw, config.get('params', {}))
...
def build_cerebro(frame, config):
    ...
    feed = Mt5RSIMeanReversionFeed(dataname=frame['data'], timeframe=bt.TimeFrame.Days, compression=1)
    cerebro.adddata(feed, name=f"{config['data']['symbol']}_{config['data']['timeframe']}")
    cerebro.addstrategy(RSIMeanReversionStrategy, **config.get('params', {}))
