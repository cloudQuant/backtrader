...
该策略是对 MT5 EA `0674_ExpHAWaves` 的 Backtrader 迁移版本。
...
- 交易方向由 `Heiken Ashi + StepUpDown` 共振决定
- 持仓按动态计算的 `SL/TP` 或 trailing stop 管理
...
2. 在 `H1` 上按原 EA 公式计算 `Heiken Ashi` 方向。
3. 若 `Heiken=1` 且 `StepUpDown=1`，则做多。
4. 若 `Heiken=-1` 且 `StepUpDown=-1`，则做空。
...
- `Heiken Ashi` 与 `StepUpDown` 的核心判定均已在策略内直接重建，无额外外部指标依赖。
