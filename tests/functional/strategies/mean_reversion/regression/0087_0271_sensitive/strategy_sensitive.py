...
class SensitiveStrategy(bt.Strategy):
    params = dict(
        fixed_lot=0.1,
        point_size=0.01,
        stoploss_pips=35,
    ...
    def next(self):
        ...
        if len(self.data0_feed) < self.p.macd_slow_period + self.p.macd_signal_period + 3:
            return
        ...
        if self.entry_order is not None or self.close_order is not None:
            return
        main0 = float(self.macd.macd[0])
        main1 = float(self.macd.macd[-1])
        signal0 = float(self.macd.signal[0])
        signal1 = float(self.macd.signal[-1])
        level = self.p.macd_open_level * self.p.point_size
        open_buy_signal = main0 < 0 and main0 > signal0 and main1 < signal1 and abs(main0) > level
        open_sell_signal = main0 > 0 and main0 < signal0 and main1 > signal1 and abs(main0) > level
        if open_buy_signal:
            if self.position.size < 0:
                self._submit_close('macd buy signal', reverse='long')
            elif not self.position:
                self._submit_entry('long', 'macd buy signal')
            return
        if open_sell_signal:
            if self.position.size > 0:
                self._submit_close('macd sell signal', reverse='short')
            elif not self.position:
                self._submit_entry('short', 'macd sell signal')
