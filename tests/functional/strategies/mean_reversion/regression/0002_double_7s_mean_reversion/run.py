...
from strategy_double_7s_mean_reversion import (
    Mt5Double7sFeed,
    Double7sMeanReversionStrategy,
    load_mt5_csv,
    prepare_double_7s_features,
)
...
def load_data(config):
    ...
    df = prepare_double_7s_features(raw, config.get('params', {}))
    return {
        'data': df,
        'fromdate': fromdate,
        'todate': todate,
...
def build_cerebro(frame, config):
    ...
    feed = Mt5Double7sFeed(dataname=frame['data'], timeframe=bt.TimeFrame.Days, compression=1)
    cerebro.adddata(feed, name=f"{config['data']['symbol']}_{config['data']['timeframe']}")
    
    cerebro.addstrategy(Double7sMeanReversionStrategy, **config.get('params', {}))
...
def upsert_global_summary(config, metrics):
    ...
    fieldnames = [
        'strategy_id', 'strategy_name', 'fromdate', 'todate', 'bars',
        'total_trades', 'won', 'lost', 'win_rate', 'profit_factor',
        'total_return_pct', 'annual_return_pct', 'sharpe_ratio',
        'max_drawdown', 'sqn', 'ulcer_index',
    ]
    rows = []
    if summary_path.exists():
        with open(summary_path, 'r', encoding='utf-8') as f:
            reader = csv.DictReader(f)
            rows = [row for row in reader if row.get('strategy_id') != config['strategy']['id']]
    row = {
...
def main():
    print(f"Loading config from {BASE_DIR / 'config.yaml'}")
    config = load_config()
    
    print(f"Loading data...")
    frame = load_data(config)
    print(f"Loaded {len(frame['data'])} bars: {frame['data'].index[0]} -> {frame['data'].index[-1]}")
