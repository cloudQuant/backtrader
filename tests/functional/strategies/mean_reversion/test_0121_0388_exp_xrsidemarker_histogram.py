...
``tests/functional/strategies/mean_reversion/regression/0121_0388_exp_xrsidemarker_histogram``.

Auto-generated by ``scripts/inline_regression_tests.py``.

Data Used:
    Uses ``{repo}/tests/datas/XAUUSD_M15.csv`` as base data and ``H4`` as signal
    timeframe.
    Period is ``2025-12-03 01:15:00`` to ``2026-03-10 09:00:00``, with
    a 15-minute timestamp shift.

Strategy Principle:
    Combine RSI and DeMarker histograms, apply optional XMA smoothing, and generate
    long/short transitions on direction changes.

Strategy Logic:
    Load execution/signal dataframes, run dual-timeframe backtest, and validate
...
class ExpXrsiDeMarkerHistogramStrategy(bt.Strategy):
    ...
    def next(self):
        """Evaluate histogram direction and submit close/open orders."""
    ...
    def notify_order(self, order):
        """Update open counters and clear temporary order state."""
    ...
    def notify_trade(self, trade):
        """Track win/loss counters for each closed trade."""
    ...
    def stop(self):
        ...
        print('========== Exp_XRSIDeMarker_Histogram Strategy Finished ==========')
...
class Mt5PandasFeed(bt.feeds.PandasData):
    """Pandas data feed with spread for execution and signal series."""
    lines = ('spread',)
    params = (
        ('datetime', None),
        ('open', 0),
...
def resample_frame(df, minutes):
    """Resample base DataFrame to the configured signal interval."""
...
def load_backtest_frame(config: dict) -> dict:
    """Build execution and signal frames from file and date range."""
...
def add_default_analyzers(cerebro):
    """Attach analyzers required by regression assertions."""
...
def finite_or_none(value):
    """Normalize NaN/infinite values to ``None``."""
    if value is None:
        return None
...
def extract_metrics(strat, start_value, frame):
    """Assemble normalized summary metrics for assertions."""
