"""Inlined regression test for tests/functional/strategies/mean_reversion/regression/0073_0245_exp_iin_ma_signal.
...
_CONFIG = {
    'strategy': {
        'name': 'Exp Iin MA Signal',
        'source_ea': 'ea/0245_Exp_Iin_MA_Signal/exp_iin_ma_signal.mq5',
    },
    'data': {
        'symbol': 'XAUUSD',
        'base_timeframe': 'M15',
...
def test_73_0073_0245_exp_iin_ma_signal() -> None:
    ...
    _close(metrics.get('final_value'), 996552.2999999999, tol=9.965523e-01, key='final_value')
    _close(metrics.get('initial_cash'), 1000000.0, tol=1.000000e+00, key='initial_cash')
    _close(metrics.get('max_drawdown'), 0.36562857404855426, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('annual_return'), -0.5419831278327375, tol=1.000000e-06, key='annual_return')
    _close(metrics.get('return_rate'), -0.0034536570135832822, tol=1.000000e-06, key='return_rate')
    _close(metrics.get('sharpe_ratio'), -18.125036537568665, tol=1.812504e-05, key='sharpe_ratio')
    _close(metrics.get('sqn'), -2.574898230377208, tol=2.574898e-06, key='sqn')
