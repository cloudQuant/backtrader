"""Inlined regression test for tests/functional/strategies/mean_reversion/regression/0032_online_mean_reversion.
...
_CONFIG = {
    'strategy': {
        'id': '0315',
        'name': 'Online Mean Reversion',
        'source_spec': 'research_papers_gold/strategy_specs/mean_reversion/0315_Online_Mean_Reversion_Strategy.md',
    },
    'data': {
        'symbol': 'XAUUSD',
        'timeframe': 'D1',
...
def test_32_0032_online_mean_reversion() -> None:
    """Migrated regression test (runonce=True only).

    Originally located at tests/functional/strategies_regression/mean_reversion/0032_online_mean_reversion.
    """
    config = load_config()
    ...
    assert metrics.get('buy_count') == 309, f"buy_count: expected=309, got={metrics.get('buy_count')!r}"
    assert metrics.get('sell_count') == 309, f"sell_count: expected=309, got={metrics.get('sell_count')!r}"
    assert metrics.get('win_count') == 184, f"win_count: expected=184, got={metrics.get('win_count')!r}"
    assert metrics.get('loss_count') == 125, f"loss_count: expected=125, got={metrics.get('loss_count')!r}"
    ...
    assert metrics.get('won') == 184, f"won: expected=184, got={metrics.get('won')!r}"
    assert metrics.get('lost') == 125, f"lost: expected=125, got={metrics.get('lost')!r}"
    _close(metrics.get('bars'), 4619.0, tol=4.619000e-03, key='bars')
    _close(metrics.get('initial_cash'), 1000000.0, tol=1.000000e+00, key='initial_cash')
    _close(metrics.get('final_value'), 966846.6968720002, tol=9.668467e-01, key='final_value')
    _close(metrics.get('net_pnl'), -33153.30312799977, tol=3.315330e-02, key='net_pnl')
    _close(metrics.get('total_return_pct'), -3.31533031279998, tol=3.315330e-06, key='total_return_pct')
    _close(metrics.get('win_rate'), 59.54692556634305, tol=5.954693e-05, key='win_rate')
    _close(metrics.get('profit_factor'), 0.9829469856716524, tol=1.000000e-06, key='profit_factor')
    _close(metrics.get('max_drawdown'), 45.256384819105854, tol=4.525638e-05, key='max_drawdown')
    _close(metrics.get('sharpe_ratio'), 0.02839874132087142, tol=1.000000e-06, key='sharpe_ratio')
    _close(metrics.get('annual_return_pct'), -0.18377255004445808, tol=1.000000e-06, key='annual_return_pct')
    _close(metrics.get('sqn'), -0.10314249786517952, tol=1.000000e-06, key='sqn')
    _close(metrics.get('ulcer_index'), 30.478383935092378, tol=3.047838e-05, key='ulcer_index')
