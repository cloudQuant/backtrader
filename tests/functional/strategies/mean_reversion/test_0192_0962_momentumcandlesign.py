"""Inlined regression test for `tests/functional/strategies/mean_reversion/regression/0192_0962_momentumcandlesign`.
...
Data Used:
    ...
    covering 2025-12-03 to 2026-03-10 with timestamps shifted by 15 minutes.
    The base M15 feed is the execution timeframe, while a resampled H12 feed
    supplies the MomentumCandleSign signal.

Strategy Principle:
    The strategy compares momentum of open and close prices. A crossover where
    open-momentum drops below close-momentum prints a buy dot below the bar
    (offset by a fraction of ATR), and the opposite crossover prints a sell
    dot above the bar. These dots act as directional entry signals, with fixed
    point-based stop-loss and take-profit levels managing risk.

Strategy Logic:
    The MomentumCandleSign indicator emits buy/sell signal levels on the H12
    feed. Each time the signal feed advances, the strategy checks protective
    stop/take-profit exits, then reads the current signal dots: opposite future
    dots close (and optionally reverse) existing positions, and a present dot
    opens a new long/short position with rounded fixed lot sizing. Order and
    trade notifications maintain entry counts, risk levels, and reverse-entry
    state for the regression metrics.
"""
...
_CONFIG = {
    'strategy': {
        'name': 'Exp_MomentumCandleSign',
        'source_ea': 'ea/0962_Exp_MomentumCandleSign',
    },
    'data': {
        'symbol': 'XAUUSD',
        'timeframe': 'M15',
...
class MomentumCandleSignStrategy(bt.Strategy):
    """Trade MomentumCandleSign buy/sell dots with ATR-free fixed risk levels."""
    ...
    def __init__(self):
        """Bind feeds, build the MomentumCandleSign indicator, and reset state."""
    ...
    def next(self):
        """Evaluate risk exits and MomentumCandleSign open/close/reverse signals."""
        self.bar_num += 1
...
def build_cerebro(config, frame):
    """Construct a configured cerebro with execution/resampled feeds and analyzers."""
...
def test_193_0192_0962_momentumcandlesign() -> None:
    """Migrated regression test (runonce=True only).

    Originally located at tests/functional/strategies_regression/mean_reversion/0192_0962_momentumcandlesign.
    """
    config = load_config()
    ...
    assert metrics.get('bar_num') == 5303, f"bar_num: expected=5303, got={metrics.get('bar_num')!r}"
    ...
    _close(metrics.get('buy_signal_count'), 17.0, tol=1.700000e-05, key='buy_signal_count')
    _close(metrics.get('sell_signal_count'), 22.0, tol=2.200000e-05, key='sell_signal_count')
    _close(metrics.get('completed_order_count'), 77.0, tol=7.700000e-05, key='completed_order_count')
    _close(metrics.get('rejected_order_count'), 0.0, tol=1.000000e-06, key='rejected_order_count')
    _close(metrics.get('initial_cash'), 1000000.0, tol=1.000000e+00, key='initial_cash')
    _close(metrics.get('final_value'), 1001121.0999999993, tol=1.001121e+00, key='final_value')
    _close(metrics.get('net_pnl'), 1121.0999999992782, tol=1.121100e-03, key='net_pnl')
    _close(metrics.get('total_return_pct'), 0.1120472036690276, tol=1.000000e-06, key='total_return_pct')
    _close(metrics.get('win_rate'), 50.0, tol=5.000000e-05, key='win_rate')
    _close(metrics.get('sharpe_ratio'), 1.846713601511068, tol=1.846714e-06, key='sharpe_ratio')
    _close(metrics.get('annual_return_pct'), 6.85782429597391, tol=6.857824e-06, key='annual_return_pct')
    _close(metrics.get('max_drawdown'), 0.37353856036166105, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('sqn'), 0.5549667873082732, tol=1.000000e-06, key='sqn')
