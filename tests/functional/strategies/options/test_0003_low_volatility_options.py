"""Inlined regression test for the Low Volatility Options multi-asset strategy.
...
Data Used:
    Three daily ``D1`` ETF series loaded from ``tests/datas/mt5_1d_data``:
    JEPI (low-volatility equity income), PBP (buy-write/covered call) and IVV
    (S&P 500 equity proxy). The window runs from 2008-01-01 to 2025-12-31. A
    synthetic put-write NAV series is simulated from the IVV proxy, and the
    three tradable series (low_vol, covered_call, put_write) are aligned on a
    common date index and added as separate PandasData feeds.

Strategy Principle:
    Builds a low-volatility options income portfolio that blends a low-vol
    equity sleeve, a covered-call sleeve and a synthetic cash-secured put-write
    sleeve at fixed target weights. The thesis is that option-overwriting and
    low-volatility tilts harvest premium and dampen drawdowns relative to plain
    equity. Volatility targeting and a drawdown-triggered risk scale reduce
    exposure when realized drawdown breaches a threshold.

Strategy Logic:
    ``prepare_low_vol_options_inputs`` aligns the three series and synthesises
    the put-write NAV via ``_simulate_put_write_series``.
    ``LowVolatilityOptionsStrategy.__init__`` normalises target weights and
    captures the peak value. ``next`` rebalances on the first bar and every
    ``rebalance_interval_days`` (when weights drift past the threshold) using
    ``order_target_percent`` against risk-scaled weights. ``notify_order``
    prunes settled orders. ``extract_metrics`` aggregates analyzer output and
    the ulcer index that the test asserts against the captured baseline.
"""
...
def prepare_low_vol_options_inputs(asset_map, params):
    """Align the source series and synthesise the put-write NAV series.

    Args:
        asset_map: Mapping of role name to loaded OHLCV DataFrame, including
            ``low_vol``, ``covered_call`` and ``equity_proxy`` entries.
        params: Strategy parameter mapping passed to the put-write simulation.

    Returns:
        A tuple ``(prepared_map, close_df, aligned_index)`` where
        ``prepared_map`` holds the three tradable frames aligned on a common
        index, ``close_df`` is their combined close prices, and
        ``aligned_index`` is the shared datetime index.

    Raises:
        ValueError: If the source series have no overlapping dates.
    """
    fromdate_index = None
    prepared_map = {}
...
class LowVolatilityOptionsStrategy(bt.Strategy):
    """Periodically rebalanced low-volatility options income portfolio.

    Holds three sleeves (low-vol equity, covered call and synthetic put-write)
    at fixed target weights, rebalancing on a fixed interval when weights drift
    past a threshold and scaling exposure down when drawdown breaches the
    configured maximum.
    """
    ...
    def __init__(self):
        """Normalise target weights and initialise counters and peak value."""
        self.bar_num = 0
        self.buy_count = 0
        self.sell_count = 0
    ...
    def next(self):
        """Rebalance toward risk-scaled target weights on the schedule."""
        self.bar_num += 1
    ...
    def notify_order(self, order):
        """Track pending order lifecycle and keep the local pending queue clean."""
...
def finite_or_none(value):
    """Convert finite numbers to float and return None for invalid entries."""
    return value if value is not None and math.isfinite(value) else None


def calculate_ulcer_index(values):
    """Calculate the ulcer index from a portfolio value sequence.
...
def load_inputs(config):
    """Load and align all strategy input series.

    Args:
        config: Inline strategy configuration dictionary.

    Returns:
        A dictionary containing prepared symbol frames, combined close matrix,
        aligned datetime index, and test date bounds.
    """
...
def build_cerebro(inputs, config):
    """Create and configure Backtrader execution engine for the test.

    Args:
        inputs: Data payload returned by :func:`load_inputs`.
        config: Inline strategy configuration dictionary.
...
def extract_metrics(strat, cerebro, inputs, config):
    """Collect strategy and portfolio metrics for regression assertion checks.
    ...
    Returns:
        A mapping containing returns, drawdown, sharpe, ulcer index and trade
        counters expected by this test.
    """
...
def normalize(value):
    """Normalize JSON-unsafe values before serialization.

    Args:
        value: Value potentially containing datetime or non-finite numbers.

    Returns:
        ISO-formatted datetime for datetime inputs, ``None`` for NaN/inf, or
        the original value for supported JSON-safe types.
    """
