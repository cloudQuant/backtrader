#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Dividend Aristocrats Strategy 回测"""
...
from strategy_0249 import (
    Mt5DividendAristocratsFeed,
    DividendAristocratsStrategy,
    load_mt5_csv,
    prepare_dividend_aristocrats_features,
)
...
def load_data(config):
    ...
    df = prepare_dividend_aristocrats_features(raw, config.get("params", {}))
...
def write_local_result(config, metrics):
    ...
    with open(path, "w", encoding="utf-8") as f:
        json.dump(
            {k: normalize(v) for k, v in metrics.items()},
            f,
            ensure_ascii=False,
            indent=2,
        )
