#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""IndicatorPeriodOptimization 策略回测"""
...
from strategy_indicator_period_optimization import (
    Mt5IndicatorPeriodOptimizationFeed, IndicatorPeriodOptimizationStrategy, load_mt5_csv, prepare_indicator_period_optimization_features,
)
...
def load_data(config):
    ...
    df = prepare_indicator_period_optimization_features(raw, config.get('params', {}))
    if df.empty:
...
def build_cerebro(frame, config):
    ...
    feed = Mt5IndicatorPeriodOptimizationFeed(dataname=frame['data'], timeframe=bt.TimeFrame.Days, compression=1)
    cerebro.adddata(feed, name=f"{config['data']['symbol']}_{config['data']['timeframe']}")
    cerebro.addstrategy(IndicatorPeriodOptimizationStrategy, **config.get('params', {}))
