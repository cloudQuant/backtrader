...
from strategy_correlation_break_hold import CorrelationBreakHoldStrategy, CorrelationSignalFeed, load_mt5_csv, prepare_correlation_features
...
def upsert_global_summary(config, metrics):
    ...
    rows.append({
        ...
        'symbol': '{0}/{1}'.format(config['data']['signal_symbol'], config['data']['hedge_symbol']),
...
def print_results(metrics):
    ...
    print(f"  Hold regime days:    {metrics['hold_regime_days']}")
    print(f"  Neutral regime days: {metrics['neutral_regime_days']}")
    print(f"  Break regime days:   {metrics['break_regime_days']}")
