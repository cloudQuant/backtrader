...
from strategy_gap_down import (
    Mt5GapDownFeed, GapDownStrategy, load_mt5_csv, prepare_gap_down_features,
)
...
def load_data(config):
    ...
    df = prepare_gap_down_features(raw, config.get('params', {}))
...
def build_cerebro(frame, config):
    ...
    feed = Mt5GapDownFeed(dataname=frame['data'], timeframe=bt.TimeFrame.Days, compression=1)
    cerebro.adddata(feed, name=f"{config['data']['symbol']}_{config['data']['timeframe']}")
    cerebro.addstrategy(GapDownStrategy, **config.get('params', {}))
