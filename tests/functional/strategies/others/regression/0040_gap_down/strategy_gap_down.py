...
def prepare_gap_down_features(df, params):
    gap_threshold = float(params.get('gap_threshold', -0.01))

    out = df.copy()
    out['gap_pct'] = (out['open'] - out['close'].shift(1)) / out['close'].shift(1)
    out['entry_signal'] = (out['gap_pct'] < gap_threshold).astype(float)

    out = out[['open', 'high', 'low', 'close', 'volume', 'openinterest',
               'gap_pct', 'entry_signal']].copy()
    return out.dropna()


class Mt5GapDownFeed(bt.feeds.PandasData):
    lines = ('gap_pct', 'entry_signal',)
    params = (
