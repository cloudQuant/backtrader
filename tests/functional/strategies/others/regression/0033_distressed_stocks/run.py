...
from strategy_distressed_stocks import (
    Mt5DistressedStocksFeed, DistressedStocksStrategy, load_mt5_csv, prepare_distressed_stocks_features,
)
...
def load_data(config):
    ...
    df = prepare_distressed_stocks_features(raw, config.get('params', {}))
    if df.empty:
...
def build_cerebro(frame, config):
    ...
    feed = Mt5DistressedStocksFeed(dataname=frame['data'], timeframe=bt.TimeFrame.Days, compression=1)
    cerebro.adddata(feed, name=f"{config['data']['symbol']}_{config['data']['timeframe']}")
    cerebro.addstrategy(DistressedStocksStrategy, **config.get('params', {}))
