...
from strategy_overnight_intraday import (
    Mt5OvernightIntradayFeed, OvernightIntradayStrategy, load_mt5_csv, prepare_overnight_intraday_features,
)
...
def load_data(config):
    ...
    df = prepare_overnight_intraday_features(raw, config.get('params', {}))
...
def build_cerebro(frame, config):
    ...
    feed = Mt5OvernightIntradayFeed(dataname=frame['data'], timeframe=bt.TimeFrame.Days, compression=1)
    cerebro.adddata(feed, name=f"{config['data']['symbol']}_{config['data']['timeframe']}")
    cerebro.addstrategy(OvernightIntradayStrategy, **config.get('params', {}))
