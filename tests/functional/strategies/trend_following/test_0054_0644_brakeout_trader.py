"""Inlined regression test for tests/functional/strategies/trend_following/regression/0054_0644_brakeout_trader.
...
_CONFIG = {
    'strategy': {
        'name': 'BrakeoutTrader',
        'source_ea': 'ea/0644_Brakeout_Trader_v1/brakeout_trader_v1.mq5',
    },
    'data': {
        'symbol': 'XAUUSD',
        'timeframe': 'H1',
...
def test_54_0054_0644_brakeout_trader() -> None:
    """Migrated regression test (runonce=True only).

    Originally located at tests/functional/strategies_regression/trend_following/0054_0644_brakeout_trader.
    """
    ...
    _close(metrics.get('completed_orders'), 38.0, tol=3.800000e-05, key='completed_orders')
    _close(metrics.get('rejected_orders'), 0.0, tol=1.000000e-06, key='rejected_orders')
    _close(metrics.get('initial_cash'), 1000000.0, tol=1.000000e+00, key='initial_cash')
    _close(metrics.get('final_value'), 999514.3999999999, tol=9.995144e-01, key='final_value')
    _close(metrics.get('net_pnl'), -485.60000000009313, tol=4.856000e-04, key='net_pnl')
    _close(metrics.get('total_return_pct'), -0.048560000000008596, tol=1.000000e-06, key='total_return_pct')
    _close(metrics.get('win_rate'), 42.10526315789473, tol=4.210526e-05, key='win_rate')
    _close(metrics.get('profit_factor'), 0.48857293312270517, tol=1.000000e-06, key='profit_factor')
    _close(metrics.get('sharpe_ratio'), -6.15344049624721, tol=6.153440e-06, key='sharpe_ratio')
    _close(metrics.get('annual_return_pct'), -10.424829709161191, tol=1.042483e-05, key='annual_return_pct')
    _close(metrics.get('max_drawdown'), 0.07567701832549427, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('sqn'), -0.8138137913845269, tol=1.000000e-06, key='sqn')
