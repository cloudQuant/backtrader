"""Inlined regression test for `DVD Level` trend-following strategy.
...
Data Used:
    ...
    - Symbol: ``XAUUSD``
    - Base and resampled timeframes: ``M15`` base, plus ``M30``, ``H1``, ``D1``
    - Date range: ``2025-12-03 01:15:00`` to ``2026-03-10 09:00:00``
    - Bar shift: ``+15`` minutes

Strategy Principle:
    - Builds EMA(2)/EMA(24) across H1 and D1, computes a score-based long/short
      regime and optionally places limit entries.
    - Long/short entries use configurable lot sizing, initial stop-loss and
      take-profit, and optional trailing adjustments.
    - Flat-state and active-position handling rely on score thresholds and risk
      guardrails.

Strategy Logic:
    - ``load_backtest_frames()`` loads base M15 data and derives M30/H1/D1 frames.
    - ``build_cerebro()`` wires all feeds and analyzers.
    - ``DvdLevelStrategy.next()`` checks market time, score, risk exits, and places
      orders accordingly.
    - ``extract_metrics()`` consolidates trade statistics and analyzer summaries.
"""
...
def resample_frame(df, rule):
    """Resample base frame to a coarser timeframe using OHLCV aggregation."""
...
class Mt5PandasFeed(bt.feeds.PandasData):
    """Backtrader feed mapping normalized MT5 OHLCV columns."""
    params = (
...
class DvdLevelStrategy(bt.Strategy):
    """Multi-timeframe EMA-score strategy with configurable money and risk controls."""
    params = dict(
        account_is_mini=True,
    ...
    def __init__(self):
        """Initialize multi-timeframe indicators, state fields, and counters."""
    ...
    def log(self, text):
        """Emit timestamped diagnostic messages."""
    ...
    def next(self):
        """Advance one bar and process scoring, entries, exits, and trailing updates."""
        self.bar_num += 1
    ...
    def notify_order(self, order):
        """Process completed/cancelled orders and update pending setup state."""
    ...
    def notify_trade(self, trade):
        """Track closed trades and outcome counters."""
...
def resolve_data_path(filename):
    """Resolve and validate input file location."""
...
def load_backtest_frames(config):
    """Load and resample multi-timeframe payload for M15/M30/H1/D1."""
...
def build_cerebro(config, payload):
    """Build backtrader engine with four feeds, strategy, and analyzers."""
...
def extract_metrics(strat, cerebro, payload, config):
    """Extract trade metrics and analyzer outputs for assertions."""
...
def run(plot=False):
    """Run backtest and return results tuple with metrics and cerebro."""
