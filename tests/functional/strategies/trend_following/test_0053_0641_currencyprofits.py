"""Inlined regression test for tests/functional/strategies/trend_following/regression/0053_0641_currencyprofits.
...
_CONFIG = {
    'strategy': {
        'name': 'Currencyprofits',
        'source_ea': 'ea/0641_Currencyprofits_01.1/currencyprofits_01.1.mq5',
    },
    'data': {
        'symbol': 'XAUUSD',
        'timeframe': 'H1',
...
def load_mt5_csv(filepath, fromdate=None, todate=None, bar_shift_minutes=0):
    """Load MT5 TSV data into a standardized DataFrame.
...
class Mt5PandasFeed(bt.feeds.PandasData):
    """PandasData feed mapping MT5 columns to OHLCV indexes."""
...
class CurrencyProfitsStrategy(bt.Strategy):
    ...
    def __init__(self):
        """Create MA/breakout indicators and initialize position counters."""
    ...
    def next(self):
        """Advance strategy state and open/close positions based on MA/6-bar rules."""
        self.bar_num += 1
    ...
    def notify_order(self, order):
        """Track order completions/rejections and clear active order.
    ...
    def notify_trade(self, trade):
        """Accumulate trade win/loss statistics after each closed trade.
...
def load_backtest_frame(config):
    """Load backtest data and return run metadata.
...
def build_cerebro(config, frame):
    """Configure and return Cerebro with data, strategy, and analyzers.
...
def extract_metrics(strat, cerebro, frame, config):
    """Collect final metrics for regression assertions.

    Args:
        strat: Strategy instance after run.
        cerebro: Completed Cerebro object.
        frame: Frame dict with date range and bars.
...
def test_53_0053_0641_currencyprofits() -> None:
    """Migrated regression test (runonce=True only).

    Originally located at tests/functional/strategies_regression/trend_following/0053_0641_currencyprofits.
    """
    ...
    assert metrics.get('sell_count') == 183, f"sell_count: expected=183, got={metrics.get('sell_count')!r}"
    assert metrics.get('total_trades') == 382, f"total_trades: expected=382, got={metrics.get('total_trades')!r}"
    assert metrics.get('trade_count') == 382, f"trade_count: expected=382, got={metrics.get('trade_count')!r}"
    assert metrics.get('won') == 179, f"won: expected=179, got={metrics.get('won')!r}"
    assert metrics.get('lost') == 203, f"lost: expected=203, got={metrics.get('lost')!r}"
    _close(metrics.get('bars'), 6129.0, tol=6.129000e-03, key='bars')
    _close(metrics.get('signal_count'), 382.0, tol=3.820000e-04, key='signal_count')
    _close(metrics.get('completed_orders'), 764.0, tol=7.640000e-04, key='completed_orders')
    _close(metrics.get('rejected_orders'), 0.0, tol=1.000000e-06, key='rejected_orders')
    _close(metrics.get('initial_cash'), 1000000.0, tol=1.000000e+00, key='initial_cash')
    _close(metrics.get('final_value'), 1002096.3999999991, tol=1.002096e+00, key='final_value')
    _close(metrics.get('net_pnl'), 2096.399999999092, tol=2.096400e-03, key='net_pnl')
    _close(metrics.get('total_return_pct'), 0.2096399999998999, tol=1.000000e-06, key='total_return_pct')
    _close(metrics.get('win_rate'), 46.8586387434555, tol=4.685864e-05, key='win_rate')
    _close(metrics.get('profit_factor'), 1.158233198478357, tol=1.158233e-06, key='profit_factor')
    _close(metrics.get('sharpe_ratio'), 5.946687259208467, tol=5.946687e-06, key='sharpe_ratio')
    _close(metrics.get('annual_return_pct'), 60.74823002738378, tol=6.074823e-05, key='annual_return_pct')
    _close(metrics.get('max_drawdown'), 0.2619171244598922, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('sqn'), 0.7457342522798194, tol=1.000000e-06, key='sqn')
