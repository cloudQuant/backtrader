"""Inlined regression test for tests/functional/strategies/trend_following/regression/0197_0479_arrows_and_curves_ea.
...
def test_196_0197_0479_arrows_and_curves_ea() -> None:
    """Migrated regression test (runonce=True only).

    Originally located at tests/functional/strategies_regression/trend_following/0197_0479_arrows_and_curves_ea.
    """
    ...
    assert metrics.get('sell_count') == 104, f"sell_count: expected=104, got={metrics.get('sell_count')!r}"
    ...
    assert metrics.get('trade_count') == 214, f"trade_count: expected=214, got={metrics.get('trade_count')!r}"
    ...
    _close(metrics.get('final_value'), 999973.2000000002, tol=9.999732e-01, key='final_value')
    _close(metrics.get('net_pnl'), -26.799999999813735, tol=2.680000e-05, key='net_pnl')
    _close(metrics.get('total_return_pct'), -0.002679999999977145, tol=1.000000e-06, key='total_return_pct')
    _close(metrics.get('win_rate'), 46.728971962616825, tol=4.672897e-05, key='win_rate')
    _close(metrics.get('profit_factor'), 0.996939206706339, tol=1.000000e-06, key='profit_factor')
    _close(metrics.get('max_drawdown'), 0.296906340633091, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('sharpe_ratio'), -0.0927292567985257, tol=1.000000e-06, key='sharpe_ratio')
    _close(metrics.get('annual_return_pct'), -0.15855119073153867, tol=1.000000e-06, key='annual_return_pct')
    _close(metrics.get('sqn'), -0.013070078498058956, tol=1.000000e-06, key='sqn')
