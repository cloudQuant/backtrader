"""Inlined regression test for the Exp_LeManSignal trend-following strategy.
...
Strategy Principle:
    This is a port of the MT5 expert advisor ``Exp_LeManSignal``. On the higher
    H4 timeframe the LeManSignal indicator compares two consecutive LPeriod-wide
    high/low ranges (shifted by one bar and by LPeriod bars) to detect range
    breakouts. An upward expansion of the high range prints a buy arrow and a
    downward expansion of the low range prints a sell arrow. Arrows act as
    trend-breakout entries, with fixed stop-loss and take-profit distances in
    points managing exits.

Strategy Logic:
    load_backtest_frame loads the M15 base frame and build_signal_frame derives
    the H4 signal frame on which the indicator is computed. On each base bar the
    strategy checks protective stop/take-profit exits, then acts once per new H4
    bar: it reads the signal-bar arrows, optionally scans recent history for an
    opposing arrow to close, and opens or reverses positions per the permission
    flags. notify_trade counts entries and tallies win/loss on close. The test
    forces runonce=True, captures extract_metrics() output, and asserts each
...
_CONFIG = {
    'strategy': {
        'name': 'Exp_LeManSignal',
        'source_ea': 'ea/0915_Exp_LeManSignal',
    },
    'data': {
        'symbol': 'XAUUSD',
        'timeframe': 'M15',
...
class ExpLeManSignalStrategy(bt.Strategy):
    """Signal-on-arrow trend strategy with fixed SL/TP and optional close permissions."""
    params = dict(
        lperiod=12,
        signal_bar=1,
        stop_loss_points=1000,
    ...
    def log(self, text):
        """Print a timestamped line for strategy event logging."""
    ...
    def _scan_history_for_close(self):
        """Scan recent signal history and close opposite position when conditions persist."""
        if not self.position:
            return
    ...
    def next(self):
        """Read latest signal bar, handle protective exits, and run open/close logic."""
        self.bar_num += 1
        if len(self.base) < 2:
            return
    ...
    def notify_trade(self, trade):
        """Update open/close trade counters and win/loss statistics."""
...
def resolve_data_path(fn):
    """Resolve and validate a relative path from the module directory.

    Args:
        fn: File path relative to this test module.
...
def load_backtest_frame(cfg):
    """Load raw data and package metadata for backtest execution.

    Args:
        cfg: Inline configuration dict.

    Returns:
        Dict with loaded data and test window bounds.
    """
    dc = cfg['data']
...
def build_signal_frame(df, mins):
    """Resample frame into higher timeframe signal data.

    Args:
        df: Base OHLCV frame.
        mins: Minutes per resample interval.

    Returns:
        OHLCV resampled frame.
    """
    r = f'{int(mins)}min'
...
def build_cerebro(cfg, frame):
    """Build the Backtrader engine with strategy, data feeds, and analyzers.

    Args:
        cfg: Inline config dictionary.
...
def extract_metrics(strat, cerebro, frame, cfg):
    """Collect counters and analyzer values used by the regression assertions.

    Args:
        strat: Strategy object after run.
        cerebro: Executed engine.
        frame: Data/metadata used for this run.
        cfg: Backtest configuration.

    Returns:
        Dictionary of metrics in deterministic form.
    """
...
def run(plot=False):
    """Execute the strategy and return run outputs.

    Args:
        plot: Whether to render chart after running.
...
def test_89_0089_0915_lemansignal() -> None:
    """Migrated regression test (runonce=True only).

    Originally located at tests/functional/strategies_regression/trend_following/0089_0915_lemansignal.
    """
    ...
    _close(metrics.get('final_value'), 998945.3999999999, tol=9.989454e-01, key='final_value')
    _close(metrics.get('net_pnl'), -1054.6000000000931, tol=1.054600e-03, key='net_pnl')
    _close(metrics.get('total_return_pct'), -0.10546000000001277, tol=1.000000e-06, key='total_return_pct')
    _close(metrics.get('win_rate'), 42.10526315789473, tol=4.210526e-05, key='win_rate')
    _close(metrics.get('profit_factor'), 0.31017791732076877, tol=1.000000e-06, key='profit_factor')
    _close(metrics.get('max_drawdown'), 0.12708116657112578, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('sharpe_ratio'), -12.912972929339498, tol=1.291297e-05, key='sharpe_ratio')
    _close(metrics.get('annual_return_pct'), -6.050389217852546, tol=6.050389e-06, key='annual_return_pct')
    _close(metrics.get('sqn'), -1.8186533921927868, tol=1.818653e-06, key='sqn')
