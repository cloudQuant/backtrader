"""Meeting Lines and Stochastic confirmation strategy functional test.
...
Strategy Principle:
    - **Market Hypothesis**: Reversals in trend direction can be detected early by strong matching-close dual candlestick patterns ("Meeting Lines") when verified by overbought/oversold momentum indicators (Stochastic Oscillator).
    - **Meeting Lines Pattern**:
        - *Bullish (Counter-attack)*: A long bearish candle followed by a long bullish candle where their closes are nearly equal, suggesting support and reversal potential.
        - *Bearish (Counter-attack)*: A long bullish candle followed by a long bearish candle where their closes are nearly equal, suggesting resistance and reversal potential.
    - **Stochastic Confirmation**: Uses Slow Stochastic %D value.
        - Long entry triggers when Bullish Meeting Lines occur and %D < 30.
        - Short entry triggers when Bearish Meeting Lines occur and %D > 70.

Strategy Logic:
    1. **Initialization**: Configures the Stochastic indicator (using `stoch_k`, `stoch_d`, `stoch_slow`) and a Simple Moving Average (SMA) of absolute candle body sizes over `ma_period` to represent average volatility.
    2. **Pattern Checking**:
        - Measures candle body sizes against average volatility (`body_multiplier * avg`).
        - Verifies close proximity of both candles (`close_tolerance_multiplier * avg`).
    3. **Execution**:
        - *Entry*: Places a buy order of `lot` size if bullish pattern and Stochastic %D is oversold (< 30). Places a sell order of `lot` size if bearish pattern and Stochastic %D is overbought (> 70).
        - *Exit*: Closes long position if Stochastic %D crosses above `stoch_exit_lower` (20) or crosses above `stoch_exit_upper` (80). Closes short position if Stochastic %D crosses below `stoch_exit_upper` (80) or crosses below `stoch_exit_lower` (20).
    4. **Reporting**: Collects performance metrics such as final portfolio value, net P&L, Sharpe ratio, win rate, drawdown, and total executed trades.
"""
...
_CONFIG = {
    'strategy': {
        'name': 'Meeting Lines + Stochastic',
        'source_ea': 'ea/1322_MQL5_Wizard_-_Trading_Signals_Based_on_Bullish_Bearish_Meeting_Lines_Pattern_+_Stochastic',
    },
    'data': {
        'symbol': 'XAUUSD',
        'timeframe': 'M15',
...
class MeetingLinesStochStrategy(bt.Strategy):
    ...
    def __init__(self):
        """Initializes Slow Stochastic, volatility SMA indicator, and portfolio metrics."""
    ...
    def next(self):
        ...
        Evaluates active positions for exit criteria using Stochastic %D levels,
        and scans for new entries based on Meeting Lines candlestick patterns and Stochastic thresholds.
        """
        self.bar_num += 1
...
def build_cerebro(config, frame):
    ...
    cerebro.addstrategy(MeetingLinesStochStrategy, **config.get('params', {}))
...
def test_160_0160_1322_meetinglines_stoch() -> None:
    """Migrated regression test (runonce=True only).

    Originally located at tests/functional/strategies_regression/trend_following/0160_1322_meetinglines_stoch.
    """
    ...
    assert metrics.get('bar_num') == 6087, f"bar_num: expected=6087, got={metrics.get('bar_num')!r}"
    ...
    _close(metrics.get('final_value'), 991077.6999999954, tol=9.910777e-01, key='final_value')
    _close(metrics.get('net_pnl'), -8922.300000004587, tol=8.922300e-03, key='net_pnl')
    _close(metrics.get('total_return_pct'), -0.8922300000004602, tol=1.000000e-06, key='total_return_pct')
    ...
    _close(metrics.get('max_drawdown'), 1.3197442100713652, tol=1.319744e-06, key='max_drawdown')
    _close(metrics.get('sharpe_ratio'), -7.762849802041505, tol=7.762850e-06, key='sharpe_ratio')
    _close(metrics.get('annual_return_pct'), -41.176798261586406, tol=4.117680e-05, key='annual_return_pct')
