"""Inlined regression test for the Two EMA Crossover + Intraday Time Filter strategy.
...
Strategy Principle:
    A dual-EMA trend-following crossover gated by an intraday time window. A
    fast EMA crossing above a slow EMA signals bullish momentum and a downward
    cross signals bearish momentum, but trades are only entered during the
    configured trading hours (here the early-session 00:00-06:00 block) to avoid
    low-quality signals at other times. Risk is bounded by ATR-scaled stop-loss
    and take-profit distances so position sizing adapts to current volatility.

Strategy Logic:
    ``__init__`` builds fast/slow EMAs and an ATR, and zeroes the trade
    counters and stop/target state. ``next`` waits for a warmup window, then on
    each bar checks ATR-based stop-loss/take-profit exits, reverses on opposite
    crossovers within trading hours, and otherwise opens a fresh long or short
    when a crossover occurs inside the allowed time window. ``notify_trade``
    tracks buy/sell, win, and loss tallies. Module-level helpers load the CSV,
...
class TwoEMATimeFilterStrategy(bt.Strategy):
    ...
    def __init__(self):
        """Build fast/slow EMAs and ATR, and zero counters and risk state."""
    ...
    def _in_trading_hours(self):
        """Return True if the current bar's hour is inside the trading window."""
        dt = bt.num2date(self.data.datetime[0])
        h = dt.hour
    ...
    def next(self):
        """Apply ATR stop/target exits and time-gated EMA-crossover entries."""
        self.bar_num += 1
