"""Inlined regression test for the Morning/Evening Star + Stochastic strategy.
...
Strategy Principle:
    Combines three-bar reversal candlestick patterns with a Stochastic filter.
    A Morning Star (or Morning Doji) marks a bullish reversal at a swing low,
    while an Evening Star (or Evening Doji) marks a bearish reversal at a swing
    high. Trades are only taken when the slow Stochastic %D confirms the move
    starts from a non-extreme zone (oversold for longs, overbought for shorts).
    Body sizes are measured against an SMA of recent bodies so only meaningful
    long candles anchor the pattern.
...
Strategy Logic:
    ...
    thresholds, and otherwise opens a long on a morning star/doji or a short on
    an evening star/doji confirmed by %D. ``notify_trade`` tracks buy/sell, win,
    and loss tallies. Module-level helpers load the CSV, build the cerebro with
...
class MorningStarStochStrategy(bt.Strategy):
    ...
    def next(self):
        """Manage Stochastic-based exits, then open star-pattern entries."""
        self.bar_num += 1
