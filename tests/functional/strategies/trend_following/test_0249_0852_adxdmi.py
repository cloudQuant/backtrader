"""Inlined regression test for trend_following/0249_0852_adxdmi.
...
class ExpADXDMIStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.ind = bt.indicators.ADXDMI(self.signal_data, period=self.p.dmi_period)
        self.bar_num = 0
        self.signal_count = 0
        self.trade_count = 0
        self.buy_count = 0
    ...
    def next(self):
        ...
        if len(self.signal_data) < int(self.p.dmi_period) + sb + 4:
            return
        if len(self.signal_data) == self._last_signal_len:
            return
        self._last_signal_len = len(self.signal_data)
        ind0 = float(self.ind.plus[-sb]) if sb else float(self.ind.plus[0])
        ind1 = float(self.ind.plus[-(sb + 1)])
        sig0 = float(self.ind.minus[-sb]) if sb else float(self.ind.minus[0])
        sig1 = float(self.ind.minus[-(sb + 1)])
...
def test_248_0249_0852_adxdmi() -> None:
    """Migrated regression test for trend_following/0249_0852_adxdmi."""
    ...
    assert strat.bar_num == 5616, f"bar_num: expected=5616, got={strat.bar_num}"
    assert strat.buy_count == 5, f"buy_count: expected=5, got={strat.buy_count}"
    assert strat.sell_count == 5, f"sell_count: expected=5, got={strat.sell_count}"
    assert strat.win_count == 4, f"win_count: expected=4, got={strat.win_count}"
    assert strat.loss_count == 6, f"loss_count: expected=6, got={strat.loss_count}"
    ...
    assert abs(final_value - 1000085.5) < 0.01, f"final_value: expected=1000085.5, got={final_value}"
