"""Inlined regression test for the Ravi AO trend-following strategy.
...
Data Used:
    ...
    bars and a second signal feed is resampled to M30 (30-minute) bars; both
...
Strategy Principle:
    This is a port of the MT5 expert advisor ``Ravi_AO``. It combines two
    momentum gauges computed on the higher M30 timeframe: the RAVI indicator
    (percentage spread between a fast and slow EMA of close) and Bill Williams'
    Awesome bt.indicators.Oscillator. A trade is taken only when both gauges flip sign in the
    same direction on consecutive bars, treating the simultaneous zero-line
    cross as confirmation of a fresh trend impulse.

Strategy Logic:
    load_backtest_frame builds the M15 base frame and resample_frame derives the
    M30 signal frame. On each execution bar the strategy applies a trailing stop
    to any open position, then acts once per new M30 bar while flat: it opens a
    long when AO and RAVI both cross up through zero and a short when both cross
    down. Entries attach protective stop and take-profit orders via
    _place_exit_orders. notify_order manages entry/stop/limit order state and
    notify_trade logs closed trades. summarize prints analyzer results, and the
    test forces runonce=True, derives metrics from the analyzers, and asserts
    each value against migration-time expectations.
"""
...
_CONFIG = {
    'strategy': {
        'name': 'Ravi AO',
        'source_ea': 'ea/0252_Ravi_AO/ravi_ao.mq5',
    },
    'data': {
        'symbol': 'XAUUSD',
        'base_timeframe': 'M15',
...
class RaviAOStrategy(bt.Strategy):
    """Dual-timeframe RAVI + Awesome Oscillator trend-following strategy."""
    ...
    def __init__(self):
        """Bind execution/signal feeds, build AO and RAVI, and reset order state."""
        ...
        self.ravi = bt.indicators.RaviIndicator(self.signal_data, fast_length=self.p.fast_length, slow_length=self.p.slow_length)
    ...
    def prenext(self):
        """Delegate to next() during the warmup phase so logic runs every bar."""
        self.next()
    ...
    def next(self):
        """Apply trailing stops, then open AO+RAVI cross trades once per M30 bar."""
    ...
    def notify_order(self, order):
        """Handle entry fills, attach exits, and reset entry/stop/limit state.
...
def build_cerebro(config, frame):
    """Assemble a Cerebro engine with the M15/M30 feeds, strategy, and analyzers.
...
def finite_or_none(value):
    ...
    Returns:
        The original value, or None if it is None or a non-finite float.
    """
    if value is None:
        return None
...
def summarize(results, start_value):
    """Print a human-readable backtest summary from the strategy's analyzers.

    Args:
        results: The list of strategies returned by cerebro.run().
...
def main():
    """Parse CLI args, run the Ravi_AO backtest, and print the summary."""
...
def test_26_0026_0252_ravi_ao() -> None:
    """Migrated regression test (runonce=True only).

    Originally located at tests/functional/strategies_regression/trend_following/0026_0252_ravi_ao.
    """
    captured = {}
