"""Two Exponential Moving Average crossover strategy functional test.
...
Strategy Principle:
    - **Market Hypothesis**: Fast versus slow Exponential Moving Average (EMA) crossovers represent momentum direction shifts. Moving average crossovers smoothly filter out high-frequency noise.
    - **EMA Indicator**:
        - *Fast EMA*: EMA with a period of 12 bars, tracking short-term price momentum.
        - *Slow EMA*: EMA with a period of 26 bars, tracking longer-term trend.
    - **Crossover Rules**:
        - *Bullish (Buy)*: Fast EMA crosses above the Slow EMA, indicating that upward momentum is expanding.
        - *Bearish (Sell)*: Fast EMA crosses below the Slow EMA, indicating that downward momentum is expanding.

Strategy Logic:
    1. **Initialization**: Configures fast and slow EMAs using `bt.indicators.EMA` based on configuration parameters (`fast_period`, `slow_period`). Sets tracking variables for trade counts and bar numbers.
    2. **Crossover Assessment**:
        - `diff1 = fast_ema_now - slow_ema_now` and `diff2 = fast_ema_prev - slow_ema_prev`.
        - `buy_sig` is True if `diff2 < 0` and `diff1 > 0` (bullish gold cross).
        - `sell_sig` is True if `diff2 > 0` and `diff1 < 0` (bearish death cross).
    3. **Execution**:
        - *Entry*: Enters long with a market order of size `lot` when a gold cross occurs and no active position. Enters short with size `lot` when a death cross occurs and no active position.
        - *Position Management (SAR - Stop and Reverse)*: If holding a long position and a death cross occurs, closes the long position and immediately reverses short. If holding a short position and a gold cross occurs, closes the short position and immediately reverses long.
    4. **Reporting**: Captures Sharpe ratio, total return, maximum drawdown, total trades, win rate, and profit factor.
"""
...
_CONFIG = {
    'strategy': {
        ...
        'source_ea': 'ea/1328_MQL5_Wizard_-_Trading_Signals_Based_on_Two_Exponential_Moving_Averages_Crossover/expert_cross_ema.mq5',
    },
    'data': {
        'symbol': 'XAUUSD',
        'timeframe': 'M15',
...
class TwoEMACrossStrategy(bt.Strategy):
    ...
    def __init__(self):
        """Initializes Fast and Slow EMAs and portfolio metrics tracking variables."""
    ...
    def next(self):
        ...
        Calculates fast and slow EMA differences to detect gold/death crossovers.
        Manages Stop-and-Reverse (SAR) execution for long and short directions.
        """
        self.bar_num += 1
