...
def compute_adx_crossing(frame, adx_period=50, atr_period=14):
    ...
    tr_smooth = wilder_smooth(tr, adx_period)
    ...
    buy_signal = (plus_di > minus_di) & (plus_di.shift(1) <= minus_di.shift(1))
    sell_signal = (plus_di < minus_di) & (plus_di.shift(1) >= minus_di.shift(1))
    ...
    return out.dropna(subset=['plus_di', 'minus_di', 'atr'])
...
class AdxCrossingStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.minus_di = self.h1.minus_di
    ...
    def _enough_history(self):
        ...
        try:
            float(self.plus_di[-idx])
            float(self.minus_di[-idx])
        except (TypeError, ValueError, IndexError):
            return False
        return True
    ...
    def next(self):
        ...
        self.log('adx plus_di={0:.2f} minus_di={1:.2f} buy_open={2} sell_open={3}'.format(float(self.plus_di[-idx]), float(self.minus_di[-idx]), buy_open, sell_open))
