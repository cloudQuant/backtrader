...
from strategy_adx_crossing import (
    AdxCrossingSignalFeed,
    AdxCrossingStrategy,
    Mt5PandasFeed,
    compute_adx_crossing,
    load_mt5_csv,
    resample_frame,
)
...
def load_backtest_frames(config):
    ...
    h1 = compute_adx_crossing(h1, adx_period=params['adx_period'], atr_period=params.get('atr_period', 14))
...
def build_cerebro(config, frame):
    ...
    feed_h1 = AdxCrossingSignalFeed(dataname=frame['h1'][['open', 'high', 'low', 'close', 'volume', 'openinterest', 'plus_di', 'minus_di', 'atr', 'buy_signal', 'sell_signal']], timeframe=bt.TimeFrame.Minutes, compression=60)

    cerebro.adddata(feed_m15, name='XAUUSD_M15')
    cerebro.adddata(feed_h1, name='XAUUSD_H1_ADXCrossing')
    cerebro.addstrategy(AdxCrossingStrategy, **config.get('params', {}))
...
def print_report(metrics):
    print('\n' + '=' * 60 + '\nBACKTEST RESULTS — Exp_ADXCrossing\n' + '=' * 60)
