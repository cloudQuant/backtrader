...
from strategy_afirma import AfirmaFeed, AfirmaStrategy, Mt5PandasFeed, compute_afirma, load_mt5_csv, resample_frame
...
def load_backtest_frames(config):
    ...
    h4 = compute_afirma(h4, periods=params['periods'], taps=params['taps'], window=params['window'])
...
def build_cerebro(config, frame):
    ...
    feed_h4 = AfirmaFeed(dataname=frame['h4'][['open', 'high', 'low', 'close', 'volume', 'openinterest', 'firma', 'arma']], timeframe=bt.TimeFrame.Minutes, compression=240)
    cerebro.adddata(feed_m15, name='XAUUSD_M15')
    cerebro.adddata(feed_h4, name='XAUUSD_H4_AFIRMA')
    cerebro.addstrategy(AfirmaStrategy, **config.get('params', {}))
