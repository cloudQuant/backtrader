...
from strategy_hulltrend import HullTrendFeed, HullTrendStrategy, Mt5PandasFeed, compute_hulltrend, load_mt5_csv, resample_frame
...
def load_backtest_frames(config):
    ...
    h4 = compute_hulltrend(h4, xlength=params['xlength'], ipc=params['ipc'], xma_method=params['xma_method'], xphase=params['xphase'])
...
def build_cerebro(config, frame):
    ...
    feed_h4 = HullTrendFeed(dataname=frame['h4'][['open', 'high', 'low', 'close', 'volume', 'openinterest', 'ind', 'sign']], timeframe=bt.TimeFrame.Minutes, compression=240)
    cerebro.adddata(feed_m15, name='XAUUSD_M15')
    cerebro.adddata(feed_h4, name='XAUUSD_H4_HULLTREND')
    cerebro.addstrategy(HullTrendStrategy, **config.get('params', {}))
...
def print_report(metrics):
    print('\n' + '=' * 60 + '\nBACKTEST RESULTS — Exp_HullTrend\n' + '=' * 60)
    for key in [
