...
def price_series(frame, applied_price):
    ...
    if key in ('PRICE_TRENDFOLLOW0', 'PRICE_TRENDFOLLOW0_'):
        return np.where(frame['close'] >= frame['open'], frame['high'], frame['low'])
    if key in ('PRICE_TRENDFOLLOW1', 'PRICE_TRENDFOLLOW1_'):
        return np.where(frame['close'] >= frame['open'], frame['low'], frame['high'])
...
def compute_hulltrend(frame, xlength=20, ipc='PRICE_CLOSE_', xma_method='MODE_LWMA', xphase=15):
    price = pd.Series(price_series(frame, ipc), index=frame.index, dtype=float)
    xlength = int(max(xlength, 1))
    xlength2 = int(max(xlength // 2, 1))
    sqrt_length = int(max(math.floor(math.sqrt(xlength)), 1))
    xma2 = smooth_series(price, xma_method, xlength2)
    xma = smooth_series(price, xma_method, xlength)
    hma = 2.0 * xma2 - xma
    xhma = smooth_series(hma, xma_method, sqrt_length)
    out = frame.copy()
    out['ind'] = hma
    out['sign'] = xhma
...
class HullTrendFeed(bt.feeds.PandasData):
    lines = ('ind', 'sign')
    params = (
        ('datetime', None), ('open', 0), ('high', 1), ('low', 2),
        ('close', 3), ('volume', 4), ('openinterest', 5),
...
class HullTrendStrategy(bt.Strategy):
    params = dict(
        mm=0.1,
        mm_mode='LOT',
        stop_loss=1000,
    ...
    def __init__(self):
        ...
        sqrt_length = int(max(math.floor(math.sqrt(max(int(self.p.xlength), 1))), 1))
        self.min_signal_bars = int(self.p.xlength) + sqrt_length + max(int(self.p.signal_bar), 1) + 5
    ...
    def next(self):
        ...
        self.log('hulltrend ind_prev={0:.5f} sign_prev={1:.5f} ind_curr={2:.5f} sign_curr={3:.5f} buy_open={4} sell_open={5}'.format(ind_prev, sign_prev, ind_curr, sign_curr, buy_open, sell_open))
