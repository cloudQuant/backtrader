...
class RobotMACDStrategy(bt.Strategy):
    ...
    def _buy_signal(self):
        ...
        return macd_curr > signal_curr and macd_prev < signal_prev and macd_curr < 0 and signal_curr < 0
    ...
    def _manage_position(self):
        ...
        if self.position.size > 0:
            if self._sell_signal() or (self.take_profit_price is not None and high >= self.take_profit_price):
                self.order = self.close()
                return True
        else:
            if self._buy_signal() or (self.take_profit_price is not None and low <= self.take_profit_price):
                self.order = self.close()
                return True
        return False
