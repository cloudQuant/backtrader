...
class ExpDonchianChannelsSystemStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.ind = DonchianChannelsSystem(self.signal_data, period=self.p.period, shift=self.p.shift, margins=self.p.margins)
        self.bar_num = 0
        self.signal_count = 0
        self.trade_count = 0
        self.buy_count = 0
    ...
    def next(self):
        ...
        if len(self.signal_data) < int(self.p.period) + int(self.p.shift) + sb + 4: return
