...
def prepare_fast_trend_features(df, params):
    """准备快速趋势跟踪策略特征"""
    out = df.copy()
    ...
    out = out[['open', 'high', 'low', 'close', 'volume', 'openinterest', 
               'ma_fast', 'ma_slow', 'golden_cross', 'death_cross']].copy()
    return out.dropna()


class Mt5FastTrendFeed(bt.feeds.PandasData):
    lines = ('ma_fast', 'ma_slow', 'golden_cross', 'death_cross',)
    params = (
        ('datetime', None), ('open', 0), ('high', 1), ('low', 2),
        ('close', 3), ('volume', 4), ('openinterest', 5),
        ('ma_fast', 6), ('ma_slow', 7), ('golden_cross', 8), ('death_cross', 9),
    )


class FastTrendFollowingStrategy(bt.Strategy):
    params = dict(
        fast_period=50,
        slow_period=100,
        lot_size=1.0,
