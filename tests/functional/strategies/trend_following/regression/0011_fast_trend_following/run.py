...
from strategy_fast_trend import (
    Mt5FastTrendFeed, FastTrendFollowingStrategy, load_mt5_csv, prepare_fast_trend_features,
)
...
def load_data(config):
    ...
    df = prepare_fast_trend_features(raw, config.get('params', {}))
...
def build_cerebro(frame, config):
    ...
    feed = Mt5FastTrendFeed(dataname=frame['data'], timeframe=bt.TimeFrame.Days, compression=1)
    cerebro.adddata(feed, name=f"{config['data']['symbol']}_{config['data']['timeframe']}")
    cerebro.addstrategy(FastTrendFollowingStrategy, **config.get('params', {}))
