"""Inlined regression test for tests/functional/strategies/trend_following/regression/0051_0631_doublema_crossover.
...
_CONFIG = {
    'strategy': {
        'name': 'DoubleMA_Crossover',
        'source_ea': 'ea/0631_DoubleMA_Crossover_EA/doublema_crossover_ea.mq5',
    },
    'data': {
        'symbol': 'XAUUSD',
        'timeframe': 'M15',
...
def test_51_0051_0631_doublema_crossover() -> None:
    """Migrated regression test (runonce=True only).

    Originally located at tests/functional/strategies_regression/trend_following/0051_0631_doublema_crossover.
    """
    ...
    assert metrics.get('buy_count') == 1414, f"buy_count: expected=1414, got={metrics.get('buy_count')!r}"
    assert metrics.get('sell_count') == 1265, f"sell_count: expected=1265, got={metrics.get('sell_count')!r}"
    assert metrics.get('total_trades') == 2678, f"total_trades: expected=2678, got={metrics.get('total_trades')!r}"
    assert metrics.get('trade_count') == 2678, f"trade_count: expected=2678, got={metrics.get('trade_count')!r}"
    assert metrics.get('won') == 1267, f"won: expected=1267, got={metrics.get('won')!r}"
    assert metrics.get('lost') == 1411, f"lost: expected=1411, got={metrics.get('lost')!r}"
    _close(metrics.get('bars'), 6129.0, tol=6.129000e-03, key='bars')
    _close(metrics.get('signal_count'), 2679.0, tol=2.679000e-03, key='signal_count')
    _close(metrics.get('completed_orders'), 5357.0, tol=5.357000e-03, key='completed_orders')
    _close(metrics.get('rejected_orders'), 0.0, tol=1.000000e-06, key='rejected_orders')
    _close(metrics.get('initial_cash'), 1000000.0, tol=1.000000e+00, key='initial_cash')
    _close(metrics.get('final_value'), 997462.8999999992, tol=9.974629e-01, key='final_value')
    _close(metrics.get('net_pnl'), -2537.1000000007916, tol=2.537100e-03, key='net_pnl')
    _close(metrics.get('total_return_pct'), -0.2537100000000847, tol=1.000000e-06, key='total_return_pct')
    _close(metrics.get('win_rate'), 47.31142643764003, tol=4.731143e-05, key='win_rate')
    _close(metrics.get('profit_factor'), 0.9721172563838306, tol=1.000000e-06, key='profit_factor')
    _close(metrics.get('sharpe_ratio'), -2.858697110141766, tol=2.858697e-06, key='sharpe_ratio')
    _close(metrics.get('annual_return_pct'), -13.964060837241501, tol=1.396406e-05, key='annual_return_pct')
    _close(metrics.get('max_drawdown'), 0.9123696900624382, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('sqn'), -0.38474026004098766, tol=1.000000e-06, key='sqn')
