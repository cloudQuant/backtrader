...
from strategy_color_schaff_rsi_trend_cycle import (
    ColorSchaffRSITrendCycleFeed,
    ColorSchaffRSITrendCycleStrategy,
    Mt5PandasFeed,
    compute_color_schaff_rsi_trend_cycle,
    load_mt5_csv,
    resample_frame,
)
...
def load_backtest_frames(config):
    ...
    h4 = compute_color_schaff_rsi_trend_cycle(h4, fast_rsi=params['fast_rsi'], slow_rsi=params['slow_rsi'], applied_price=params['applied_price'], cycle=params['cycle'], high_level=params['high_level'], low_level=params['low_level'])
...
def build_cerebro(config, frame):
    ...
    feed_h4 = ColorSchaffRSITrendCycleFeed(dataname=frame['h4'][['open', 'high', 'low', 'close', 'volume', 'openinterest', 'stc', 'color_code', 'buy_signal', 'sell_signal']], timeframe=bt.TimeFrame.Minutes, compression=240)

    cerebro.adddata(feed_m15, name='XAUUSD_M15')
    cerebro.adddata(feed_h4, name='XAUUSD_H4_ColorSchaffRSITrendCycle')
    cerebro.addstrategy(ColorSchaffRSITrendCycleStrategy, **config.get('params', {}))
...
def print_report(metrics):
    print('\n' + '=' * 60 + '\nBACKTEST RESULTS — Exp_ColorSchaffRSITrendCycle\n' + '=' * 60)
