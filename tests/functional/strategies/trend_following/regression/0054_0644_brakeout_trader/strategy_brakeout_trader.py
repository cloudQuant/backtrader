...
class BrakeoutTraderStrategy(bt.Strategy):
    """EA 0644: Breakout trader — enter when close crosses a manually set price level.
    Close[1] > AppPrice && Close[2] <= AppPrice → Buy.
    Close[1] < AppPrice && Close[2] >= AppPrice → Sell.
    Fixed SL/TP.
    """
    ...
    def next(self):
        ...
        close_2 = float(self.data.close[-2])
        app = float(self.p.app_price)
        price = float(self.data.close[0])

        long_signal = close_1 > app and close_2 <= app and self.p.buys
        short_signal = close_1 < app and close_2 >= app and self.p.sells
        ...
        if self.position:
            if self.position.size < 0 and long_signal:
                ...
                return
            if self.position.size > 0 and short_signal:
