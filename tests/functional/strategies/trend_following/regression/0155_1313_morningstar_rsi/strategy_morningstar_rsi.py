...
class MorningStarRsiStrategy(bt.Strategy):
    """
    Morning Star / Evening Star + RSI confirmation.
    Buy: Morning Star (or Doji) AND RSI < rsi_entry_long
    Sell: Evening Star (or Doji) AND RSI > rsi_entry_short
    Exit: RSI crosses overbought/oversold levels
    """
    params = dict(
        rsi_period=11,
        rsi_entry_long=40,
    ...
    def next(self):
        ...
        if self.position:
            ...
            if (self._is_morning_star() or self._is_morning_doji()) and rsi1 < self.p.rsi_entry_long:
                self.log(f'buy morningstar+rsi rsi={rsi1:.1f}')
                self.buy(size=self.p.lot)
                return
            if (self._is_evening_star() or self._is_evening_doji()) and rsi1 > self.p.rsi_entry_short:
                self.log(f'sell eveningstar+rsi rsi={rsi1:.1f}')
