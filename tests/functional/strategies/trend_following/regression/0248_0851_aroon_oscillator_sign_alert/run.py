...
from strategy_aroon_oscillator_sign_alert import ExpAroonOscillatorSignAlertStrategy, Mt5PandasFeed, load_mt5_csv
...
def run(plot=False):
    ...
    cerebro.addstrategy(ExpAroonOscillatorSignAlertStrategy, **params)
    results = cerebro.run()
    if plot:
        cerebro.plot()
    return results
