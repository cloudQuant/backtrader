...
from strategy_kaufwmacross import KaufWMAcrossFeed, KaufWMAcrossStrategy, Mt5PandasFeed, compute_kaufwmacross, load_mt5_csv, resample_frame
...
def load_backtest_frames(config):
    ...
    signal_minutes = data_cfg.get('signal_tf_minutes', 360)
    signal_df = resample_frame(base, '{0}min'.format(signal_minutes))
    signal_df = compute_kaufwmacross(signal_df, ama_period=params['ama_period'], fast_ma_period=params['fast_ma_period'], slow_ma_period=params['slow_ma_period'], ama_price=params['ama_price'], ma_period=params['ma_period'], ma_type=params['ma_type'], ma_price=params['ma_price'], point=params['point'])
...
def build_cerebro(config, frame):
    ...
    feed_signal = KaufWMAcrossFeed(dataname=frame['signal'][['open', 'high', 'low', 'close', 'volume', 'openinterest', 'sell_arrow', 'buy_arrow']], timeframe=bt.TimeFrame.Minutes, compression=signal_minutes)
    cerebro.adddata(feed_m15, name='XAUUSD_M15')
    cerebro.adddata(feed_signal, name='XAUUSD_SIGNAL')
    cerebro.addstrategy(KaufWMAcrossStrategy, **config.get('params', {}))
...
def print_report(metrics):
    print('\n' + '=' * 60 + '\nBACKTEST RESULTS — Exp_KaufWMAcross\n' + '=' * 60)
    for key in [
        'fromdate', 'todate', 'bars_m15', 'bars_signal', 'bar_num', 'buy_signals', 'sell_signals',
        'buy_count', 'sell_count', 'trade_count', 'completed_orders', 'rejected_orders',
