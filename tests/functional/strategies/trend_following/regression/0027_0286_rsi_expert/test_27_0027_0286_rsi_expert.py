"""Inlined regression test for tests/functional/strategies/trend_following/regression/0027_0286_rsi_expert.
...
_CONFIG = {
    'strategy': {
        'name': 'RSI Expert',
        'source_ea': 'ea/0286_RSI_Expert/rsi_expert.mq5',
    },
    'data': {
        'symbol': 'XAUUSD',
        'base_timeframe': 'M15',
...
def test_27_0027_0286_rsi_expert() -> None:
    """Migrated regression test (runonce=True only).

    Originally located at tests/functional/strategies_regression/trend_following/0027_0286_rsi_expert.
    """
    captured = {}
    ...
    assert metrics.get('trade_num') == 272, f"trade_num: expected=272, got={metrics.get('trade_num')!r}"
    assert metrics.get('total_trades') == 272, f"total_trades: expected=272, got={metrics.get('total_trades')!r}"
    _close(metrics.get('final_value'), 998052.8000000002, tol=9.980528e-01, key='final_value')
    _close(metrics.get('initial_cash'), 1000000.0, tol=1.000000e+00, key='initial_cash')
    _close(metrics.get('max_drawdown'), 0.2392637730133372, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('annual_return'), -0.3571085409264164, tol=1.000000e-06, key='annual_return')
    _close(metrics.get('return_rate'), -0.0019490982585127384, tol=1.000000e-06, key='return_rate')
    _close(metrics.get('sharpe_ratio'), -10.994033691969854, tol=1.099403e-05, key='sharpe_ratio')
    _close(metrics.get('sqn'), -1.4387608654354074, tol=1.438761e-06, key='sqn')
