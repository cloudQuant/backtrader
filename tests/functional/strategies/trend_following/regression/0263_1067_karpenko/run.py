...
def load_backtest_frames(config):
    ...
    full_base = load_mt5_csv(resolve_data_path(data_cfg['file']), bar_shift_minutes=data_cfg.get('bar_shift_minutes', 0))
    if full_base.empty:
        raise ValueError('Loaded data frame is empty')
    signal_minutes = data_cfg.get('signal_tf_minutes', 240)
    signal_df = resample_frame(full_base, '{0}min'.format(signal_minutes))
    signal_df = compute_karpenko(signal_df, basic_ma=params['basic_ma'], history=params['history'])
    base = full_base[(full_base.index >= fromdate) & (full_base.index <= todate)].copy()
    signal_df = signal_df[(signal_df.index >= fromdate) & (signal_df.index <= todate)].copy()
    if base.empty:
        raise ValueError('Loaded data frame is empty in requested backtest window')
...
def build_cerebro(config, frame):
    ...
    feed_signal = KarpenkoFeed(dataname=sig, timeframe=bt.TimeFrame.Minutes, compression=signal_minutes)
    cerebro.adddata(feed_m15, name='XAUUSD_M15')
    cerebro.adddata(feed_signal, name='XAUUSD_H4_KARPENKO')
    cerebro.addstrategy(KarpenkoStrategy, **config.get('params', {}))
...
def print_report(metrics):
    print('\n' + '=' * 60 + '\nBACKTEST RESULTS — Exp_Karpenko\n' + '=' * 60)
    for key in [
        'fromdate', 'todate', 'bars_m15', 'bars_signal', 'bar_num', 'buy_signals', 'sell_signals',
        'buy_count', 'sell_count', 'trade_count', 'completed_orders', 'rejected_orders', 'initial_cash',
        'final_value', 'net_pnl', 'total_return_pct', 'total_trades', 'won', 'lost', 'win_rate',
        'profit_factor', 'sharpe_ratio', 'annual_return_pct', 'max_drawdown', 'sqn',
    ]:
        value = metrics[key]
...
def run(plot=False):
    ...
    if frame['signal'].empty:
        print('\nStarting backtest...')
        print('No signal bars available in requested window after Karpenko warmup; returning zero-trade report.')
        metrics = build_empty_metrics(frame, config)
