...
def compute_color_schaff_momentum_trend_cycle(frame, fast_momentum=23, slow_momentum=50, applied_price='close', cycle=10, high_level=60, low_level=-60):
    price = frame['close'].astype(float)
    if str(applied_price).lower() == 'open':
        price = frame['open'].astype(float)
    elif str(applied_price).lower() == 'high':
        price = frame['high'].astype(float)
    elif str(applied_price).lower() == 'low':
        price = frame['low'].astype(float)
    elif str(applied_price).lower() in ('median', 'price_median'):
        price = (frame['high'].astype(float) + frame['low'].astype(float)) / 2.0
    elif str(applied_price).lower() in ('typical', 'price_typical'):
        price = (frame['high'].astype(float) + frame['low'].astype(float) + frame['close'].astype(float)) / 3.0
    elif str(applied_price).lower() in ('weighted', 'price_weighted'):
        price = (frame['high'].astype(float) + frame['low'].astype(float) + 2.0 * frame['close'].astype(float)) / 4.0

    fast = compute_momentum(price, fast_momentum)
    slow = compute_momentum(price, slow_momentum)
    macd = fast - slow
...
class ColorSchaffMomentumTrendCycleFeed(bt.feeds.PandasData):
...
class ColorSchaffMomentumTrendCycleStrategy(bt.Strategy):
    params = dict(
        mm=0.1,
        mm_mode='LOT',
        stop_loss=1000,
    ...
    def next(self):
        ...
        self.log('schaff_momentum stc={0:.6f} color={1:.0f} buy_open={2} sell_open={3}'.format(float(self.stc[-idx]), float(self.color_code[-idx]), buy_open, sell_open))
