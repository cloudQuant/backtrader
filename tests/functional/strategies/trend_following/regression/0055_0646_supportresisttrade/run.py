...
import yaml
from strategy_supportresisttrade import Mt5PandasFeed, SupportResistTradeStrategy, load_mt5_csv
...
def build_cerebro(config, frame):
    ...
    cerebro.addstrategy(SupportResistTradeStrategy, **config.get('params', {}))
...
def print_report(metrics):
    print('\n' + '=' * 60 + '\nBACKTEST RESULTS — SupportResistTrade\n' + '=' * 60)
    for key in ['fromdate','todate','bars','bar_num','signal_count','buy_count','sell_count','trade_count',
        ...
        value = metrics[key]
        if isinstance(value, float): print(f'  {key:20s}: {value:,.2f}')
        else: print(f'  {key:20s}: {value}')
    print('=' * 60)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(); parser.add_argument('--plot', action='store_true'); args = parser.parse_args()
    run(plot=args.plot)
