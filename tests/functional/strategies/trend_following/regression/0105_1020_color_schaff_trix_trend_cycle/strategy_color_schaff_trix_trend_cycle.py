...
def compute_color_schaff_trix_trend_cycle(frame, fast_trix=23, slow_trix=50, applied_price='close', cycle=10, high_level=60, low_level=-60):
    price = compute_applied_price(frame, applied_price)
    fast = compute_trix(price, fast_trix)
    slow = compute_trix(price, slow_trix)
    macd = fast - slow
...
class ColorSchaffTriXTrendCycleFeed(bt.feeds.PandasData):
...
class ColorSchaffTriXTrendCycleStrategy(bt.Strategy):
    params = dict(
        mm=0.1,
        mm_mode='LOT',
        stop_loss=1000,
    ...
    def next(self):
        ...
        self.log('schaff_trix stc={0:.6f} color={1:.0f} buy_open={2} sell_open={3}'.format(float(self.stc[-idx]), float(self.color_code[-idx]), buy_open, sell_open))
