...
class ElliStrategy(bt.Strategy):
    """EA 0630: ADX Wilder + Ichimoku Kinko Hyo trend following.
    Buy when: ts>ks && ks>sa && sa>sb && close>ks && ADX+DI crosses from low to high && |ts-ks|>po points.
    Sell mirror condition.
    Fixed SL/TP.
    """
    ...
    def _signal(self):
        ts = float(self.ichimoku.tenkan_sen[0])
        ks = float(self.ichimoku.kijun_sen[0])
        sa = float(self.ichimoku.senkou_span_a[0])
        sb = float(self.ichimoku.senkou_span_b[0])
        close_val = float(self.data.close[0])
        cf = abs(ts - ks) / self._point()

        plus_di_0 = float(self.di.plusDI[0])
        plus_di_1 = float(self.di.plusDI[-1])

        if (ts > ks and ks > sa and sa > sb and close_val > ks and
                plus_di_1 < self.p.convert_low and plus_di_0 > self.p.convert_high and cf > self.p.po):
            return 'buy'
        if (ts < ks and ks < sa and sa < sb and close_val < ks and
                plus_di_1 < self.p.convert_low and plus_di_0 > self.p.convert_high and cf > self.p.po):
            return 'sell'
