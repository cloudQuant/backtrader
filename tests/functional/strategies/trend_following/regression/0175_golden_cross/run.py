...
from strategy_golden_cross import (
    Mt5GoldenCrossFeed, GoldenCrossStrategy, load_mt5_csv, prepare_golden_cross_features,
)
...
def load_data(config):
    ...
    df = prepare_golden_cross_features(raw, config.get('params', {}))
...
def build_cerebro(frame, config):
    ...
    feed = Mt5GoldenCrossFeed(dataname=frame['data'], timeframe=bt.TimeFrame.Days, compression=1)
    cerebro.adddata(feed, name=f"{config['data']['symbol']}_{config['data']['timeframe']}")
    cerebro.addstrategy(GoldenCrossStrategy, **config.get('params', {}))
