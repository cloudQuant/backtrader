...
from strategy_xd_rangeswitch import (
    Mt5PandasFeed,
    XdRangeSwitchFeed,
    XdRangeSwitchStrategy,
    compute_xd_rangeswitch,
    load_mt5_csv,
    resample_frame,
)
...
def load_backtest_frames(config):
    ...
    h4 = compute_xd_rangeswitch(h4, n=params['n'])
...
def build_cerebro(config, frame):
    ...
    feed_h4 = XdRangeSwitchFeed(dataname=frame['h4'][['open', 'high', 'low', 'close', 'volume', 'openinterest', 'upper_trend', 'lower_trend', 'sell_signal', 'buy_signal']], timeframe=bt.TimeFrame.Minutes, compression=240)
    cerebro.adddata(feed_m15, name='XAUUSD_M15')
    cerebro.adddata(feed_h4, name='XAUUSD_H4_XD')
    cerebro.addstrategy(XdRangeSwitchStrategy, **config.get('params', {}))
...
def print_report(metrics):
    print('\n' + '=' * 60 + '\nBACKTEST RESULTS — Exp_XD-RangeSwitch\n' + '=' * 60)
    for key in [
