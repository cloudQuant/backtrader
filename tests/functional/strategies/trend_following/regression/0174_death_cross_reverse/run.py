...
from strategy_death_cross_reverse import (
    Mt5DeathCrossReverseFeed, DeathCrossReverseStrategy, load_mt5_csv, prepare_death_cross_reverse_features,
)
...
def load_data(config):
    ...
    df = prepare_death_cross_reverse_features(raw, config.get('params', {}))
...
def build_cerebro(frame, config):
    ...
    feed = Mt5DeathCrossReverseFeed(dataname=frame['data'], timeframe=bt.TimeFrame.Days, compression=1)
    cerebro.adddata(feed, name=f"{config['data']['symbol']}_{config['data']['timeframe']}")
    cerebro.addstrategy(DeathCrossReverseStrategy, **config.get('params', {}))
