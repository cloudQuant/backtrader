"""Inlined regression test for tests/functional/strategies/trend_following/regression/0125_1188_2ma_rsi.
...
def test_125_0125_1188_2ma_rsi() -> None:
    """Migrated regression test (runonce=True only).

    Originally located at tests/functional/strategies_regression/trend_following/0125_1188_2ma_rsi.
    """
    ...
    assert metrics.get('bar_num') == 6105, f"bar_num: expected=6105, got={metrics.get('bar_num')!r}"
    ...
    _close(metrics.get('final_value'), 1000305.1000000001, tol=1.000305e+00, key='final_value')
    _close(metrics.get('net_pnl'), 305.10000000009313, tol=3.051000e-04, key='net_pnl')
    _close(metrics.get('total_return_pct'), 0.03051000000000581, tol=1.000000e-06, key='total_return_pct')
    _close(metrics.get('win_rate'), 71.42857142857143, tol=7.142857e-05, key='win_rate')
    _close(metrics.get('profit_factor'), 3.2351648351649773, tol=3.235165e-06, key='profit_factor')
    _close(metrics.get('max_drawdown'), 0.013648873967897648, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('sharpe_ratio'), 7.731925231574446, tol=7.731925e-06, key='sharpe_ratio')
    _close(metrics.get('annual_return_pct'), 1.8225407344407103, tol=1.822541e-06, key='annual_return_pct')
    _close(metrics.get('sqn'), 1.0861094080924576, tol=1.086109e-06, key='sqn')
