...
Strategy Principle:
    A Parabolic SAR plus shifted moving-average trend system. The SAR position
    and a shifted MA define direction: when the SAR sits below the MA (or the
    shifted close is below the MA) a long is favored, and the mirror condition
    favors a short. Each position carries fixed pip-based stop-loss and take-
    profit levels plus a trailing stop that ratchets once price advances beyond
    the trailing distance plus a step.

Strategy Logic:
    ``__init__`` builds the MA (SMA or EMA) and Parabolic SAR indicators and
    zeroes the trade counters and per-position risk state. ``next`` waits for the
    warmup, sets initial protection, checks stop/take exits, updates the trailing
    stop while in a position, and otherwise opens a long/short on the SAR/MA
    signal. ``notify_order`` records the position side and protection on fills and
    clears the working order, and ``notify_trade`` tallies win/loss. The
...
class SarTradingV20Strategy(bt.Strategy):
    """Parabolic SAR + shifted MA trend system with fixed risk and trailing stop."""
    ...
    def __init__(self):
        """Build the MA and Parabolic SAR indicators and zero counters/risk state."""
    ...
    def next(self):
        """Set protection, manage stop/take/trailing exits, else open on SAR/MA signal."""
        self.bar_num += 1
    ...
    def notify_order(self, order):
        """Record the position side and protection on fills, then clear the order.
