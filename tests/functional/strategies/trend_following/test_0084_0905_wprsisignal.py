"""Inlined regression test for the 0905 Exp_WPRSIsignal multi-timeframe EA.
...
Data Used:
    ...
    ``indicator_minutes``) carries the WPRSI indicator. The window is clipped to
    2025-12-03 01:15:00 .. 2026-03-10 09:00:00.

Strategy Principle:
    WPRSIsignal combines Williams %R with RSI on the higher (H1) timeframe to
    flag exhausted swings. A buy arrow prints when %R crosses up through -20
    with RSI above 50 (and no recent overbought touch within the filter window);
    a sell arrow prints on the mirror oversold cross. The EA opens longs/shorts
    on the arrows and closes opposing positions, with fixed point-based stop loss
    and take profit.
...
Strategy Logic:
    ...
    and ``ExpWPRSISignalStrategy`` with the standard analyzer stack.
    ``WPRSISignalIndicator`` recomputes the %R/RSI buy/sell arrows. The strategy
...
_CONFIG = {
    'strategy': {
        'name': 'Exp_WPRSIsignal',
        'source_ea': 'ea/0905_Exp_WPRSIsignal',
    },
    'data': {
        'symbol': 'XAUUSD',
        'timeframe': 'M15',
...
class ExpWPRSISignalStrategy(bt.Strategy):
    ...
    def next(self):
        """Manage exits, then act on the latest WPRSI buy/sell arrow signals."""
        self.bar_num += 1
        if len(self.base) < 2:
            return
...
def resolve_data_path(fn):
    """Resolve ``fn`` against this test's directory and confirm it exists.
...
def load_backtest_frame(cfg):
    ...
    Args:
        cfg: Resolved config mapping whose ``data`` section names the file,
...
def build_signal_frame(df, mins):
    ...
    Args:
        ...
        mins: Target resample period in minutes (the H1 indicator timeframe).
...
def build_cerebro(cfg, frame):
    ...
    Args:
        cfg: Resolved config mapping providing ``backtest``, ``data`` and ``params``.
...
def extract_metrics(strat, cerebro, frame, cfg):
    ...
    Args:
        ...
        frame: Frame mapping returned by :func:`load_backtest_frame`.
        cfg: Resolved config mapping for cash references.
...
def test_84_0084_0905_wprsisignal() -> None:
    """Migrated regression test (runonce=True only).

    Originally located at tests/functional/strategies_regression/trend_following/0084_0905_wprsisignal.
    """
    ...
    assert metrics.get('bar_num') == 5977, f"bar_num: expected=5977, got={metrics.get('bar_num')!r}"
    ...
    _close(metrics.get('final_value'), 999297.7000000002, tol=9.992977e-01, key='final_value')
    _close(metrics.get('net_pnl'), -702.2999999998137, tol=7.023000e-04, key='net_pnl')
    _close(metrics.get('total_return_pct'), -0.07022999999998225, tol=1.000000e-06, key='total_return_pct')
    _close(metrics.get('win_rate'), 40.54054054054054, tol=4.054054e-05, key='win_rate')
    _close(metrics.get('profit_factor'), 0.6755820399113208, tol=1.000000e-06, key='profit_factor')
    _close(metrics.get('max_drawdown'), 0.14773644729533034, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('sharpe_ratio'), -4.680057408818669, tol=4.680057e-06, key='sharpe_ratio')
    _close(metrics.get('annual_return_pct'), -4.0696966955523095, tol=4.069697e-06, key='annual_return_pct')
    _close(metrics.get('sqn'), -0.6121486766163374, tol=1.000000e-06, key='sqn')
