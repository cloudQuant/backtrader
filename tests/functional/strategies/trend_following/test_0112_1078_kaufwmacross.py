"""Inlined regression test for tests/functional/strategies/trend_following/regression/0112_1078_kaufwmacross.
...
_CONFIG = {
    'strategy': {
        'name': 'Exp_KaufWMAcross',
        'source_ea': 'ea/1078_Exp_KaufWMAcross',
    },
    'data': {
        'symbol': 'XAUUSD',
        'timeframe': 'M15',
...
def test_112_0112_1078_kaufwmacross() -> None:
    """Migrated regression test (runonce=True only).

    Originally located at tests/functional/strategies_regression/trend_following/0112_1078_kaufwmacross.
    """
    ...
    _close(metrics.get('bars_signal'), 270.0, tol=2.700000e-04, key='bars_signal')
    _close(metrics.get('buy_signals'), 9.0, tol=9.000000e-06, key='buy_signals')
    _close(metrics.get('sell_signals'), 9.0, tol=9.000000e-06, key='sell_signals')
    _close(metrics.get('completed_orders'), 28.0, tol=2.800000e-05, key='completed_orders')
    _close(metrics.get('rejected_orders'), 0.0, tol=1.000000e-06, key='rejected_orders')
    _close(metrics.get('initial_cash'), 1000000.0, tol=1.000000e+00, key='initial_cash')
    _close(metrics.get('final_value'), 1012510.8000000023, tol=1.012511e+00, key='final_value')
    _close(metrics.get('net_pnl'), 12510.800000002258, tol=1.251080e-02, key='net_pnl')
    _close(metrics.get('total_return_pct'), 1.2510800000002265, tol=1.251080e-06, key='total_return_pct')
    _close(metrics.get('win_rate'), 64.28571428571429, tol=6.428571e-05, key='win_rate')
    _close(metrics.get('profit_factor'), 3.8272994350282525, tol=3.827299e-06, key='profit_factor')
    _close(metrics.get('sharpe_ratio'), 20.771637755308063, tol=2.077164e-05, key='sharpe_ratio')
    _close(metrics.get('annual_return_pct'), 108.6342114650515, tol=1.086342e-04, key='annual_return_pct')
    _close(metrics.get('max_drawdown'), 0.3294543900008476, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('sqn'), 2.375496459005684, tol=2.375496e-06, key='sqn')
