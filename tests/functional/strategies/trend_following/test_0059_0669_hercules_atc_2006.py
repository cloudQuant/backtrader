"""Hercules A.T.C. 2006 strategy regression test.

Data Used:
    XAUUSD M15 bars from ``tests/datas/XAUUSD_M15.csv`` are loaded from MT5 TSV
    export with a 15-minute shift and clipped to the configured range
    ``2026-01-15 00:00:00`` through ``2026-03-10 09:00:00``.
    The backtest uses M15 base data and H1/H4/D1 resampled derived feeds.

Strategy Principle:
    The strategy combines fast/slow MAs, RSI and envelope conditions from higher
    timeframes to form breakout/trend entries. Position size, risk and exits are
    controlled with configurable triggers and two-tier take-profit logic.

Strategy Logic:
    The module loads and normalizes MT5 data, initializes multi-timeframe feeds,
    runs ``HerculesAtc2006Strategy`` and collects counters from order/trade
    callbacks. The exported metrics are asserted against migration-time
...
class HerculesAtc2006Strategy(bt.Strategy):
    """Hercules A.T.C. 2006 multi-timeframe strategy implementation."""
    params = dict(
        lots=0.01,
    ...
    def __init__(self):
        """Prepare indicators, state counters, and leg/order bookkeeping."""
    ...
    def next(self):
        """Drive cross detection, entry filtering, and exit management each bar."""
        self.bar_num += 1
        self._clear_orders()
        self._manage_legs()
    ...
    def notify_order(self, order):
        """Record completed/rejected orders and update leg-open state."""
    ...
    def notify_trade(self, trade):
        """Update trade outcome counters once a position is closed."""
...
def resolve_data_path(filename):
    """Resolve a configured filename under the current strategy test directory.

    Args:
        filename: Relative path to the dataset file.
...
def load_backtest_frame(config):
    """Load historical bars and limit by configured from/to timestamps.

    Args:
        config: Strategy configuration with data section.

    Returns:
        A dict containing loaded DataFrame and effective date bounds.
    """
...
def build_cerebro(config, frame):
    ...
    Returns:
        Initialized ``bt.Cerebro`` instance.
    """
...
def extract_metrics(strat, cerebro, frame, config):
    """Extract strategy and analyzer metrics for stable regression assertions.

    Args:
        strat: Finished strategy instance.
        cerebro: Cerebro object after execution.
        frame: Backtest payload used for execution.
        config: Execution configuration.

    Returns:
        Dictionary of performance, PnL and trade outcome statistics.
    """
...
def run(plot=False):
    ...
    Args:
        plot: Toggle plotting after run.
