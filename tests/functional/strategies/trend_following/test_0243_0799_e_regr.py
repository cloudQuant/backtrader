"""Inlined regression test for the e-Regr trend-following channel strategy.
...
Data Used:
    ...
    ``tests/datas/XAUUSD_M15.csv``. The base M15 frame is resampled into two
    feeds: an H1 (60-minute) trading feed and a D1 (1440-minute) protection
    feed. The window spans 2025-12-03 01:15 to 2026-03-10 09:00 with each bar
    timestamp shifted forward by 15 minutes so bars are stamped at their close.

Strategy Principle:
    This is a port of the MT5 expert advisor ``e-Regr``. On the H1 trading feed
    a polynomial regression channel of degree ``regr_degree`` is fitted over the
    last ``regr_bars`` closes, with channel bands set ``regr_kstd`` standard
    deviations from the regression midline. The strategy fades band touches
    (buy at the lower band, sell at the upper band) and exits back toward the
    midline, with an optional trailing stop and fixed stop/take levels. A D1
    protection filter blocks trading when the prior day's range is too large.

Strategy Logic:
    load_backtest_frame loads the base frame; build_cerebro resamples it into
    the H1 and D1 feeds and wires the strategy and analyzers. On each H1 bar the
    strategy computes the regression channel, manages open-position exits
    (stop, target, midline, trailing, and D1 protection), and otherwise opens a
    position within the trade-time window when price pierces a channel band.
    notify_order records entry price and risk levels and counts entries;
    notify_trade tallies win/loss. extract_metrics gathers analyzer output, and
    the test forces runonce=True and asserts each metric against migration-time
    expectations.
"""
...
class ERegrStrategy(bt.Strategy):
    """Polynomial regression-channel strategy fading band touches on the H1 feed."""
    ...
    def __init__(self):
        """Bind the H1/D1 feeds, reset counters, and parse the trade-time window."""
    ...
    def log(self, text):
        """Print a timestamped log line using the current H1 bar datetime.
    ...
    def next(self):
        """Compute the regression channel, manage exits, and fade band touches."""
        self.bar_num += 1
    ...
    def notify_order(self, order):
        """Record entry price/risk levels on fills, count entries, and clear orders.
    ...
    def notify_trade(self, trade):
        """Tally win/loss on close and reset per-position risk state.
...
def resample_frame(df, minutes):
    ...
    Returns:
        A resampled OHLCV DataFrame, dropping bars without price data and
        filling missing open interest with zero.
    """
    rule = f'{minutes}min'
...
def build_cerebro(config, frame):
    """Assemble a Cerebro engine with the H1/D1 feeds, strategy, and analyzers.
