"""Inlined regression test for trend_following/0247_0819_i4_drf_v2.
...
class ExpI4DRFV2Strategy(bt.Strategy):
    ...
    def next(self):
        ...
        if len(self.signal_data) < int(self.p.period) + sb + 3:
            return
...
def test_246_0247_0819_i4_drf_v2() -> None:
    """Migrated regression test for trend_following/0247_0819_i4_drf_v2."""
    ...
    cerebro.addstrategy(
        ExpI4DRFV2Strategy,
        period=11, signal_bar=1, trend_mode="DIRECT",
    ...
    assert strat.bar_num == 5940, f"bar_num: expected=5940, got={strat.bar_num}"
    assert strat.buy_count == 23, f"buy_count: expected=23, got={strat.buy_count}"
    assert strat.sell_count == 24, f"sell_count: expected=24, got={strat.sell_count}"
    assert strat.win_count == 21, f"win_count: expected=21, got={strat.win_count}"
    assert strat.loss_count == 26, f"loss_count: expected=26, got={strat.loss_count}"
    assert strat.trade_count == 47, f"trade_count: expected=47, got={strat.trade_count}"
    assert total_trades == 47, f"total_trades: expected=47, got={total_trades}"
    assert abs(final_value - 999715.9) < 0.01, f"final_value: expected=999715.9, got={final_value}"
