"""Regression test for `tests/functional/strategies/trend_following/regression/0143_1267_color_schaff_trend_cycle` after inline migration.
...
Data Used:
    ...
    - Data pipeline: raw MT5 data is converted into `DataFrame` and consumed by
      both a base feed and the custom Schaff Trend Cycle indicator.

Strategy Principle:
    - Builds a Schaff-style oscillator (`stc`) from fast/slow moving averages.
    - Applies threshold/range coloring rules to classify momentum regime.
    - Generates buy/sell/close signals from color transitions and supports reverse exits.
    - Tracks trade statistics for regression invariants.

Strategy Logic:
    - `load_mt5_csv` loads and normalizes the raw historical data.
    - `ColorSchaffTrendCycleIndicator` calculates oscillator and color lines.
    - `ColorSchaffTrendCycleStrategy` consumes indicator output and routes orders.
...
class Mt5PandasFeed(bt.feeds.PandasData):
    """Base Backtrader feed mapping a normalized MT5 DataFrame to OHLCV lines."""
    params = (
...
class ColorSchaffTrendCycleStrategy(bt.Strategy):
    ...
    def next(self):
        """Run per-bar signal evaluation and place/cross exits or reversals."""
        self.bar_num += 1
    ...
    def notify_trade(self, trade):
        """Count open/close trade transitions and win/loss outcomes."""
...
def resolve_data_path(filename):
    """Resolve and validate the test CSV location.

    Args:
        filename: Relative file path as configured in `_CONFIG['data']['file']`.
...
def load_backtest_frame(config):
    """Build backtest input frame including parsed date boundaries.

    Args:
        config: Strategy configuration dictionary.

    Returns:
        dict: Contains normalized data and `fromdate`/`todate`.
    """
...
def build_cerebro(config, frame):
    """Configure Cerebro with feed, strategy, and analyzers.

    Args:
        config: Strategy configuration dictionary.
        frame: Backtest frame with market data.

    Returns:
        bt.Cerebro: Execution engine.
    """
...
def extract_metrics(strat, cerebro, frame, config):
    """Build regression metrics dictionary from analyzer output.

    Args:
        strat: Completed strategy.
        cerebro: Cerebro object used in this run.
        frame: Backtest frame with input metadata.
        config: Configuration including initial cash.

    Returns:
        dict: Metrics used by `test_143_0143_1267_color_schaff_trend_cycle`.
    """
...
def run(plot=False):
    ...
    Args:
        plot: If True, invoke `cerebro.plot()`.
