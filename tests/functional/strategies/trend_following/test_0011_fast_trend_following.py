"""Inlined regression test for tests/functional/strategies/trend_following/regression/0011_fast_trend_following.
...
Strategy Principle:
    - This is a trend-following crossover strategy named "Fast Trend Following".
    - Market Assumptions: Markets establish persistent historical trends. When the short-term moving average (50 SMA) crosses above the long-term moving average (100 SMA), it signals a shift in momentum to a strong upward trend, which can be captured by entering long.
    - Indicators:
        - ma_fast: rolling 50-day Simple Moving bt.indicators.Average.
        - ma_slow: rolling 100-day Simple Moving bt.indicators.Average.
...
_CONFIG = {
    'strategy': {
        ...
        'source_spec': 'research_papers_gold/strategy_specs/trend_following/0404_Fast_Trend_Following_Strategy.md',
    },
    'data': {
        'symbol': 'XAUUSD',
        'timeframe': 'D1',
...
def prepare_fast_trend_features(df, params):
    """Prepare and compute indicators for the Fast Trend Following strategy.
    ...
    # Golden Cross: fast line crosses above the slow line
    out['golden_cross'] = ((out['ma_fast'].shift(1) <= out['ma_slow'].shift(1)) & 
                           (out['ma_fast'] > out['ma_slow'])).astype(float)
    
    # Death Cross: fast line crosses below the slow line
...
class Mt5FastTrendFeed(bt.feeds.PandasData):
    """Custom backtrader Pandas data feed with Fast Trend lines."""
...
class FastTrendFollowingStrategy(bt.Strategy):
    """Strategy class implementing the Fast Trend Following crossover logic.
...
#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Fast Trend Following strategy backtest."""
...
def load_data(config):
    """Load daily market data and prepare Fast Trend Following strategy features.
...
def test_11_0011_fast_trend_following() -> None:
    """Migrated regression test (runonce=True only).

    Originally located at tests/functional/strategies_regression/trend_following/0011_fast_trend_following.
    """
    ...
    _close(metrics.get('final_value'), 3677303.57228, tol=3.677304e+00, key='final_value')
    _close(metrics.get('net_pnl'), 2677303.57228, tol=2.677304e+00, key='net_pnl')
    _close(metrics.get('total_return_pct'), 267.730357228, tol=2.677304e-04, key='total_return_pct')
    _close(metrics.get('win_rate'), 52.0, tol=5.200000e-05, key='win_rate')
    _close(metrics.get('profit_factor'), 1.832195748007377, tol=1.832196e-06, key='profit_factor')
    _close(metrics.get('max_drawdown'), 39.23231412068499, tol=3.923231e-05, key='max_drawdown')
    _close(metrics.get('sharpe_ratio'), 0.6360516615591444, tol=1.000000e-06, key='sharpe_ratio')
    _close(metrics.get('annual_return_pct'), 7.497295748907727, tol=7.497296e-06, key='annual_return_pct')
    _close(metrics.get('sqn'), 1.197365331029939, tol=1.197365e-06, key='sqn')
    _close(metrics.get('ulcer_index'), 22.89015732091878, tol=2.289016e-05, key='ulcer_index')
