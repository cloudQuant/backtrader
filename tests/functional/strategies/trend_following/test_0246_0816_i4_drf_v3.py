"""Inlined regression test for trend_following/0246_0816_i4_drf_v3.
...
class ExpI4DRFV3Strategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.ind = bt.indicators.I4DRFV3(self.signal_data, period=self.p.period)
        self.signal_count = 0
        self.trade_count = 0
        self.buy_count = 0
        self.sell_count = 0
...
def test_245_0246_0816_i4_drf_v3() -> None:
    """Migrated regression test for trend_following/0246_0816_i4_drf_v3."""
    ...
    assert strat.sell_count == 22, f"sell_count: expected=22, got={strat.sell_count}"
    assert strat.win_count == 16, f"win_count: expected=16, got={strat.win_count}"
    assert strat.loss_count == 27, f"loss_count: expected=27, got={strat.loss_count}"
    assert strat.trade_count == 43, f"trade_count: expected=43, got={strat.trade_count}"
    assert total_trades == 43, f"total_trades: expected=43, got={total_trades}"
    assert abs(final_value - 998740.6) < 0.01, f"final_value: expected=998740.6, got={final_value}"
