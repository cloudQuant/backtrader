...
Data Used:
    ...
    - Primary timeframe is M15 with indicator timeframe H1 generated by resampling.
    - Date window is 2025-12-03 01:15:00 to 2026-03-10 09:00:00.
    - A 15-minute bar shift is applied when loading the source file.

Strategy Principle:
    - Uses NRTR_extr indicator to generate sign-up / sign-down arrows and trend
      channel levels from high/low price structure.
    - Buys on bullish signal arrows, sells on bearish arrows, and applies symmetric
      stop-loss / take-profit controls.

Strategy Logic:
    - build_signal_frame converts M15 bars to H1 for indicator data feed.
    - build_cerebro feeds both base and signal data, strategy parameters, and analyzers.
    - next() deduplicates repeated signals, enforces close checks, and executes
      new entries.
    - notify_trade() tracks counts and win/loss stats.
...
class Mt5PandasFeed(bt.feeds.PandasData):
    """Minimal pandas feed mapping for MT5 OHLCV data."""
...
class ExpNRTRExtrStrategy(bt.Strategy):
    ...
    def next(self):
        """Apply signal filtering, closing rules, and entry rules for each bar."""
        self.bar_num += 1
        if len(self.base) < 2:
            return
    ...
    def notify_trade(self, trade):
        """Update trade counters as positions are opened and closed."""
...
def resolve_data_path(fn):
    ...
    Args:
        fn: Relative filename configured in test cfg.
...
def load_backtest_frame(cfg):
    """Load and validate a bounded M15 dataframe for backtesting.

    Args:
        cfg: Inline merged test configuration.

    Returns:
        Dict containing dataframe and resolved date boundaries.
    """
    dc = cfg['data']
...
def build_signal_frame(df, mins):
    """Resample minute bars to a higher timeframe for indicator generation.

    Args:
        df: Base minute dataframe.
        mins: Resample interval in minutes.

    Returns:
        Resampled dataframe used by the signal feed.
    """
    r = f'{int(mins)}min'
...
def build_cerebro(cfg, frame):
    """Create configured Backtrader engine and attach analyzer stack.

    Args:
        cfg: Inline merged config.
        frame: Loaded base dataframe frame.
...
def extract_metrics(strat, cerebro, frame, cfg):
    """Collect strategy counters and analyzer metrics for validation.

    Args:
        strat: Executed strategy object.
        cerebro: Cerebro engine after run.
        frame: Backtest frame with base data.
        cfg: Inline merged config.

    Returns:
        Dict of metrics for regression assertions.
    """
