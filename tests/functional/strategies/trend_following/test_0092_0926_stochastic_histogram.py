"""Regression test for tests/functional/strategies/trend_following/regression/0092_0926_stochastic_histogram.
...
Data Used:
    - XAUUSD 15-minute OHLCV data from ``tests/datas/XAUUSD_M15.csv``.
    - The same base bars are used for execution feed, and a resampled higher
      timeframe signal feed (H4 by default) is generated from this data.
    - Testing range is constrained by ``fromdate``/``todate`` from inline config.

Strategy Principle:
    - Build a stochastic indicator signal on the higher timeframe and derive trend
      context by comparing the stochastic main and signal lines.
    - When bars cross threshold colors or moving average lines and position rules
      allow, the strategy opens or closes long/short positions.
    - A fixed-size position model with optional stop-loss and take-profit protects
      each trade.

Strategy Logic:
    - Load and normalize MT5-formatted data, then build base and signal feeds.
    - Run a Backtrader `Cerebro` instance with analyzers.
    - During each bar, evaluate exits first, then open/close actions from signal
      changes.
    - Collect trade and risk metrics and compare them against expected
...
class Mt5PandasFeed(bt.feeds.PandasData):
    """PandasData adapter mapping cleaned MT5 column order into Backtrader fields."""
    params = (
        ('datetime', None),
        ('open', 0),
        ('high', 1),
...
class ExpStochasticHistogramStrategy(bt.Strategy):
    """Backtrader strategy implementation of the Exp Stochastic Histogram approach.

    The strategy listens to both a base execution feed and a higher-timeframe
    signal feed, generates signals from stochastic state transitions or color
    transitions, enforces position close rules, and records execution counters
    ...
    def log(self, text):
        """Write a timestamped strategy log line to stdout.

        Args:
            text: Human-readable message for debugging signal or trade events.
        """
    ...
    def next(self):
        """Handle bar-by-bar signal evaluation, exits, and order submissions."""
        self.bar_num += 1
        if len(self.base) < 2:
            return
    ...
    def notify_trade(self, trade):
        """Update trade statistics when positions open and close."""
...
def resolve_data_path(filename):
    """Resolve a strategy data filename relative to the test module directory.

    Args:
        filename (str): Path configured in the inline config, e.g.
            ``{repo}/tests/datas/XAUUSD_M15.csv``.
...
def load_backtest_frame(config):
    """Load and slice the backtest frame according to config date bounds.

    Args:
        config (dict): Inline test configuration holding ``data`` section.

    Returns:
        dict: Dictionary with parsed DataFrame and effective datetime bounds.
    """
...
def build_signal_frame(df, indicator_minutes):
    """Resample the base frame into a compact signal feed timeframe.

    Args:
        df (pandas.DataFrame): Base OHLCV DataFrame indexed by datetime.
        indicator_minutes (int): Target indicator timeframe in minutes.

    Returns:
        pandas.DataFrame: Aggregated DataFrame using first/high/max/low/last style.
    """
...
def build_cerebro(config, frame):
    """Build the Backtrader engine, add data feeds, strategy, and analyzers.

    Args:
        config (dict): Inline config containing strategy/backtest parameters.
        frame (dict): Backtest frame produced by ``load_backtest_frame``.

    Returns:
        bt.Cerebro: Fully configured Cerebro instance ready for ``run()``.
    """
...
def extract_metrics(strat, cerebro, frame, config):
    """Collect summary metrics from strategy analyzers and execution counters.

    Args:
        strat (bt.Strategy): Strategy instance after ``run()``.
        cerebro (bt.Cerebro): Cerebro instance that executed the backtest.
        frame (dict): Backtest data frame payload used for sizing/bounds.
        config (dict): Inline config with backtest metadata.

    Returns:
        dict: Metric snapshot used by regression assertions.
    """
...
def run(plot=False):
    ...
    Args:
        plot (bool): If ``True``, invoke ``cerebro.plot()`` after execution.
