"""Inlined regression test for tests/functional/strategies/price_patterns/regression/0015_1204_heiken_ashi.
...
Strategy Principle:
    - This strategy implements a trend-following system based on Heiken Ashi candle color changes.
    - Market Assumptions: Heiken Ashi chart modifications smooth out price noise, making trend regimes easier to identify. A change in the candle color (bullish to bearish or vice-versa) signals a high probability momentum shift.
    - Calculations:
        - HA_Close = (Open + High + Low + Close) / 4.0.
        - HA_Open = (prev_HA_Open + prev_HA_Close) / 2.0.
    - Entry Signals:
        - Buy Entry (bullish shift): Current HA_Close > HA_Open, while previous candle was bearish (HA_Close <= HA_Open).
        - Sell Entry (bearish shift): Current HA_Close < HA_Open, while previous candle was bullish (HA_Close >= HA_Open).
    - Exit Signals:
        - Opposing Signal Exit: Close any open positions immediately when a reverse trend shift signal occurs.
"""
...
_CONFIG = {
    'strategy': {
        'name': 'Heiken Ashi Color Change',
        'source_ea': 'ea/1204_Heiken_Ashi_Based_EA/heiken_ashi_based_ea.mq5',
    },
    'data': {
        'symbol': 'XAUUSD',
        'timeframe': 'M15',
...
class HeikenAshiStrategy(bt.Strategy):
    """Strategy class implementing Heiken Ashi smoothed candlestick color change momentum.
...
def test_15_0015_1204_heiken_ashi() -> None:
    """Migrated regression test (runonce=True only).

    Originally located at tests/functional/strategies_regression/price_patterns/0015_1204_heiken_ashi.
    """
    ...
    assert metrics.get('buy_count') == 760, f"buy_count: expected=760, got={metrics.get('buy_count')!r}"
    assert metrics.get('sell_count') == 760, f"sell_count: expected=760, got={metrics.get('sell_count')!r}"
    assert metrics.get('win_count') == 519, f"win_count: expected=519, got={metrics.get('win_count')!r}"
    assert metrics.get('loss_count') == 1000, f"loss_count: expected=1000, got={metrics.get('loss_count')!r}"
    assert metrics.get('total_trades') == 1520, f"total_trades: expected=1520, got={metrics.get('total_trades')!r}"
    assert metrics.get('trade_count') == 1519, f"trade_count: expected=1519, got={metrics.get('trade_count')!r}"
    assert metrics.get('won') == 519, f"won: expected=519, got={metrics.get('won')!r}"
    assert metrics.get('lost') == 1000, f"lost: expected=1000, got={metrics.get('lost')!r}"
    _close(metrics.get('bars'), 6129.0, tol=6.129000e-03, key='bars')
    _close(metrics.get('initial_cash'), 1000000.0, tol=1.000000e+00, key='initial_cash')
    _close(metrics.get('final_value'), 999417.2999999995, tol=9.994173e-01, key='final_value')
    _close(metrics.get('net_pnl'), -582.7000000005355, tol=5.827000e-04, key='net_pnl')
    _close(metrics.get('total_return_pct'), -0.058270000000049116, tol=1.000000e-06, key='total_return_pct')
    _close(metrics.get('win_rate'), 34.14473684210526, tol=3.414474e-05, key='win_rate')
    _close(metrics.get('profit_factor'), 0.9916048310284887, tol=1.000000e-06, key='profit_factor')
    _close(metrics.get('max_drawdown'), 0.8406362219497617, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('sharpe_ratio'), -0.472763575530141, tol=1.000000e-06, key='sharpe_ratio')
    _close(metrics.get('annual_return_pct'), -3.3921323662911895, tol=3.392132e-06, key='annual_return_pct')
    _close(metrics.get('sqn'), -0.08491959923463521, tol=1.000000e-06, key='sqn')
