...
class NCandlesV3Strategy(bt.Strategy):
    params = dict(
        n_candles=3,
        lot=0.01,
        take_profit=50,
    ...
    def _layers(self):
        if not self.position:
            return 0
        return int(round(abs(float(self.position.size)) / float(self.p.lot)))
    ...
    def _trail(self):
        ...
        if self.position.size > 0:
            ...
            if current - entry > ts and (self.stop_price is None or new_sl > float(self.stop_price) + step):
                self.stop_price = new_sl
        else:
            new_sl = self._round(current + ts)
            if entry - current > ts and (self.stop_price is None or new_sl < float(self.stop_price) - step):
                self.stop_price = new_sl
