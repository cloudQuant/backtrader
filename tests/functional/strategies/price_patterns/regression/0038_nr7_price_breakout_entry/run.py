...
from strategy_nr7_price_breakout_entry import (
    Mt5NR7PriceBreakoutFeed,
    NR7PriceBreakoutEntryStrategy,
    load_mt5_csv,
    prepare_nr7_price_breakout_features,
)
...
def load_data(config):
    ...
    df = prepare_nr7_price_breakout_features(raw, config.get('params', {}))
...
def build_cerebro(frame, config):
    ...
    feed = Mt5NR7PriceBreakoutFeed(dataname=frame['data'], timeframe=bt.TimeFrame.Days, compression=1)
    cerebro.adddata(feed, name=f"{config['data']['symbol']}_{config['data']['timeframe']}")
    cerebro.addstrategy(NR7PriceBreakoutEntryStrategy, **config.get('params', {}))
