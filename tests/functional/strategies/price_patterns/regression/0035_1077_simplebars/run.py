...
from strategy_simplebars import Mt5PandasFeed, SimpleBarsFeed, SimpleBarsStrategy, compute_simplebars, load_mt5_csv, resample_frame
...
def load_backtest_frames(config):
    ...
    signal_df = compute_simplebars(signal_df, period=params['rperiod'], useclose=params['useclose'])
...
def build_cerebro(config, frame):
    ...
    feed_signal = SimpleBarsFeed(dataname=frame['signal'][['open', 'high', 'low', 'close', 'volume', 'openinterest', 'color']], timeframe=bt.TimeFrame.Minutes, compression=signal_minutes)
    cerebro.adddata(feed_m15, name='XAUUSD_M15')
    cerebro.adddata(feed_signal, name='XAUUSD_H6_SIMPLEBARS')
    cerebro.addstrategy(SimpleBarsStrategy, **config.get('params', {}))
...
def print_report(metrics):
    print('\n' + '=' * 60 + '\nBACKTEST RESULTS — Exp_SimpleBars\n' + '=' * 60)
    for key in [
