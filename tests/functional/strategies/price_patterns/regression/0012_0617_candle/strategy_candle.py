...
class CandleStrategy(bt.Strategy):
    ...
    def _is_bullish(self):
        return float(self.data.close[0]) > float(self.data.open[0])
    ...
    def _set_risk(self, side, price):
        ...
        if side == 'buy':
            self.stop_price = self._round(price - trail) if trail > 0 else None
            self.take_profit_price = self._round(price + tp) if tp > 0 else None
        else:
            self.stop_price = self._round(price + trail) if trail > 0 else None
            self.take_profit_price = self._round(price - tp) if tp > 0 else None

    def _maybe_open(self, side):
    ...
    def _manage_position(self):
        ...
        if self.position.size > 0:
            if self._is_bearish():
                ...
                return
            if trail > 0 and price - float(self.position.price) > trail:
                new_sl = self._round(price - trail)
            ...
            if self._is_bullish():
                ...
                self.order = self.close()
                return
            if trail > 0 and float(self.position.price) - price > trail:
                new_sl = self._round(price + trail)
                if self.stop_price is None or new_sl < float(self.stop_price):
                    self.stop_price = new_sl
    ...
    def next(self):
        self.bar_num += 1
        if len(self) <= 2 * int(self.p.min_bars):
            return
        ...
        if self._is_bullish():
            self._maybe_open('buy')
        elif self._is_bearish():
            self._maybe_open('sell')
    ...
    def notify_order(self, order):
        ...
        if order.status == bt.Order.Completed:
            ...
            if self.position:
                ...
                self.pending_reentry = None
                if pending_reentry and self.order is None:
                    self._maybe_open(pending_reentry)
