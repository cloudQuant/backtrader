...
from strategy_exchange_price import (
    ExchangePriceFeed,
    ExchangePriceStrategy,
    Mt5PandasFeed,
...
def load_backtest_frames(config):
    ...
    signal_df = resample_frame(full_base, f'{signal_minutes}min')
    signal_df = compute_exchange_price(
        signal_df,
        count_bars_s=params['count_bars_s'],
        count_bars_l=params['count_bars_l'],
...
def build_cerebro(config, frame):
    ...
    feed_signal = ExchangePriceFeed(dataname=sig_df, timeframe=bt.TimeFrame.Minutes, compression=signal_minutes)
    cerebro.adddata(feed_m15, name='XAUUSD_M15')
    cerebro.adddata(feed_signal, name='XAUUSD_H8_EXCHANGE_PRICE')
    cerebro.addstrategy(ExchangePriceStrategy, **config.get('params', {}))
...
def print_report(metrics):
    print('\n' + '=' * 60 + '\nBACKTEST RESULTS — Exp_ExchangePrice\n' + '=' * 60)
    for key in [
