...
def compute_exchange_price(frame, count_bars_s=96, count_bars_l=288, point=0.01):
    series = frame['close'].astype(float)
    out = frame.copy()
    out['ind'] = (series - series.shift(int(count_bars_s))) / float(point)
    out['sign'] = (series - series.shift(int(count_bars_l))) / float(point)
...
class ExchangePriceFeed(bt.feeds.PandasData):
    lines = ('ind', 'sign')
    params = (
...
class ExchangePriceStrategy(bt.Strategy):
    params = dict(
        mm=0.1,
        mm_mode='LOT',
        stop_loss=1000,
    ...
    def __init__(self):
        ...
        self.min_signal_bars = max(int(self.p.count_bars_s), int(self.p.count_bars_l)) + max(int(self.p.signal_bar), 1) + 5
    ...
    def next(self):
        ...
        self.log('exchange_price ind_prev={0:.5f} sign_prev={1:.5f} ind_curr={2:.5f} sign_curr={3:.5f} buy_open={4} sell_open={5}'.format(ind_prev, sign_prev, ind_curr, sign_curr, buy_open, sell_open))
