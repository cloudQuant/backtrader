"""Inlined regression test for tests/functional/strategies/price_patterns/regression/0038_nr7_price_breakout_entry.
...
_CONFIG = {
    'strategy': {
        'id': '0084',
        'name': 'NR7 Price Breakout Entry',
        'source_spec': 'research_papers_gold/strategy_specs/technical_indicators/0084_NR7_Price_Breakout_Entry.md',
    },
    'data': {
        'symbol': 'XAUUSD',
        'timeframe': 'D1',
...
def test_38_0038_nr7_price_breakout_entry() -> None:
    """Migrated regression test (runonce=True only).

    Originally located at tests/functional/strategies_regression/price_patterns/0038_nr7_price_breakout_entry.
    """
    ...
    assert metrics.get('buy_count') == 132, f"buy_count: expected=132, got={metrics.get('buy_count')!r}"
    ...
    assert metrics.get('won') == 107, f"won: expected=107, got={metrics.get('won')!r}"
    ...
    _close(metrics.get('win_rate'), 46.724890829694324, tol=4.672489e-05, key='win_rate')
    _close(metrics.get('profit_factor'), 0.8249209262528656, tol=1.000000e-06, key='profit_factor')
    _close(metrics.get('final_value'), 676555.0075939989, tol=6.765550e-01, key='final_value')
    _close(metrics.get('net_pnl'), -323444.9924060011, tol=3.234450e-01, key='net_pnl')
    _close(metrics.get('total_return_pct'), -32.34449924060011, tol=3.234450e-05, key='total_return_pct')
    _close(metrics.get('annual_return_pct'), -2.122858357646546, tol=2.122858e-06, key='annual_return_pct')
    _close(metrics.get('max_drawdown'), 42.98557214804522, tol=4.298557e-05, key='max_drawdown')
    _close(metrics.get('sharpe_ratio'), -0.21138854811596253, tol=1.000000e-06, key='sharpe_ratio')
    _close(metrics.get('sqn'), -1.095469144585298, tol=1.095469e-06, key='sqn')
    _close(metrics.get('ulcer_index'), 22.70086748159583, tol=2.270087e-05, key='ulcer_index')
