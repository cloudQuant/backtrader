...
Strategy Principle:
    - This strategy ("Harami + RSI") implements a reversal strategy using Japanese Bullish and Bearish Harami candlestick patterns, filtered by the Relative Strength Index (RSI).
    - Market Assumptions: A Harami pattern represents a potential trend reversal when a small body candle is fully contained within the large body of the preceding candle. These patterns are verified by overbought or oversold RSI filters.
    - Indicators:
        - SMA: Simple Moving Average (7-period, `ma_period`) of close prices.
        - RSI: Relative Strength Index (37-period, `rsi_period`).
    - Entry Signals:
        - Buy Entry (Bullish Harami): A Bullish Harami forms (previous candle is bearish and large, current candle is bullish and small, completely within the previous candle's body) below the SMA, and previous RSI < 40 (`rsi_entry_long`).
        - Sell Entry (Bearish Harami): A Bearish Harami forms (previous candle is bullish and large, current candle is bearish and small, completely within the previous candle's body) above the SMA, and previous RSI > 60 (`rsi_entry_short`).
    - Exit Signals:
        - Long Exit: RSI crosses up through 70 (`rsi_exit_upper`) or down through 30 (`rsi_exit_lower`), or a reverse bearish harami forms.
        - Short Exit: RSI crosses down through 30 (`rsi_exit_lower`) or up through 70 (`rsi_exit_upper`), or a reverse bullish harami forms.
"""
...
_CONFIG = {
    'strategy': {
        'name': 'Harami + RSI',
        'source_ea': 'ea/1335_Harami_RSI/expert_abh_bh_rsi.mq5',
    },
    'data': {
        'symbol': 'XAUUSD',
        'timeframe': 'M15',
...
class HaramiRSIStrategy(bt.Strategy):
    """Strategy class implementing Harami pattern recognition with RSI filtering.
