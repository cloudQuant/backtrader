...
class MorningStarCciStrategy(bt.Strategy):
    """Morning/Evening Star + CCI confirmation."""
    params = dict(
        cci_period=25, cci_entry_long=-50, cci_entry_short=50,
        cci_exit_upper=80, cci_exit_lower=-80,
        ma_period=5, lot=0.1, point=0.01, price_digits=2,
    )
    ...
    def _is_morning_doji(self):
        ...
        if avg < self.p.point:
            return False
        return ((o3 - c3) > avg and abs(c2 - o2) < avg * 0.1 and
    ...
    def _is_evening_doji(self):
        ...
        if avg < self.p.point:
            return False
        return ((c3 - o3) > avg and abs(c2 - o2) < avg * 0.1 and
                c2 > c3 and o2 > o3 and o1 < c2 and c1 < c2)
    ...
    def next(self):
        ...
        if self.position:
            ...
            if (self._is_morning_star() or self._is_morning_doji()) and cci1 < self.p.cci_entry_long:
                self.buy(size=self.p.lot)
                return
            if (self._is_evening_star() or self._is_evening_doji()) and cci1 > self.p.cci_entry_short:
...
def test_018_0019_1318_morningstar_cci() -> None:
    """Migrated regression test for price_patterns/0019_1318_morningstar_cci."""
    ...
    cerebro.addstrategy(MorningStarCciStrategy)
    ...
    assert strat.trade_count == 28
    assert total_trades == 28
    assert abs(final_value - 1002677.7) < 0.01
