...
Data Used:
    ...
    covering 2008-01-01 to 2025-12-31 (no bar shift). Volatility, ATR, and the
    walk-forward optimized signal columns are precomputed and exposed as feed
    lines.

Strategy Principle:
    The strategy uses walk-forward optimization to avoid overfitting. Over
    rolling train/test windows it grid-searches combinations of momentum and
    mean-reversion lookbacks and a signal threshold, selecting the combination
    with the best in-sample Sharpe ratio and applying it out of sample. The
    composite score blends momentum return and mean-reversion edge, sized
    toward a target volatility with an ATR-based stop.

Strategy Logic:
    Walk-forward feature engineering and the out-of-sample signal series are
    precomputed and exposed as feed lines. On each bar, while flat, a positive
    or negative signal opens a long or short sized by target percentage and
    records the entry ATR; while in a position the strategy exits on an
    ATR-multiple stop, an opposite signal, or a flat signal. Order and trade
    notifications maintain entry state and win/loss counts, while the broker
...
def prepare_walk_forward_features(df, params):
    """Compute volatility, ATR, and volatility-scaled target-percent features.

    Args:
        df: Source OHLCV DataFrame indexed by datetime.
        params: Strategy parameter mapping controlling windows and targets.

    Returns:
        A copy of the frame with daily return, annualized volatility, ATR,
        ATR percentage, and target-percent columns added.
    """
    out = df.copy()
...
def run_walk_forward(feature_df, params):
    """Run rolling walk-forward optimization to build the out-of-sample signal.

    For each train/test window, grid-searches momentum/mean-reversion lookbacks
    and signal thresholds, selects the best in-sample Sharpe combination, and
    applies it to the subsequent out-of-sample window.

    Args:
        feature_df: Feature DataFrame produced by prepare_walk_forward_features.
        params: Strategy parameter mapping controlling windows and the grid.

    Returns:
        A tuple of (oos_df, wf_summary) where oos_df carries the concatenated
        out-of-sample bars with chosen signal columns and wf_summary records
        the selected parameters and train Sharpe per window.
    """
...
class Mt5WalkForwardFeed(bt.feeds.PandasData):
    """PandasData feed exposing walk-forward feature and signal lines."""
...
class GoldWalkForwardStrategy(bt.Strategy):
    """Trade walk-forward optimized signals with ATR stops and reversals."""
    ...
    def next(self):
        """Open walk-forward signal trades and manage ATR-stop/reverse exits."""
        self.bar_num += 1
...
def load_inputs(config):
    """Load source data, build features, and run walk-forward optimization.

    Args:
        config: Resolved configuration mapping.

    Returns:
        A dict with the out-of-sample DataFrame, walk-forward summary,
        fromdate/todate bounds, and resolved strategy params.
    """
...
def build_cerebro(config, inputs):
    """Construct a configured cerebro with the walk-forward feed and analyzers."""
...
def extract_metrics(strat, cerebro, inputs, config):
    """Collect analyzer results, walk-forward stats, and counters into a dict."""
