...
from strategy_macro_fx import MacroFXFeed, MacroFXStrategy, load_mt5_csv, prepare_macro_fx_data
...
def upsert_global_summary(config, metrics):
    ...
    rows.append({
        ...
        'symbol': ','.join((config['data'].get('pairs') or {}).keys()),
...
def print_results(metrics):
    ...
    print(f"  Neutral signal days: {metrics['neutral_signal_days']}")
...
def main():
    ...
    print(f"Loaded macro FX bars: {len(inputs['score_df'])}")
