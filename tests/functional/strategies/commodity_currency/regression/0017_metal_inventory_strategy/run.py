...
from strategy_metal_inventory import InventoryScoreFeed, MetalInventoryStrategy, load_mt5_csv, prepare_inventory_proxy_data
...
def upsert_global_summary(config, metrics):
    ...
    rows.append({
        ...
        'symbol': ','.join((config['data'].get('metals') or {}).keys()),
...
def main():
    ...
    print(f"Loaded metal inventory proxy bars: {len(inputs['score_df'])}")
