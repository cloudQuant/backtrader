...
from strategy_commodity_skewness import CommoditySkewnessFeed, CommoditySkewnessStrategy, load_mt5_csv, prepare_skewness_data
...
def upsert_global_summary(config, metrics):
    ...
    rows.append({
        ...
        'symbol': ','.join((config['data'].get('commodities') or {}).keys()),
...
def print_results(metrics):
    ...
    print(f"  Long books:          {metrics['long_books']}")
    print(f"  Short books:         {metrics['short_books']}")
...
def main():
    ...
    print(f"Loaded commodity skewness bars: {len(inputs['score_df'])}")
