...
from strategy_distance_pairs_trading import (
    Mt5DistancePairsTradingFeed, DistancePairsTradingStrategy, load_mt5_csv, prepare_distance_pairs_trading_features,
)
...
def load_data(config):
    ...
    df = prepare_distance_pairs_trading_features(raw, config.get('params', {}))
...
def build_cerebro(frame, config):
    ...
    feed = Mt5DistancePairsTradingFeed(dataname=frame['data'], timeframe=bt.TimeFrame.Days, compression=1)
    cerebro.adddata(feed, name=f"{config['data']['symbol']}_{config['data']['timeframe']}")
    cerebro.addstrategy(DistancePairsTradingStrategy, **config.get('params', {}))
