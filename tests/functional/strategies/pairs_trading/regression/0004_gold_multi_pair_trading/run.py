...
from strategy_gold_multi_pair_trading import (
    GoldMultiPairTradingStrategy,
    MultiPairSignalFeed,
    PAIR_SYMBOLS,
    load_mt5_csv,
    prepare_multi_pair_inputs,
)
...
def upsert_global_summary(config, metrics):
    ...
    rows.append({
        ...
        'symbol': f"{config['data']['gold_symbol']}/" + '/'.join(config['data']['pair_symbols']),
