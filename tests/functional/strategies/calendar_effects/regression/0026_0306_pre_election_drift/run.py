...
from strategy_pre_election_drift import (
    Mt5PreElectionDriftFeed, PreElectionDriftStrategy, load_mt5_csv, prepare_pre_election_drift_features,
)
...
def load_data(config):
    ...
    df = prepare_pre_election_drift_features(raw, config.get('params', {}))
    if df.empty:
...
def build_cerebro(frame, config):
    ...
    feed = Mt5PreElectionDriftFeed(dataname=frame['data'], timeframe=bt.TimeFrame.Days, compression=1)
    cerebro.adddata(feed, name=f"{config['data']['symbol']}_{config['data']['timeframe']}")
    cerebro.addstrategy(PreElectionDriftStrategy, **config.get('params', {}))
