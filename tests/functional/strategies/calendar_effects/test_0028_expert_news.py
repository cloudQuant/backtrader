"""Inlined regression test for the Expert_NEWS straddle breakout strategy.
...
Data Used:
    ...
    single 15-minute feed is used; pending orders and risk levels are managed
    internally.

Strategy Principle:
    This is a port of the MT5 expert advisor Expert_NEWS, a news-style straddle.
    It assumes a sharp directional move can break out in either direction, so it
    brackets price with virtual buy-stop and sell-stop orders a fixed step away.
    Whichever side is hit first triggers a market entry in that direction, with
    a fixed stop loss and take profit, optional break-even protection, and a
    trailing stop. The straddle is continuously re-centred on price until filled.

Strategy Logic:
    Each bar, while flat, the strategy maintains and periodically re-prices the
    virtual buy/sell stop orders and checks whether the intrabar range triggers
    either side, opening a market position (or reversing an existing one). While
    in a position it manages stop-loss, take-profit, break-even, and trailing
    logic via close orders. Order notifications resolve fills/closes, update risk
    prices, and re-open after a reversal; trade notifications tally win/loss
    counts. build_cerebro wires the feed, commission, strategy, and analyzers;
    extract_metrics gathers return, drawdown, Sharpe, SQN, and trade statistics;
    and the test hooks extract_metrics while forcing runonce=True to assert each
...
class ExpertNewsStrategy(bt.Strategy):
    """News-style straddle: brackets price with virtual stops and trails fills."""
    ...
    def __init__(self):
        """Initialize counters, order/risk state, and virtual stop trackers."""
        self.bar_num = 0
        self.buy_count = 0
        self.sell_count = 0
        self.trade_count = 0
    ...
    def next(self):
        """Manage open positions, refresh the straddle, and check triggers."""
        self.bar_num += 1
        if self.order is not None:
            return
    ...
    def notify_order(self, order):
        """Resolve fills/closes, update risk state, and re-open after reversal.
...
def build_cerebro(config, frame):
    ...
    Returns:
        A configured Cerebro instance ready to run the Expert_NEWS strategy.
    """
