...
from strategy_extreme_gain import (
    Mt5ExtremeGainFeed, ExtremeShortTermGainStrategy, load_mt5_csv, prepare_extreme_gain_features,
)
...
def load_data(config):
    ...
    df = prepare_extreme_gain_features(raw, config.get('params', {}))
...
def build_cerebro(frame, config):
    ...
    feed = Mt5ExtremeGainFeed(dataname=frame['data'], timeframe=bt.TimeFrame.Days, compression=1)
    cerebro.adddata(feed, name=f"{config['data']['symbol']}_{config['data']['timeframe']}")
    cerebro.addstrategy(ExtremeShortTermGainStrategy, **config.get('params', {}))
