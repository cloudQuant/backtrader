"""Inlined regression test for tests/functional/strategies/machine_learning/regression/0019_1154_artificial_intelligence.
...
_CONFIG = {
    'strategy': {
        ...
        'source_ea': 'ea/1154_ArtificialIntelligence',
    },
    'data': {
        'symbol': 'XAUUSD',
        'timeframe': 'M15',
...
class ArtificialIntelligenceStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.ac = bt.indicators.AIAcceleratorOscillator(self.data)
...
def test_19_0019_1154_artificial_intelligence() -> None:
    """Migrated regression test (runonce=True only).

    Originally located at tests/functional/strategies_regression/machine_learning/0019_1154_artificial_intelligence.
    """
    ...
    _close(metrics.get('signal_count'), 6068.0, tol=6.068000e-03, key='signal_count')
    _close(metrics.get('initial_cash'), 1000000.0, tol=1.000000e+00, key='initial_cash')
    _close(metrics.get('final_cash'), 990380.9989999952, tol=9.903810e-01, key='final_cash')
    _close(metrics.get('final_value'), 990380.9999999952, tol=9.903810e-01, key='final_value')
    _close(metrics.get('net_pnl'), -9619.000000004773, tol=9.619000e-03, key='net_pnl')
    _close(metrics.get('total_return_pct'), -0.9619000000004818, tol=1.000000e-06, key='total_return_pct')
    _close(metrics.get('win_rate'), 25.0, tol=2.500000e-05, key='win_rate')
    _close(metrics.get('profit_factor'), 0.4074702886247576, tol=1.000000e-06, key='profit_factor')
    _close(metrics.get('open_position_size'), -0.1, tol=1.000000e-06, key='open_position_size')
    _close(metrics.get('open_position_price'), 4227.4, tol=4.227400e-03, key='open_position_price')
    _close(metrics.get('max_drawdown'), 1.4108409007215585, tol=1.410841e-06, key='max_drawdown')
    _close(metrics.get('sharpe_ratio'), -8.303135113523918, tol=8.303135e-06, key='sharpe_ratio')
    _close(metrics.get('annual_return_pct'), -43.57564883456604, tol=4.357565e-05, key='annual_return_pct')
    _close(metrics.get('sqn'), -0.7935412338471147, tol=1.000000e-06, key='sqn')
