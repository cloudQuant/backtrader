...
from strategy_crypto_optimal_allocation import CryptoOptimalAllocationStrategy, load_mt5_csv, prepare_allocation_inputs, _optimize_weights
...
def upsert_global_summary(config, metrics):
    ...
    rows.append({
        ...
        'symbol': ','.join((config['data'].get('assets') or {}).keys()),
...
def print_results(metrics):
    ...
    print(f"  Avg crypto weight:   {metrics['avg_crypto_weight'] * 100:.2f}%")
...
def main():
    ...
    print(f"Loaded crypto-optimal-allocation bars: {len(inputs['returns_df'])}")
