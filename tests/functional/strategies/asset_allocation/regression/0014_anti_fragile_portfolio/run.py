...
from strategy_anti_fragile_portfolio import (
    Mt5AntiFragilePortfolioFeed, AntiFragilePortfolioStrategy, load_mt5_csv, prepare_anti_fragile_portfolio_features,
)
...
def load_data(config):
    ...
    df = prepare_anti_fragile_portfolio_features(raw, config.get('params', {}))
...
def build_cerebro(frame, config):
    ...
    feed = Mt5AntiFragilePortfolioFeed(dataname=frame['data'], timeframe=bt.TimeFrame.Days, compression=1)
    cerebro.adddata(feed, name=f"{config['data']['symbol']}_{config['data']['timeframe']}")
    cerebro.addstrategy(AntiFragilePortfolioStrategy, **config.get('params', {}))
