...
from strategy_equity_bond_allocation import EquityBondAllocationStrategy, load_mt5_csv, prepare_asset_data
...
def upsert_global_summary(config, metrics):
    ...
    rows.append({
        ...
        'symbol': ','.join([config['data']['equity_symbol'], config['data']['bond_symbol'], config['data']['gold_symbol']]),
...
def print_results(metrics):
    ...
    print(f"  Risk-on days:        {metrics['risk_on_days']}")
...
def main():
    ...
    print(f"Loaded equity-bond allocation bars: {len(inputs['aligned_index'])}")
