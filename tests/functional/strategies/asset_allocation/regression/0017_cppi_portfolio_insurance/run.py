...
from strategy_cppi_portfolio_insurance import (
    Mt5CppiPortfolioInsuranceFeed, CppiPortfolioInsuranceStrategy, load_mt5_csv, prepare_cppi_portfolio_insurance_features,
)
...
def load_data(config):
    ...
    df = prepare_cppi_portfolio_insurance_features(raw, config.get('params', {}))
...
def build_cerebro(frame, config):
    ...
    feed = Mt5CppiPortfolioInsuranceFeed(dataname=frame['data'], timeframe=bt.TimeFrame.Days, compression=1)
    cerebro.adddata(feed, name=f"{config['data']['symbol']}_{config['data']['timeframe']}")
    cerebro.addstrategy(CppiPortfolioInsuranceStrategy, **config.get('params', {}))
