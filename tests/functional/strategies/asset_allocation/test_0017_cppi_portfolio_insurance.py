"""Inlined regression test for tests/functional/strategies/asset_allocation/regression/0017_cppi_portfolio_insurance.
...
Strategy Principle:
    - This strategy ("CPPI Portfolio Insurance") implements a Constant Proportion Portfolio Insurance (CPPI) dynamic asset allocation strategy.
    - Market Assumptions: Markets exhibit medium-to-long term trends but can suffer significant drawdowns. Capital preservation can be achieved by establishing a floor value (e.g. 80% of running peak value) and dynamically adjusting exposure to the risky asset based on the "cushion" (the difference between portfolio value and the floor).
    - Indicators:
        - Running Peak: Cumulative maximum of Close price.
        - Floor Value: 80% of the running peak (`floor_pct`).
        - Cushion Percentage: Difference between Close and Floor, divided by Close.
        - Target Exposure: Cushion Percentage multiplied by CPPI multiplier (3.0, `multiplier`), capped at 100%.
    - Entry Signals:
        - Long Entry: On rebalance day (every 21 days, `rebalance_days`), if Target Exposure rises above 10%, place a buy order with position size equal to the calculated exposure.
    - Exit Signals:
        - Exit Long: On rebalance day, if Target Exposure falls below 5%, close the position.
        - Dynamic Rebalancing: On rebalance day, if the difference between current position size and target exposure size exceeds 20%, rebalance the position.
"""
...
_CONFIG = {
    'strategy': {
        'id': '0350',
        'name': 'CPPI Portfolio Insurance',
        'source_spec': 'research_papers_gold/strategy_specs/asset_allocation/0350_CPPI_Portfolio_Insurance_Strategy.md',
    },
    'data': {
        'symbol': 'XAUUSD',
        'timeframe': 'D1',
...
"""CPPI Portfolio Insurance Strategy backtest."""
...
def test_17_0017_cppi_portfolio_insurance() -> None:
    """Migrated regression test (runonce=True only).

    Originally located at tests/functional/strategies_regression/asset_allocation/0017_cppi_portfolio_insurance.
    """
    ...
    _close(metrics.get('final_value'), 1533998.876562, tol=1.533999e+00, key='final_value')
    _close(metrics.get('net_pnl'), 533998.876562, tol=5.339989e-01, key='net_pnl')
    _close(metrics.get('total_return_pct'), 53.39988765620001, tol=5.339989e-05, key='total_return_pct')
    _close(metrics.get('win_rate'), 40.0, tol=4.000000e-05, key='win_rate')
    _close(metrics.get('profit_factor'), 2.2730721344226725, tol=2.273072e-06, key='profit_factor')
    _close(metrics.get('max_drawdown'), 27.344618107330326, tol=2.734462e-05, key='max_drawdown')
    _close(metrics.get('sharpe_ratio'), 0.4468090190770332, tol=1.000000e-06, key='sharpe_ratio')
    _close(metrics.get('annual_return_pct'), 2.352056722660216, tol=2.352057e-06, key='annual_return_pct')
    _close(metrics.get('sqn'), 1.2434975160310915, tol=1.243498e-06, key='sqn')
    _close(metrics.get('ulcer_index'), 16.357833794184888, tol=1.635783e-05, key='ulcer_index')
