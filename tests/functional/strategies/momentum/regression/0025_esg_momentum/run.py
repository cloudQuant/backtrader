...
from strategy_esg_momentum import (
    Mt5EsgMomentumFeed, EsgMomentumStrategy, load_mt5_csv, prepare_esg_momentum_features,
)
...
def load_data(config):
    ...
    df = prepare_esg_momentum_features(raw, config.get('params', {}))
...
def build_cerebro(frame, config):
    ...
    feed = Mt5EsgMomentumFeed(dataname=frame['data'], timeframe=bt.TimeFrame.Days, compression=1)
    cerebro.adddata(feed, name=f"{config['data']['symbol']}_{config['data']['timeframe']}")
    cerebro.addstrategy(EsgMomentumStrategy, **config.get('params', {}))
