...
def prepare_esg_momentum_features(df, params):
    ...
    out = df.copy()
    out['momentum'] = out['close'].pct_change(mom_period)
    ret = out['close'].pct_change()
    vol = ret.rolling(vol_period).std()
    out['vol_score'] = 1.0 - vol.rolling(min(252, len(vol))).rank(pct=True)  # Low vol = high score
    ...
    out = out[['open', 'high', 'low', 'close', 'volume', 'openinterest',
               'momentum', 'vol_score', 'signal', 'rebalance_flag']].copy()
    return out.dropna()
