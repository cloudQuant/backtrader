"""Tests for MixBroker mid-frequency context interface."""
import pytest
...
def test_midfreq_context_exposes_readonly_state_views():
    """Test midfreq context exposes readonly state views."""
    symbol = "BTC/USDT"
...
def test_midfreq_context_reports_account_state_after_tick_execution():
    """Test midfreq context reports account state after tick execution."""
    data = DummyData()
    broker = MixBroker(cash=1000.0)
...
def test_midfreq_context_snapshot_all_includes_symbols_with_open_positions_without_market_events():
    """Test midfreq context snapshot all includes symbols with open positions."""
    broker = MixBroker(cash=1000.0)
    context = broker.get_context()
