...
class SimpleMAMultiDataStrategy(bt.Strategy):
    ...
    def notify_trade(self, trade):
        if trade.isclosed:
            self.log(f'Trade closed: {trade.getdataname()}, PnL: {trade.pnl:.2f}, Net: {trade.pnlcomm:.2f}')
        if trade.isopen:
            self.log(f'Trade opened: {trade.getdataname()} @ {trade.price:.2f}')

    def stop(self):
        print(f"策略结束: bar_num={self.bar_num}, buy_count={self.buy_count}, sell_count={self.sell_count}")
...
def run_strategy(max_bonds=100, initial_cash=10000000.0, commission=0.0002, verbose=False):
    ...
    max_drawdown = drawdown_analysis['max']['drawdown'] if drawdown_analysis['max']['drawdown'] else 0
...
def get_test_results():
    ...
    if _test_results is None:
        _test_results = run_strategy(max_bonds=100, initial_cash=10000000.0, 
                                      commission=0.0002, verbose=False)
