...
# -*- coding: utf-8 -*-
"""
测试用例: Order Close 收盘订单

参考来源: backtrader-master2/samples/order-close/close-daily.py
测试在收盘价执行订单
"""
...
def test_order_close():
    ...
    assert abs(sharpe_ratio - (0.3210201519350739)) < 1e-6, f"Expected sharpe_ratio=0.0, got {sharpe_ratio}"
    assert abs(annual_return - (0.014632998393384895)) < 1e-6, f"Expected annual_return=0.0, got {annual_return}"
    assert abs(max_drawdown - 4.113455968673918) < 1e-6, f"Expected max_drawdown=0.0, got {max_drawdown}"
