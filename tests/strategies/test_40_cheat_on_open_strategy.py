...
# -*- coding: utf-8 -*-
"""
测试用例: Cheat On Open 开盘价作弊策略

参考来源: backtrader-master2/samples/cheat-on-open/cheat-on-open.py
演示在开盘价执行订单的功能
"""
...
def test_cheat_on_open_strategy():
    ...
    print("=" * 50)
    print("Cheat On Open 开盘价作弊策略回测结果:")
    ...
    assert abs(sharpe_ratio - (11.647332609673429)) < 1e-6, f"Expected sharpe_ratio=0.0, got {sharpe_ratio}"
    assert abs(annual_return - (0.024145144571516192)) < 1e-6, f"Expected annual_return=0.0, got {annual_return}"
    assert abs(max_drawdown - 3.430658473286522) < 1e-6, f"Expected max_drawdown=0.0, got {max_drawdown}"
