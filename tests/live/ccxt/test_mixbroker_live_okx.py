...
class MultiSymbolLiveStrategy(bt.Strategy):
    ...
    def notify_tick(self, tick):
        ...
        if self.ticks_received[symbol] % 10 == 1:
            print(f"[on_tick] {symbol}: bid={data.get('bid'):.2f}, "
    ...
    def notify_orderbook(self, ob):
        ...
        if self.orderbooks_received[symbol] % 5 == 1:
            ...
            print(f"[on_orderbook] {symbol}: best_bid={best_bid:.2f}, "
    ...
    def notify_bar(self, bar):
        ...
        print(f"[on_bar] {symbol}: open={data.get('open'):.2f}, "
    ...
    def next(self):
        ...
        if self.next_calls % 20 == 1:
            print(f"[next] Call #{self.next_calls}, "
                  f"symbols with data: {current_symbols}")
            for symbol in current_symbols:
                if symbol in self.latest_tick:
                    print(f"  {symbol} tick: "
                          f"last={self.latest_tick[symbol].get('last'):.2f}")
                if symbol in self.latest_orderbook:
                    ob = self.latest_orderbook[symbol]
                    spread = ((ob['asks'][0][0] - ob['bids'][0][0])
                              if ob.get('bids') and ob.get('asks') else 0)
    ...
    try:
        while time.time() - start_time < duration:
            ...
            strategy.notify_orderbook(
                _wrap('orderbook', symbol, orderbook))
    except Exception as e:
    ...
    try:
        ...
        for symbol in symbols:
            if symbol not in exchange.markets:
                raise ValueError(
                    f"Symbol {symbol} not found in OKX markets")
        ...
        tasks = []
        for symbol in symbols:
            tasks.append(watch_ticker(
                exchange, symbol, strategy, start_time, duration_seconds))
            tasks.append(watch_orderbook(
                exchange, symbol, strategy, start_time, duration_seconds))
            tasks.append(watch_ohlcv(
                exchange, symbol, strategy, start_time, duration_seconds))
...
def _print_and_assert(strategy, symbols):
    ...
    for symbol in symbols:
        assert stats['ticks_received'].get(symbol, 0) > 0, \
            f"No ticks received for {symbol}"
    ...
    assert len(stats['symbols_in_next']) == len(symbols), \
