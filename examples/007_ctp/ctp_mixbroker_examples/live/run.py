"""CTP MixBroker live trading runner with SimNow.

This module runs live trading examples using BtApiStore and BtApiBroker
connected to SimNow for testing.
"""
...
from ctp_mix_examples_common import format_summary, get_strategy_class


def _build_parser():
    parser = argparse.ArgumentParser(description="Run CTP SimNow mixed-mode live examples")
    parser.add_argument("--config", default="single_symbol.yaml", help="YAML config path")
    return parser


def main():
    """Run the CTP MixBroker live trading example.
    ...
    print(f"Running mixed-mode live example with {config_path}")
