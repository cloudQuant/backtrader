            ...
            with started_store(env_key, stop_on_exit=False) as (store, config, ek):
                ...
                class RepeatOrderStrategy(bt.Strategy):
                    ...
                    def next(self):
                        ...
                        # Same symbol/side/offset/size/price in one bar is the
                        # exact duplicate key the monitor is expected to catch.
                        while self.orders_placed < 3:
                            ensure_ctp_trading_admission(store, symbol)
                            order = self.buy(size=1, exectype=bt.Order.Limit, price=self.limit_price, offset="open", position_side="long")
                            if order:
