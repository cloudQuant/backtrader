...
2. Backend switching:
   ``cerebro.plot()`` / ``cerebro.plot(backend='plotly')`` / ``cerebro.plot(backend='bokeh')``
3. Custom color schemes - Tableau10/Tableau20, etc.
3. Custom decimal places and legend width
4. Save as HTML file

Usage:
    python examples/006_plots/example_plotly_charts.py
"""
...
def example_backend_compare():
    """Example 0: Compare plotting backends."""
    print("\n" + "=" * 60)
    print("Example 0: Backend Comparison")
    print("=" * 60)
    print("matplotlib, plotly, and bokeh render from the same strategy setup.")
    ...
    for backend in ("matplotlib", "plotly", "bokeh"):
        try:
            ...
            cerebro.run()
            print(f"\nPlotting with cerebro.plot(backend='{backend}') ...")
            cerebro.plot(backend=backend, style="candle", iplot=False)
            print(f"✓ {backend} backend call completed")
        except Exception as exc:
            print(f"✗ {backend} backend call skipped: {exc}")
...
def example_backend_switch():
    ...
    print("All three are now supported via the same Cerebro API path.")
...
def example_custom_scheme():
    ...
    data = _build_nasdaq_data(
        start=datetime.datetime(2000, 1, 1), end=datetime.datetime(2013, 6, 30)
    )
    cerebro.adddata(data, name="NVDA")
    cerebro.addstrategy(PlotlySMAStrategy)
...
def example_color_schemes():
    ...
    from backtrader.plot.plot_plotly import TABLEAU10, TABLEAU20, TABLEAU10_LIGHT, get_color_scheme
...
if __name__ == "__main__":
    ...
    # Run all examples
    if os.environ.get("RUN_PLOTLY_BACKEND_COMPARE", "0") == "1":
        example_backend_compare()
