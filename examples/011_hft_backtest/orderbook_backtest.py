#!/usr/bin/env python
"""Example 3: Order book depth backtesting with TickBroker.

Demonstrates using OrderBookChannel + TickBroker + Cerebro for precise
...
Usage:
    ...
    # Then run:
    python examples/011_hft_backtest/orderbook_backtest.py
"""
...
def main():
    ...
    engine with TickBroker, and executes the OBSpreadStrategy.

    Raises:
        SystemExit: If the required data file is not found.
    """
    symbol = "BTC/USDT"
    ob_file = PROJECT_ROOT / "tests" / "datas" / "tick_data" / f'orderbook_{symbol.replace("/", "_")}.jsonl'
    ...
    channel = OrderBookChannel(symbol=symbol, dataname=str(ob_file), depth=20)
    queue = StreamingEventQueue(channels=[channel], preload_window=5.0)

    # 2. Set up Cerebro with TickBroker. TickBroker handles order-book depth
    # matching directly; enable_impact activates the supplied impact model.
    impact_model = LinearImpactModel(coefficient=0.0005)
    cerebro = bt.Cerebro()
    cerebro.setbroker(TickBroker(cash=100000.0, impact_model=impact_model, enable_impact=True))
