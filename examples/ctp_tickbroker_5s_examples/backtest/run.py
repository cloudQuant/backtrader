...
from ctp_bar_examples_common import build_tick_backtest_channel, format_summary, get_strategy_class
from ctp_example_support import attach_trade_logger, load_config


def _build_parser():
    parser = argparse.ArgumentParser(description='Run TickBroker-style 5s backtests')
...
def main():
    ...
    cerebro = bt.Cerebro()
    cerebro.setbroker(TickBroker(cash=initial_cash, **broker_kwargs))
    ...
    channel = build_tick_backtest_channel(config)

    print(f'Running TickBroker-style 5s backtest with {config_path}')
