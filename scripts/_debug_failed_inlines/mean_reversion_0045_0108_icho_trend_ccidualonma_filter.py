...
def test_45_0045_0108_icho_trend_ccidualonma_filter() -> None:
    ...
    _close(metrics.get('final_value'), 899830.0000000044, tol=8.998300e-01, key='final_value')
    _close(metrics.get('net_pnl'), -100169.99999999558, tol=1.001700e-01, key='net_pnl')
    _close(metrics.get('total_return_pct'), -10.016999999999554, tol=1.001700e-05, key='total_return_pct')
    _close(metrics.get('win_rate'), 31.182795698924732, tol=3.118280e-05, key='win_rate')
    _close(metrics.get('profit_factor'), 0.5783079274068487, tol=1.000000e-06, key='profit_factor')
    _close(metrics.get('max_drawdown'), 12.512583248780091, tol=1.251258e-05, key='max_drawdown')
    _close(metrics.get('sharpe_ratio'), -2.678169839452656, tol=2.678170e-06, key='sharpe_ratio')
    _close(metrics.get('annual_return_pct'), -27.41873145931093, tol=2.741873e-05, key='annual_return_pct')
    _close(metrics.get('sqn'), -1.5308071003181558, tol=1.530807e-06, key='sqn')
