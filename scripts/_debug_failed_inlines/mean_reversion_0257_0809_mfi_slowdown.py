"""Inlined regression test for tests/functional/strategies/mean_reversion/regression/0257_0809_mfi_slowdown.
...
_CONFIG = {
    'strategy': {
        'name': 'Exp_MFI_Slowdown',
        'source_ea': 'ea/0809_Exp_MFI_Slowdown',
    },
    'data': {
        'symbol': 'XAUUSD',
        'timeframe': 'M15',
...
def test_258_0257_0809_mfi_slowdown() -> None:
    ...
    Originally located at tests/functional/strategies_regression/mean_reversion/0257_0809_mfi_slowdown.
    """
    ...
    assert metrics.get('trade_num') == 111, f"trade_num: expected=111, got={metrics.get('trade_num')!r}"
    assert metrics.get('total_trades') == 111, f"total_trades: expected=111, got={metrics.get('total_trades')!r}"
    assert metrics.get('stop_count') == 0, f"stop_count: expected=0, got={metrics.get('stop_count')!r}"
    _close(metrics.get('final_value'), 998044.5999999996, tol=9.980446e-01, key='final_value')
    _close(metrics.get('sharpe_ratio'), -6.049902425934021, tol=6.049902e-06, key='sharpe_ratio')
    _close(metrics.get('annual_return'), -10.932303032983889, tol=1.093230e-05, key='annual_return')
    _close(metrics.get('max_drawdown'), 0.0027429270075756225, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('return_rate'), -0.19554000000003846, tol=1.000000e-06, key='return_rate')
