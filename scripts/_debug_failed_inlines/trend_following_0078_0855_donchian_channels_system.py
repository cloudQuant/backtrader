"""Inlined regression test for tests/functional/strategies/trend_following/regression/0078_0855_donchian_channels_system.
...
_CONFIG = {
    'strategy': {
        'name': 'Exp_Donchian_Channels_System',
        'source_ea': 'ea/0855_Exp_Donchian_Channels_System',
    },
    'data': {
        'symbol': 'XAUUSD',
        'timeframe': 'M15',
...
def test_78_0078_0855_donchian_channels_system() -> None:
    """Migrated regression test (runonce=True only).

    Originally located at tests/functional/strategies_regression/trend_following/0078_0855_donchian_channels_system.
    """
    ...
    assert metrics.get('rows') == 5756, f"rows: expected=5756, got={metrics.get('rows')!r}"
    ...
    _close(metrics.get('sum_profit'), 230.40000000000867, tol=2.304000e-04, key='sum_profit')
    _close(metrics.get('final_value'), 1000230.3999999998, tol=1.000230e+00, key='final_value')
    _close(metrics.get('sharpe_ratio'), 1.4753384214241052, tol=1.475338e-06, key='sharpe_ratio')
    _close(metrics.get('annual_return'), 0.013719655559591017, tol=1.000000e-06, key='annual_return')
    _close(metrics.get('max_drawdown'), 0.0007155226862251729, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('return_rate'), 0.023039999999974192, tol=1.000000e-06, key='return_rate')
