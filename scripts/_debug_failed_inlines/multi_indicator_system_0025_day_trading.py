...
def test_25_0025_day_trading() -> None:
    ...
    assert metrics.get('rows') == 18330, f"rows: expected=18330, got={metrics.get('rows')!r}"
    assert metrics.get('bar_num') == 18330, f"bar_num: expected=18330, got={metrics.get('bar_num')!r}"
    ...
    _close(metrics.get('sum_profit'), 421.50000000001455, tol=4.215000e-04, key='sum_profit')
    ...
    _close(metrics.get('max_drawdown'), 0.0006557652360449143, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('return_rate'), 0.042150000000029664, tol=1.000000e-06, key='return_rate')
