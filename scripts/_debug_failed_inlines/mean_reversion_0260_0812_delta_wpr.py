"""Inlined regression test for tests/functional/strategies/mean_reversion/regression/0260_0812_delta_wpr.
...
_CONFIG = {
    'strategy': {
        'name': 'Exp_Delta_WPR',
        'source_ea': 'ea/0812_Exp_Delta_WPR',
    },
    'data': {
        'symbol': 'XAUUSD',
        'timeframe': 'M15',
...
def test_261_0260_0812_delta_wpr() -> None:
    ...
    Originally located at tests/functional/strategies_regression/mean_reversion/0260_0812_delta_wpr.
    """
    ...
    assert metrics.get('rows') == 5632, f"rows: expected=5632, got={metrics.get('rows')!r}"
    ...
    assert metrics.get('trade_num') == 13, f"trade_num: expected=13, got={metrics.get('trade_num')!r}"
    ...
    _close(metrics.get('sum_profit'), 693.4000000000015, tol=6.934000e-04, key='sum_profit')
    _close(metrics.get('final_value'), 1000693.4000000001, tol=1.000693e+00, key='final_value')
    _close(metrics.get('sharpe_ratio'), 7.127287931144239, tol=7.127288e-06, key='sharpe_ratio')
    _close(metrics.get('annual_return'), 0.0418519009634283, tol=1.000000e-06, key='annual_return')
    _close(metrics.get('max_drawdown'), 0.0006113019453102578, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('return_rate'), 0.06934000000000662, tol=1.000000e-06, key='return_rate')
