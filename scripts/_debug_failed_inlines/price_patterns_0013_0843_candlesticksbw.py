"""Inlined regression test for tests/functional/strategies/price_patterns/regression/0013_0843_candlesticksbw.
...
_CONFIG = {
    'strategy': {
        'name': 'Exp_CandlesticksBW',
        'source_ea': 'ea/0843_Exp_CandlesticksBW',
    },
    'data': {
        'symbol': 'XAUUSD',
        'timeframe': 'M15',
...
def test_13_0013_0843_candlesticksbw() -> None:
    ...
    Originally located at tests/functional/strategies_regression/price_patterns/0013_0843_candlesticksbw.
    """
    ...
    assert metrics.get('rows') == 5524, f"rows: expected=5524, got={metrics.get('rows')!r}"
    assert metrics.get('bar_num') == 5524, f"bar_num: expected=5524, got={metrics.get('bar_num')!r}"
    ...
    assert metrics.get('trade_num') == 66, f"trade_num: expected=66, got={metrics.get('trade_num')!r}"
    ...
    _close(metrics.get('sum_profit'), 512.2999999999774, tol=5.123000e-04, key='sum_profit')
    _close(metrics.get('final_value'), 1000512.2999999998, tol=1.000512e+00, key='final_value')
    _close(metrics.get('sharpe_ratio'), 2.3405549062556537, tol=2.340555e-06, key='sharpe_ratio')
    _close(metrics.get('annual_return'), 0.030757890176028112, tol=1.000000e-06, key='annual_return')
    _close(metrics.get('max_drawdown'), 0.0017471585812984189, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('return_rate'), 0.0512299999999799, tol=1.000000e-06, key='return_rate')
