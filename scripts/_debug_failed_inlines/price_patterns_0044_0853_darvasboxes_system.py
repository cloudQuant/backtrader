"""Inlined regression test for tests/functional/strategies/price_patterns/regression/0044_0853_darvasboxes_system.
...
_CONFIG = {
    'strategy': {
        'name': 'Exp_DarvasBoxes_System',
        'source_ea': 'ea/0853_Exp_DarvasBoxes_System',
    },
    'data': {
        'symbol': 'XAUUSD',
        'timeframe': 'M15',
...
def test_44_0044_0853_darvasboxes_system() -> None:
    ...
    Originally located at tests/functional/strategies_regression/price_patterns/0044_0853_darvasboxes_system.
    """
    ...
    assert metrics.get('rows') == 5984, f"rows: expected=5984, got={metrics.get('rows')!r}"
    assert metrics.get('bar_num') == 5984, f"bar_num: expected=5984, got={metrics.get('bar_num')!r}"
    ...
    _close(metrics.get('sum_profit'), -778.5999999999785, tol=7.786000e-04, key='sum_profit')
    _close(metrics.get('final_value'), 999221.3999999998, tol=9.992214e-01, key='final_value')
    _close(metrics.get('sharpe_ratio'), -8.463959511628868, tol=8.463960e-06, key='sharpe_ratio')
    _close(metrics.get('annual_return'), -0.04502628425929777, tol=1.000000e-06, key='annual_return')
    _close(metrics.get('max_drawdown'), 0.0008897010652417081, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('return_rate'), -0.07786000000001847, tol=1.000000e-06, key='return_rate')
