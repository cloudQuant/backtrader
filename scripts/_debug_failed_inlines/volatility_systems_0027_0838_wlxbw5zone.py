"""Inlined regression test for tests/functional/strategies/volatility_systems/regression/0027_0838_wlxbw5zone.
...
_CONFIG = {
    'strategy': {
        'name': 'Exp_wlxBW5Zone',
        'source_ea': 'ea/0838_Exp_wlxBW5Zone',
    },
    'data': {
        'symbol': 'XAUUSD',
        'timeframe': 'M15',
...
def test_27_0027_0838_wlxbw5zone() -> None:
    """Migrated regression test (runonce=True only).

    Originally located at tests/functional/strategies_regression/volatility_systems/0027_0838_wlxbw5zone.
    """
    ...
    assert metrics.get('rows') == 5448, f"rows: expected=5448, got={metrics.get('rows')!r}"
    assert metrics.get('bar_num') == 5448, f"bar_num: expected=5448, got={metrics.get('bar_num')!r}"
    ...
    _close(metrics.get('sum_profit'), 490.6999999999698, tol=4.907000e-04, key='sum_profit')
    _close(metrics.get('final_value'), 1000490.6999999993, tol=1.000491e+00, key='final_value')
    _close(metrics.get('sharpe_ratio'), 3.358526165013546, tol=3.358526e-06, key='sharpe_ratio')
    _close(metrics.get('annual_return'), 0.029442471882247887, tol=1.000000e-06, key='annual_return')
    _close(metrics.get('max_drawdown'), 0.0012112350363191, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('return_rate'), 0.04906999999991779, tol=1.000000e-06, key='return_rate')
