"""Inlined regression test for tests/functional/strategies/trend_following/regression/0248_0851_aroon_oscillator_sign_alert.
...
_CONFIG = {
    'strategy': {
        'name': 'Exp_AroonOscillatorSignAlert',
        'source_ea': 'ea/0851_Exp_AroonOscillatorSignAlert',
    },
    'data': {
        'symbol': 'XAUUSD',
        'timeframe': 'M15',
...
def test_247_0248_0851_aroon_oscillator_sign_alert() -> None:
    ...
    Originally located at tests/functional/strategies_regression/trend_following/0248_0851_aroon_oscillator_sign_alert.
    """
    ...
    assert metrics.get('rows') == 5876, f"rows: expected=5876, got={metrics.get('rows')!r}"
    ...
    assert metrics.get('trade_num') == 41, f"trade_num: expected=41, got={metrics.get('trade_num')!r}"
    ...
    _close(metrics.get('sum_profit'), 1300.1999999999862, tol=1.300200e-03, key='sum_profit')
    _close(metrics.get('final_value'), 1001300.2000000002, tol=1.001300e+00, key='final_value')
    _close(metrics.get('sharpe_ratio'), 7.072035488903727, tol=7.072035e-06, key='sharpe_ratio')
    _close(metrics.get('annual_return'), 0.07988630694472991, tol=1.000000e-06, key='annual_return')
    _close(metrics.get('max_drawdown'), 0.000849497394871393, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('return_rate'), 0.13002000000001956, tol=1.000000e-06, key='return_rate')
