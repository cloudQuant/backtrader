"""Inlined regression test for tests/functional/strategies/mean_reversion/regression/0259_0811_rsi_slowdown.
...
_CONFIG = {
    'strategy': {
        'name': 'Exp_RSI_Slowdown',
        'source_ea': 'ea/0811_Exp_RSI_Slowdown',
    },
    'data': {
        'symbol': 'XAUUSD',
        'timeframe': 'M15',
...
def test_260_0259_0811_rsi_slowdown() -> None:
    ...
    Originally located at tests/functional/strategies_regression/mean_reversion/0259_0811_rsi_slowdown.
    """
    ...
    assert metrics.get('trade_num') == 21, f"trade_num: expected=21, got={metrics.get('trade_num')!r}"
    ...
    _close(metrics.get('sum_profit'), -424.50000000002547, tol=4.245000e-04, key='sum_profit')
    _close(metrics.get('final_value'), 999575.5000000003, tol=9.995755e-01, key='final_value')
    _close(metrics.get('sharpe_ratio'), -3.2427608704608297, tol=3.242761e-06, key='sharpe_ratio')
    _close(metrics.get('annual_return'), -0.02480140601778963, tol=1.000000e-06, key='annual_return')
    _close(metrics.get('max_drawdown'), 0.001069429442698035, tol=1.000000e-06, key='max_drawdown')
    _close(metrics.get('return_rate'), -0.042449999999960575, tol=1.000000e-06, key='return_rate')
