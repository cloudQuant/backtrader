...
def parse_log_file(log_path: str) -> ProfileReport:
    ...
    # 提取调用统计
    calls_match = re.search(r'(\d+) function calls \((\d+) primitive calls\) in ([\d.]+) seconds', content)
    if calls_match:
...
def compare_reports(master: ProfileReport, remove: ProfileReport) -> None:
    ...
    print(f"{'总执行时间':<30} {master.total_time:<20.4f} {remove.total_time:<20.4f} {f'+{time_pct:.1f}%  (+{time_diff:.2f}秒)':<30}")
    
    calls_diff = remove.total_calls - master.total_calls
    calls_pct = (calls_diff / master.total_calls) * 100
    print(f"{'函数调用总数':<30} {master.total_calls:<20,} {remove.total_calls:<20,} {f'+{calls_pct:.1f}%  (+{calls_diff:,})':<30}")
    
    funcs_diff = remove.total_functions - master.total_functions
    funcs_pct = (funcs_diff / master.total_functions) * 100
    print(f"{'函数种类数':<30} {master.total_functions:<20,} {remove.total_functions:<20,} {f'+{funcs_pct:.1f}%  (+{funcs_diff})':<30}")
    ...
    if degraded_funcs:
        print(f"{'函数名':<70} {'Master(秒)':<12} {'Remove(秒)':<12} {'增加(秒)':<12} {'增幅(%)':<12}")
        print("-" * 100)
        for func_name, master_stat, remove_stat, diff, pct in degraded_funcs[:20]:
            ...
            print(f"{short_name:<70} {master_stat.tottime:<12.4f} {remove_stat.tottime:<12.4f} {diff:<12.4f} {pct:<12.1f}")
    else:
        print("未发现显著的性能退化")
    ...
    if call_explosion:
        ...
        for func_name, master_stat, remove_stat, ratio in call_explosion[:20]:
            short_name = func_name if len(func_name) <= 70 else func_name[:67] + "..."
            print(f"{short_name:<70} {master_stat.ncalls:<15,} {remove_stat.ncalls:<15,} {ratio:<12.1f}x")
    else:
        print("未发现调用次数暴增的函数")
    ...
    master_bt_time = sum(stat.tottime for name, stat in master.functions.items() 
                         if 'backtrader' in name)
    remove_bt_time = sum(stat.tottime for name, stat in remove.functions.items() 
                         if 'backtrader' in name)
    ...
    print(f"7. Backtrader相关函数总耗时: Master={master_bt_time:.2f}秒, Remove={remove_bt_time:.2f}秒 (增加{remove_bt_time-master_bt_time:.2f}秒)")
