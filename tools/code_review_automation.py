...
class CodeReviewAutomation:
    ...
    def load_review_standards(self):
        ...
        return {
            ...
            "coverage": {"min_coverage": 80, "critical_coverage": 95},
            "performance": {"max_execution_time": 10.0, "max_memory_increase": 1024 * 1024},  # 1MB
        }
    ...
    def generate_review_summary(self, review_time):
        ...
        high_priority_issues = sum(
            1 for r in self.review_results if r.priority == "high" and r.status != "pass"
        )
        medium_priority_issues = sum(
            1 for r in self.review_results if r.priority == "medium" and r.status != "pass"
        )
        low_priority_issues = sum(
            1 for r in self.review_results if r.priority == "low" and r.status != "pass"
        )
        ...
        # 关键问题报告
        critical_issues = [
            r for r in self.review_results if r.priority == "high" and r.status == "fail"
        ]
        if critical_issues:
    ...
    def make_review_decision(self):
        ...
        critical_failures = sum(
            1 for r in self.review_results if r.priority == "high" and r.status == "fail"
        )

        major_failures = sum(
            1 for r in self.review_results if r.priority == "medium" and r.status == "fail"
        )

        # 特殊检查：元类移除和API兼容性
        metaclass_passed = any(
            r.category == "metaclass_removal" and r.status == "pass" for r in self.review_results
        )
        api_compat_passed = any(
            r.category == "api_compatibility" and r.status == "pass" for r in self.review_results
        )
