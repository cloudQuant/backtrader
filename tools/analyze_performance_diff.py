...
For analyzing performance differences between Master version and Remove-Metaprogramming version.
"""
...
def parse_log_file(log_path: str) -> ProfileReport:
    ...
    # Parse function statistics (from SECTION 2: TOP 50 FUNCTIONS BY TOTAL TIME)
    functions = {}
...
def compare_reports(master: ProfileReport, remove: ProfileReport) -> None:
    ...
    for func_name, master_stat in master.functions.items():
        if func_name in remove.functions:
            remove_stat = remove.functions[func_name]
            if master_stat.percall_tot > 0.000001:  # Avoid division by near-zero
