...
def compare_with_baseline(current_log, baseline_file):
    ...
    try:
        ...
        print(
            f"{'总执行时间':<30} {baseline_data['total_time']:>19.2f}s {current_log['total_time']:>19.2f}s {time_diff:+19.2f}s ({time_pct:+.1f}%)"
        )
        ...
        print(
            f"{'总函数调用':<30} {baseline_data['total_calls']:>19,} {current_log['total_calls']:>19,} {calls_diff:+19,} ({calls_pct:+.1f}%)"
        )
...
def generate_optimization_recommendations(bottlenecks, log_data):
    ...
    for i, rec in enumerate(recommendations, 1):
        ...
        print(
            f"   优先级: {'高' if rec['priority'] == 1 else '中' if rec['priority'] == 2 else '低'}"
        )
