...
class MetaclassDetector(ast.NodeVisitor):
        ...
        for keyword in getattr(node, "keywords", []):
            if keyword.arg == "metaclass":
                ...
                if metaclass_name and metaclass_name in self.known_metaclasses:
                    self._record_metaclass_usage(node.name, metaclass_name, node.lineno, "explicit")

        # 检查通过继承获得的元类
        for base in node.bases:
            base_name = self._get_name(base)
            if base_name and base_name in self.known_metaclasses:
                self._record_metaclass_usage(node.name, base_name, node.lineno, "inheritance")
    ...
    def visit_Call(self, node: ast.Call):
        ...
        if func_name == "type" and len(node.args) >= 3:
            self._record_dynamic_creation(node, "type_call", "Dynamic class creation with type()")

        # 检查with_metaclass调用
        elif func_name == "with_metaclass" or "with_metaclass" in func_name:
            self._record_dynamic_creation(node, "metaclass_direct", "six.with_metaclass usage")
    ...
    def visit_Assign(self, node: ast.Assign):
        ...
        for target in node.targets:
            if isinstance(target, ast.Attribute) and target.attr == "__class__":
                self._record_dynamic_creation(node, "class_assignment", "__class__ assignment")

        self.generic_visit(node)

    def _record_metaclass_usage(
        self, class_name: str, metaclass_name: str, line_number: int, usage_type: str
    ):
        """记录元类使用"""
        # 评估复杂度
    ...
    def _record_dynamic_creation(self, node: ast.AST, creation_type: str, description: str):
        """记录动态创建"""
        ...
        for item in node.body:
            ...
            if isinstance(item, ast.FunctionDef):
                if item.name in ["__new__", "__init_subclass__", "__metaclass__"]:
                    self._record_metaclass_usage(
                        node.name, f"special_method_{item.name}", item.lineno, "special_method"
                    )
...
class MetaclassDetectionTool:
    ...
    def detect_metaclass_usage(self):
        ...
        for root, dirs, files in os.walk(self.project_root):
            ...
            dirs[:] = [
                d
                for d in dirs
                if not d.startswith(".") and d not in ["__pycache__", "build", "dist", "tests"]
            ]
    ...
    def generate_migration_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "summary": {
                ...
                "high_priority_migrations": len(
                    [u for u in self.all_metaclass_usages if u.migration_priority <= 2]
                ),
                "complex_migrations": len(
                    [u for u in self.all_metaclass_usages if u.complexity_level == "complex"]
                ),
            },
    ...
    def _create_migration_plan(
        self, metaclass_name: str, usages: List[MetaclassUsage]
    ...
    def _generate_migration_steps(self, metaclass_name: str) -> List[str]:
        """生成迁移步骤"""
        common_steps = ["创建备份分支", "编写现有功能的测试用例", "分析现有实现的所有功能"]
        ...
        steps = common_steps + specific_steps.get(
            metaclass_name,
            [f"分析{metaclass_name}的具体功能", "设计替代方案", "逐步迁移", "验证功能完整性"],
        )

        steps.extend(["运行回归测试", "性能测试对比", "代码审查", "文档更新"])
    ...
    def _identify_prerequisites(self, metaclass_name: str) -> List[str]:
        """识别前置条件"""
        prerequisites = {
            "MetaSingleton": ["SingletonMixin基类实现完成"],
            "MetaParams": ["ParameterDescriptor系统完成", "参数继承机制测试通过"],
            "MetaLineSeries": ["LineDescriptor系统完成", "LineBuffer性能验证通过"],
        }
    ...
    def _identify_risk_factors(
        ...
        # 特定元类的风险
        specific_risks = {
            "MetaLineSeries": ["性能敏感，需要仔细验证", "涉及核心数据访问逻辑"],
    ...
    def _generate_test_requirements(self, metaclass_name: str) -> List[str]:
        """生成测试要求"""
        base_requirements = ["功能完整性测试", "性能回归测试", "内存泄漏检查"]

        specific_requirements = {
            "MetaSingleton": ["单例模式测试", "线程安全测试", "多进程环境测试"],
            "MetaParams": ["参数继承测试", "参数验证测试", "默认值处理测试"],
            "MetaLineSeries": ["数据访问测试", "索引操作测试", "大数据集性能测试"],
        }
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if not recommendations:
            recommendations = ["建立详细的测试覆盖", "制定回滚计划", "分阶段验证迁移结果"]
    ...
    def save_report(self, report: Dict[str, Any]) -> str:
        ...
        filename = (
            f"analysis_results/metaclass_detection_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
    ...
    def print_summary(self, report: Dict[str, Any]):
        ...
        for plan in report["migration_plans"][:10]:
            print(
                f"{plan['target_class']:<20} {plan['estimated_effort']:<10} {len(plan['prerequisites']):<10}"
            )
