...
class APIInspector:
            ...
            try:
                ...
                if inspect.ismethod(attr) or inspect.isfunction(attr):
                    ...
                    class_info["attributes"][name] = {
                        ...
                        "value": (
                            repr(attr)[:100] if len(repr(attr)) < 100 else repr(attr)[:100] + "..."
                        ),
                    }
    ...
    def _compare_method_signatures(
        self, class_name: str, method_name: str, old_method: Dict, new_method: Dict
    ) -> APICompatibilityResult:
        """比较方法签名"""
        issues = []
        is_compatible = True
            ...
            if param_name not in new_params:
                ...
                if old_param["default"] != new_param["default"]:
                    if old_param["default"] is None and new_param["default"] is not None:
                        ...
                        issues.append(
                            f"参数 '{param_name}' 的默认值从 {old_param['default']} 变为 {new_param['default']}"
                        )

                # 检查类型注解变化
                if old_param["annotation"] != new_param["annotation"]:
                    issues.append(
                        f"参数 '{param_name}' 的类型注解从 {old_param['annotation']} 变为 {new_param['annotation']}"
                    )
...
class BehaviorTester:
    ...
    def run_behavior_comparison(
        self, old_implementation, new_implementation, test_case: CompatibilityTestCase
    ...
    def _run_test_implementation(
        self, implementation, test_case: CompatibilityTestCase
    ) -> Tuple[Any, float, Exception]:
        """运行测试实现"""
        result = None
        exception = None
    ...
    def _determine_difference_type(
        self, old_result, new_result, old_exception, new_exception
    ) -> str:
        """确定差异类型"""
        if old_exception and new_exception:
...
class CompatibilityTestFramework:
    ...
    def test_api_compatibility(
        self, old_branch: str = "main", new_branch: str = "remove-metaprogramming"
    ) -> List[APICompatibilityResult]:
        """测试API兼容性"""
        print("开始API兼容性测试...")
    ...
    def compare_behavior(
        self, test_cases: List[CompatibilityTestCase] = None
    ) -> List[BehaviorComparisonResult]:
        """比较新旧实现的行为"""
        print("开始行为比较测试...")
        ...
        for test_case in test_cases:
            ...
            try:
                ...
                result = self.behavior_tester.run_behavior_comparison(old_impl, new_impl, test_case)
    ...
    def _generate_comprehensive_report(
        self,
        api_results: List[APICompatibilityResult],
        ...
        api_stats = {
            ...
            "compatibility_rate": (
                len([r for r in api_results if r.is_compatible]) / len(api_results) * 100
                if api_results
                else 0
            ),
        }
        ...
        behavior_stats = {
            ...
            "equivalence_rate": (
                len([r for r in behavior_results if r.is_equivalent]) / len(behavior_results) * 100
                if behavior_results
                else 0
            ),
            "average_performance_delta": (
                sum(r.performance_delta for r in behavior_results) / len(behavior_results)
                if behavior_results
                else 0
            ),
        }
        ...
        report = {
            ...
            "summary": {
                "overall_compatibility": (
                    "PASS"
                    if api_stats["compatibility_rate"] > 95
                    and behavior_stats["equivalence_rate"] > 90
                    else "FAIL"
                ),
        ...
        if incompatible_apis:
            ...
            if removal_issues:
                recommendations.append(
                    f"有{len(removal_issues)}个移除类/方法的问题，建议添加废弃警告和兼容性层"
                )
    ...
    def save_report(self, report: Dict[str, Any]) -> str:
        ...
        filename = (
            f"test_results/compatibility_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
