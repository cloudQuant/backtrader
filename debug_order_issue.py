...
class DebugStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        if current_date == "2019-05-31" and self.order is None:
            ...
            self.log(
                f"创建订单时 data.datetime.datetime() = {bt.num2date(target_data.datetime[0])}"
            )
        ...
        if current_date == "2019-06-18":
            ...
            if current_date == target_date:
                ...
                self.log(
                    f"复牌时 data.datetime.datetime() = {bt.num2date(target_data.datetime[0])}"
                )
                if self.order:
                    ...
                    self.log(
                        f"比较: data.datetime[0] ({target_data.datetime[0]}) vs order.created.dt ({self.order.created.dt})"
                    )
                    self.log(
                        f"条件检查: data.datetime[0] <= order.created.dt ? {target_data.datetime[0] <= self.order.created.dt}"
                    )
