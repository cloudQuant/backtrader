...
class TradeLogger(Observer):
    ...
    def __init__(self):
        ...
        self._ensure_single_lineiterator_registration()
    ...
    def _ensure_single_lineiterator_registration(self):
        if not hasattr(self, "_owner") or self._owner is None:
            return
        if not hasattr(self._owner, "_lineiterators"):
            return
        if self._ltype not in self._owner._lineiterators:
            return
        observers = self._owner._lineiterators[self._ltype]
        seen_self = False
        keep = []
