...
class ReportGenerator:
    ...
    def print_summary(self):
        ...
        print("\n*** PnL ***")
        print(f"Start Capital         : {metrics.get('start_cash', 'N/A'):,.2f}" if metrics.get('start_cash') else "Start Capital         : N/A")
        print(f"Net Profit            : {metrics.get('rpl', 'N/A'):,.2f}" if metrics.get('rpl') else "Net Profit            : N/A")
        print(f"Total Return          : {metrics.get('total_return', 'N/A'):.2f}%" if metrics.get('total_return') else "Total Return          : N/A")
        print(f"Annual Return         : {metrics.get('annual_return', 'N/A'):.2f}%" if metrics.get('annual_return') else "Annual Return         : N/A")
        
        print("\n*** Risk ***")
        print(f"Max Drawdown ($)      : {metrics.get('max_money_drawdown', 'N/A'):,.2f}" if metrics.get('max_money_drawdown') else "Max Drawdown ($)      : N/A")
        print(f"Max Drawdown (%)      : {metrics.get('max_pct_drawdown', 'N/A'):.2f}%" if metrics.get('max_pct_drawdown') else "Max Drawdown (%)      : N/A")
        print(f"Sharpe Ratio          : {metrics.get('sharpe_ratio', 'N/A'):.2f}" if metrics.get('sharpe_ratio') else "Sharpe Ratio          : N/A")
        
        print("\n*** Trades ***")
        print(f"Total Trades          : {metrics.get('total_number_trades', 0)}")
        print(f"Win Rate              : {metrics.get('pct_winning', 'N/A'):.2f}%" if metrics.get('pct_winning') else "Win Rate              : N/A")
        print(f"SQN Score             : {metrics.get('sqn_score', 'N/A'):.2f}" if metrics.get('sqn_score') else "SQN Score             : N/A")
