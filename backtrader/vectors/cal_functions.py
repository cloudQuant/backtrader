...
try:
    from backtrader.utils.cal_performance_indicators import calculation_by_cython as ts
except ImportError:
    from backtrader.utils.cal_performance_indicators import calculation_by_numba as ts

try:
    import backtrader.utils.cs_long_short_signals.calculation_by_cython as cls
except ImportError:
    import backtrader.utils.cs_long_short_signals.calculation_by_numba as cls
...
def get_rate_sharpe_drawdown(arr):
    ...
    if isinstance(arr, pd.Series):
        arr = arr.to_numpy()
    return [ts.cal_sharpe_ratio(arr), ts.cal_average_rate(arr), ts.cal_max_drawdown(arr)]
...
def get_sharpe(arr):
    ...
    if isinstance(arr, pd.Series):
        arr = arr.to_numpy()
    return ts.cal_sharpe_ratio(arr)
...
def get_average_rate(arr):
    ...
    if isinstance(arr, pd.Series):
        arr = arr.to_numpy()
    return ts.cal_average_rate(arr)
...
def get_max_drawdown(arr):
    ...
    if isinstance(arr, pd.Series):
        arr = arr.to_numpy()
    return ts.cal_max_drawdown(arr)
...
def cal_long_short_factor_value_c(s, a=0.2):
    s = s.values
    return cls.cal_long_short_factor_value_cy(s, a)
