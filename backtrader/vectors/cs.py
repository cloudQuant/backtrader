...
class AlphaCs(object):
    ...
    def cal_signals(self, factors_df, engine="numpy"):
        # 计算信号值
        if engine == "numba":
            _signals = cal_signals_by_numba.cal_signals_by_numba(
                factors_df.to_numpy(), self.params["percent"], self.params["hold_days"]
            )
        else:
            _signals = cal_signals_by_numpy(
                self.factors.to_numpy(), self.params["percent"], self.params["hold_days"]
            )
    ...
    def cal_values(self, datas, signals_arr, hold_days):
        ...
        for i in range(data_rows - 1):
            ...
            if (i + 1) % hold_days == 0:
                ...
                return_arr = (
                    (symbol_close_price_arr - symbol_open_price_arr)
                    / symbol_open_price_arr
                    * sig_arr
                )
        ...
        return_arr = (
            (symbol_close_price_arr - symbol_open_price_arr) / symbol_open_price_arr * sig_arr
        )
        # print("计算得到的return_arr",return_arr)
        symbol_value_arr_cur = symbol_value_arr * (1 + return_arr)
    ...
    def cal_performance(self, total_value_arr, index_list, total_value_save_path=None):
        ...
        if self.time_frame == "Days":
            sharpe_ratio, average_rate, max_drawdown = get_rate_sharpe_drawdown(
                values_df["total_value"]
            )
        else:
        ...
        for key in self.params:
            if (
                "df" not in key
                and "save_path" not in key
                and "opens_arr" not in key
                and "closes_arr" not in key
            ):
    ...
    def run_alphalens(
        ...
        alphalens.tears.create_full_tear_sheet(
            data, long_short=long_short, group_neutral=group_neutral, by_group=by_group
        )
