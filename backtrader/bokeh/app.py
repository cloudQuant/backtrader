...
from . import tabs as default_tabs
...
class BacktraderBokeh:
    ...
    def _plot_candlestick(self, fig, df):
        ...
        # Plot up candles
        if len(up) > 0:
            fig.figure.segment(up['datetime'], up['high'], up['datetime'], up['low'],
                              color=self.scheme.barup_wick)
            fig.figure.vbar(up['datetime'], width, up['open'], up['close'],
                           fill_color=self.scheme.barup, line_color=self.scheme.barup_outline)
        
        # Plot down candles
        if len(down) > 0:
            fig.figure.segment(down['datetime'], down['high'], down['datetime'], down['low'],
                              color=self.scheme.bardown_wick)
            fig.figure.vbar(down['datetime'], width, down['open'], down['close'],
                           fill_color=self.scheme.bardown, line_color=self.scheme.bardown_outline)
    ...
    def _plot_line(self, fig, df):
        ...
        if 'close' in df.columns:
            fig.figure.line(df[x_col], df['close'], 
                           line_width=self.scheme.line_width,
    ...
    def generate_data(self, figid=None, start=None, end=None, back=None, 
