...
def test_cython():
    ...
    for n in [10000, 100000, 1000000]:
        ...
        for i in range(100):
            ...
            max_drawdown = get_max_drawdown(data1)
        ...
        print(
            f"python计算结果为：sharpe_ratio={sharpe_ratio}，average_rate={average_rate}，max_drawdown={max_drawdown}"
        )
        ...
        for i in range(100):
            a = time.perf_counter()
            arr = data["total_value"].to_numpy()
            sharpe_ratio = ts.cal_sharpe_ratio(arr)
            average_rate = ts.cal_average_rate(arr)
            max_drawdown = ts.cal_max_drawdown(arr)
        ...
        print(
            f"cython计算结果为：sharpe_ratio={sharpe_ratio}，average_rate={average_rate}，max_drawdown={max_drawdown}"
        )
        ...
        for i in range(100):
            ...
            sharpe_ratio = ns.cal_sharpe_ratio(arr)
            average_rate = ns.cal_average_rate(arr)
            max_drawdown = ns.cal_max_drawdown(arr)
        ...
        print(
            f"numba计算结果为：sharpe_ratio={sharpe_ratio}，average_rate={average_rate}，max_drawdown={max_drawdown}"
        )
...
if __name__ == "__main__":
    ...
    c = (
        ...
        .add_yaxis("cython", [i[2] for i in result])
        .reversal_axis()
