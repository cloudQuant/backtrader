...
| Line | 类型 | 说明 |
|------|------|------|
| `funding_rate` | float | 当前资金费率（8小时费率，如 0.0001 = 0.01%） |
...
2. **WebSocket 必需**: 此数据源强制要求 WebSocket，不会降级到 HTTP
3. **WebSocket 稳定性**: WebSocket 可能断开，内置自动重连机制
4. **数据同步**: 资金费率更新频率低于价格，属于正常现象
5. **启动检查**: 如果 WebSocket 连接超时，会抛出 `WebSocketRequiredError`
