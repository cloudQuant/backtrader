...
| **Cerebro**| `cerebro.py` (~2000 行) | 回测引擎，协调所有组件 |
...
1. **丰富的功能库**：60+ 内置指标、22 种分析器、多种数据源和经纪商支持
2. **多种运行模式**：runonce（向量化）、next（事件驱动）、live（实盘）
3. **性能优化**：Cython 扩展、TS/CS 模式、多进程支持
...
| 上交所 | 过户费（万 0.1）+ 佣金（万 3）+ 印花税（千 1，卖出） |

| 深交所 | 佣金（万 3）+ 印花税（千 1，卖出） |
...
class PerMoneyCommission(Commission):
    def __init__(self, rate=1.5e-4, min_comm=5.0):  # A 股最低 5 元
...
1. **高级并行计算**：提供简洁的并行计算接口
2. **中国市场适配**：完善 A 股交易成本模型
3. **简化 API**：提供更友好的策略开发接口
...
stock_commission = bt.commissions.AStockCommission(
    ...
    stamp_duty=0.001,       # 印花税千 1
    transfer_fee=0.00001,   # 过户费万 0.1（上交所）
...
class AStockCommission(CommissionInfo):
    ...
    params = (
        ('commission', 0.0003),      # 佣金率 万 3
        ('min_commission', 5.0),     # 最低佣金
        ('stamp_duty', 0.001),       # 印花税 千 1
