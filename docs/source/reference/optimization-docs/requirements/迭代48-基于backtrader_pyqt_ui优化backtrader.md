...
|**GUI**| 完整桌面应用 | 无 GUI | backtrader 缺少官方 GUI |

|**进度反馈**| Observer 实时更新 | 无 | backtrader 缺少进度监控 |

|**配置管理**| 持久化 | 无 | backtrader 缺少配置系统 |
...
# 根据时间差判断时间框架
    ...
    def load_dataframe(self, file_path, **kwargs):
        ...
        Args:
            ...
            - *kwargs: 额外参数（separator, datetime_format 等）
...
def on_progress(percent, remaining):
    if remaining:
        print(f"\r 进度: {percent*100:.1f}% | 剩余: {remaining:.0f}秒", end='')

    else:
        print(f"\r 进度: {percent*100:.1f}%", end='')
...
| Phase 1 | 创建 utils 目录结构 | 0.5 天 |

| Phase 2 | 实现 ProgressObserver | 1 天 |

| Phase 3 | 实现 ConfigManager | 1 天 |

| Phase 4 | 实现 DataManager | 1.5 天 |

| Phase 5 | 实现 CerebroEnhanced | 0.5 天 |
