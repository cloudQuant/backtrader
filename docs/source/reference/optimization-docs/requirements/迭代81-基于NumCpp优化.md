...
NumCpp 是 NumPy 的 C++实现版本，提供类似 NumPy 的 API，具有以下核心特点：

- **C++实现**: 高性能 C++数值计算库
- **NumPy API**: 类似 NumPy 的 API 设计
- **矩阵运算**: 高效矩阵运算
- **向量化**: 向量化计算支持
- **模板编程**: C++模板元编程
- **头文件库**: Header-only 设计
...
|**数组支持**| 1D 时间序列 | 1D/2D 数组 |

|**切片操作**| 基础索引 | NumPy 风格 Slice |

|**广播机制**| 无 | 完整广播支持 |

|**线性代数**| 依赖 numpy/linalg | 内置 Linalg 模块 |

|**类型系统**| Python 动态类型 | C++模板+类型特征 |

|**内存管理**| Python GC | 自定义 Allocator |
...
- **Shape 类**: 2D 形状表示，支持 size/isnull/issquare 等方法
- **Slice 类**: NumPy 风格切片，支持负索引和步长
- **边界检查**: 自动验证和修正索引

#### 5. 内存管理策略

- **自定义 Allocator**: 支持 STL 分配器接口
...
借鉴 NumCpp 项目的高性能数值计算设计，为 backtrader 添加以下功能模块，提升计算性能和 API 易用性：
...
- *FR1.1 NdArray 兼容接口**
- 需求描述: 提供类似 NumCpp 的 NdArray 接口
- 优先级: 高
- 验收标准:
  - 实现 Array 类支持 1D/2D 数组
  - 支持 shape 属性查询
  - 支持 reshape 操作
  - 支持 astype 类型转换
...
class Array:
    ...
    提供类似 NumCpp 的 NdArray 接口，兼容 NumPy 数组
    """
...
# Windows: 尝试使用 IsProcessorFeaturePresent
            try:
                kernel32 = ctypes.windll.kernel32

# 检测各种 SIMD 特性
                cls._has_sse = True  # 假设现代 CPU 都有
                cls._has_sse2 = True
            except:
                pass
...
1. 实现 Statistical 统计函数
2. 实现 Mathematical 数学函数
3. 实现 Logical 逻辑函数
4. 文档完善

### API 兼容性保证

所有新增功能与现有 backtrader API 兼容：
...
2. **集成测试**: 与 NumPy 结果对比验证
3. **性能测试**: 对比 NumPy 性能，目标 < 2 倍
