...
pysystemtrade 是 Rob Carver 开发的系统化交易框架，基于其著作《Systematic Trading》，具有以下核心特点：
...
1. **系统理论**: 系统化交易的理论框架
2. **预测规则**: ForecastScalarFixed 预测规则设计
3. **仓位管理**: Position sizing 科学方法
4. **组合优化**: Portfolio 优化算法
5. **成本分析**: 滑点和成本建模
6. **生产系统**: 生产环境的系统设计
...
1. **丰富的功能库**：60+ 指标、22 种分析器
2. **多种运行模式**：runonce、next、live
3. **性能优化**：Cython 扩展、TS/CS 模式
...
# 应用缓冲区
        if self.p.use_buffer:
            ...
            buffer = max(buffer, 1)  # 至少 1 个合约的缓冲
