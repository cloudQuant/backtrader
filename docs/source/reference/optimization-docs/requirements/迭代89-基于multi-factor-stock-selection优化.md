...
### multi-factor-stock-selection 项目简介

multi-factor-stock-selection 是一个多因子选股框架，具有以下核心特点：
...
|**数据模型**| Line 系统（时间序列） | DataFrame（截面数据） |

|**因子系统**| 内置 60+技术指标 | 财务/技术/情绪多因子体系 |
...
|**回测模式**| 事件驱动（逐 bar） | 批量处理（向量化） |

|**多资产支持**| 原生支持 | 专门针对股票池 |

|**因子合成**| 无 | Z-score 标准化+加权合成 |

|**机器学习**| 无 | LightGBM 集成 |

### Backtrader 可借鉴的优势

1.**因子生命周期管理**：因子墓地机制、动态 IC 监控

1. **因子评价体系**：IC/ICIR 计算、因子有效性评估
2. **截面分析能力**：多股票横向比较、排序选股
...
- *需求**:
- FR-07.1 因子 IC 监控
- FR-07.2 因子失效检测（连续低 IC）
- FR-07.3 因子自动退场
- FR-07.4 因子归档（因子墓地）
- FR-07.5 新因子上线流程

- *验收标准**:
- 连续 3 个月 ICIR<0.3 触发退场警告
- 退场因子自动归档
- 支持因子恢复
...
# 剔除无效值
                ...
                if valid_mask.sum() < 10:  # 至少 10 个有效样本
                    continue
...
# 计算加权合成因子
            ...
            return -abs(weighted_ic)  # 最大化 IC = 最小化负 IC
...
# 移回因子文件
    ...
    def check_underperforming_factors(self,
        ...
        Args:
            min_icir: 最小 ICIR 阈值
            consecutive_periods: 连续低 ICIR 的期数
...
1. 实现 FactorBase 基类和因子注册机制
2. 实现 FactorEvaluator 评价器
3. 添加常用技术因子（动量、波动率、RSI 等）
4. 单元测试
...
1. 实现 FactorCompositor 合成引擎
2. 实现 StockSelector 选股引擎
3. 实现 PortfolioConstructor 组合构建器
4. 集成测试

### 第四阶段：因子生命周期管理（1 周）

1. 实现 FactorGraveyard
2. 实现因子监控和退场机制
3. 单元测试

### 第五阶段：Backtrader 集成（2 周）

1. 实现 MultiFactorStrategy
2. 实现多数据源处理
3. 集成测试
4. 文档编写
...
selector.add_filter(MarketCapFilter(min_cap=50))  # 市值>50 亿
