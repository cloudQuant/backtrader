...
1. backtrader/run_test_with_log.py 这个运行 test_19 这个策略，然后对比 master 分支上的结果，看看 remove-metaprogramming 分支上的结果究竟差在哪里，然后去定位。
2. 如果必要，可以修改 test_19 这个测试用例，增加注释，然后方便在两个分支上进行对比。
