...
目前问题是：使用 pytest tests\add_tests/test_ind_psar.py tests\add_tests/test_ind_hurst.py 不并行的时候运行不报错
使用 pytest pytest tests\add_tests/test_ind_psar.py tests\add_tests/test_ind_hurst.py  -n 3 这种并行模式，就开始报错
