...
对比 logs 中 master 分支和 remove-metaprogramming 分支的交易日志,
...
两个分支中 IndexError 触发的日期**相差一天**（除个别日期外），导致：
...
在`linebuffer.py`的`__getitem__`方法中，对数据源(data feed)的 future 访问添加显式边界检查：
