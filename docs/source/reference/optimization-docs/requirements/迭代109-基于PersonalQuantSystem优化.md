...
### PersonalQuantSystem 项目简介

PersonalQuantSystem 是一个个人量化交易系统，具有以下核心特点：
...
通过对 PersonalQuantSystem 项目的深入研究，总结出以下核心架构特点：
...
2. **错误处理简陋**: 主要依赖 Python 异常，缺少业务语义
...
提供统一的配置管理系统，支持 YAML 配置文件，实现回测与实盘配置的统一管理。
...
2. 实现 TradingError 异常体系
3. 实现常量枚举定义
4. 实现 OutputManager 基础输出
5. 实现 SimpleStrategy 简化基类
...
1. **统一配置管理**: 通过 YAML 配置文件实现参数外部化管理
