...
prophet 是 Facebook 开源的时间序列预测库，具有以下核心特点：
...
1. **趋势自动识别**: 线性/Logistic/平坦趋势自动检测
2. **季节性建模**: 傅里叶级数建模多周期季节性
3. **变点检测**: 自动检测趋势变化点
4. **不确定性量化**: 预测区间支持风险评估
5. **插件式扩展**: 易于添加自定义季节性和回归量
6. **交叉验证**: 时间序列交叉验证(rolling forecast)
...
from ruptures import Pelt, Binseg  # 需要安装 ruptures 库
...
# Hurst = 0.5 表示随机游走
        ...
        if hurst > 0.5:
            persistence = (hurst - 0.5)* 2  # 映射到 0-1
        else:
            persistence = 0
...
print(f"最佳 MAPE: {best['best_score']:.2f}%")
...
1. **Prophet 预测策略**(~500 行)
   - ProphetIndicator 指标
   - ProphetSignal 信号指标
   - ProphetStrategy 策略基类
