...
**prenext** 阶段在累积足够的数据K线之前运行，此时指标可能还没有产生有效值。
...
class MyStrategy(bt.Strategy):
    ...
    def prenext(self):
        ...
        print(f"K线 {len(self)}: 正在累积数据...")
```

**特点：**
- 从第 0 根K线运行到 `minperiod - 1`
- 指标可能没有有效值
- 用于初始化和预热
...
def nextstart(self):
    ...
    print(f"K线 {len(self)}: 第一根有效K线！")
    # 默认实现会自动调用 next()
```

**特点：**
- 在第 `minperiod` 根K线时运行一次
...
**特点：**
- 从第 `minperiod` 根K线运行到数据结束
- 所有指标都有有效值
- 主要策略逻辑写在这里

## 最小周期系统

每个组件都有一个 `minperiod` 属性，表示产生有效输出前需要多少根K线。
...
class PhaseExample(bt.Strategy):
    ...
    def nextstart(self):
        # 这是第一根有有效 SMA 值的K线
        print(f"第一根有效K线: {len(self)}")

    def next(self):
        # 正常执行
        if len(self) == self.params.period + 1:
            print(f"第二根有效K线: SMA = {self.sma[0]:.2f}")
