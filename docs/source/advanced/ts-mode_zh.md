...
TS 模式通过一次性处理整个时间序列而不是逐根 K 线处理来实现**向量化回测**。这种方法利用:
...
| 完整回测 (10 万根 K 线) | 基准 | 3-5x | 快 3-5 倍 |
...
data = bt.feeds.PandasData(dataname=df)
data.qbuffer(10000)  # 内存中只保留 1 万根 K 线
