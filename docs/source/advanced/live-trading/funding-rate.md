...
2. **WebSocket Required**: This data feed mandates WebSocket; it will not fall back to HTTP
3. **WebSocket Stability**: WebSocket may disconnect; built-in auto-reconnect is included
4. **Data Sync**: Funding rate updates less frequently than price — this is normal
5. **Startup Check**: If WebSocket connection times out, `WebSocketRequiredError` is raised
