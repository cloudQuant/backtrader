...
- **永远不要在代码中硬编码 API 密钥**- 使用环境变量或 `.env` 文件
- **使用只读 API Key 进行测试**- 测试时使用没有交易权限的密钥
- **限制 IP 白名单**- 在交易所设置 IP 白名单保护账户
- **定期轮换密钥**- 定期更换 API 密钥

### 测试建议

- **先用测试网/沙盒环境测试**- Binance、OKX 等提供测试网环境
- **小仓位测试**- 实盘先用最小仓位验证策略
- **模拟交易验证**- 先用历史数据回测验证策略逻辑

### 性能优化

- **WebSocket 优于 REST**- 低延迟策略优先使用 WebSocket
...
| 环境配置 | `docs/CCXT_ENV_CONFIG.md` |

| CCXT 需求文档 | `docs/opts/优化需求/迭代 94-CCXT 实盘交易优化-完整版.md` |
