...
当前backtrader中已有三个数据容器类：

| 类名 | 文件 | 事件类型 | 用途 |
|------|------|---------|------|
| `TickerData` | `backtrader/ticker.py` | TickerEvent | Ticker数据（最新价、买卖价） |
...
2. **与迭代138不兼容**:
   - 迭代138使用`dataclass`定义事件（TickEvent, OrderBookSnapshot等）
   - 现有容器使用字典+getter模式
...
class EventData(ABC):
    ...
    所有事件数据的统一接口
    """
    timestamp: float                    # Unix时间戳（秒，支持毫秒）
    symbol: str                         # 交易对符号
    exchange: str = ''                  # 交易所名称
...
class TickEvent(EventData):
    ...
    last_price: Optional[float] = None  # 最新价（同price）
    last_volume: Optional[float] = None # 最新量（同volume）
...
class BarEvent(EventData):
    ...
    close: float                        # 收盘价
    volume: float                       # 成交量
    timeframe: str = '1m'              # 时间周期：1m/5m/1h等
...
class StrategyBase(LineIterator):
    ...
    def on_orderbook(self, orderbook: OrderBookSnapshot):
        ...
        Args:
            orderbook: OrderBookSnapshot实例（统一格式）
...
| 操作 | 旧方式 | 新方式 | 提升 |
|------|--------|--------|------|
| 获取price | `get_last_price()` 调用 | `tick.price` 直接访问 | **5-10倍** |
...
2. **性能提升**: dataclass直接属性访问
3. **向后兼容**: 适配器层保留现有API
4. **内置验证**: 所有事件都有`validate()`方法
5. **类型安全**: 使用类型提示

### 8.2 迁移路径

```
阶段1（Phase 0）：创建新的events.py
    ↓
阶段2（Phase 1）：修改现有容器为适配器
    ↓
阶段3（Phase 2）：新代码使用新格式
    ↓
阶段4（Phase 3+）：逐步废弃旧API
```
