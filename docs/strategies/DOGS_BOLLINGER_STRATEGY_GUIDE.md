...
cerebro.addstrategy(
    ...
    period=60,              # 布林带周期（可改为 20、120 等）
    devfactor=2.0,          # 标准差倍数（可改为 1.5、3.0 等）
...
# 添加 ATR 过滤（只在波动率足够时交易）

if self.atr[0] < self.data.close[0] *0.01:  # ATR 小于 1%
    return  # 波动率太低，不交易
...
- `OKX_MIN_TRADING_ANALYSIS.md` - DOGS/USDT 分析报告
