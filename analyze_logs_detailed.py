...
def compare_versions(master_file, remove_file):
    ...
    if "total_time" in master_stats and "total_time" in remove_stats:
        ...
        print(
            f"{'total_time (seconds)':<30} {m_time:>20.2f} {r_time:>20.2f} {diff:>15.2f} {pct:>9.1f}%"
        )
    ...
    print(
        f"{'排名':<5} {'函数位置':<80} {'Master(s)':>12} {'Remove(s)':>12} {'增量(s)':>12} {'增长率':>10}"
    )
    print("-" * 145)
    ...
    for loc, remove_func in remove_dict.items():
        if loc in master_dict:
            ...
            if time_diff > 0.001:  # 只关注有显著变化的
                pct_change = (
                    (time_diff / master_func["tottime"] * 100)
                    if master_func["tottime"] > 0
                    else float("inf")
                )
                changes.append(
                    {
    ...
    for i, change in enumerate(changes[:50], 1):
        ...
        print(
            f"{i:<5} {loc:<80} {change['master_time']:>12.3f} {change['remove_time']:>12.3f} {change['time_diff']:>12.3f} {pct_str:>10}"
        )

    # 调用次数增加最多的函数
    print("\n### TOP 30 调用次数增加最多的函数\n")
    print(
        f"{'排名':<5} {'函数位置':<80} {'Master次数':>15} {'Remove次数':>15} {'增量':>15} {'增长率':>10}"
    )
    print("-" * 150)
    ...
    for i, change in enumerate(changes[:30], 1):
        loc = change["location"][:78]
        pct_str = (
            f"+{change['call_diff']/change['master_calls']*100:.0f}%"
            if change["master_calls"] > 0
            else "NEW"
        )
        print(
            f"{i:<5} {loc:<80} {change['master_calls']:>15,} {change['remove_calls']:>15,} {change['call_diff']:>15,} {pct_str:>10}"
        )
    ...
    for func_name in builtin_funcs:
        ...
        if master_func or remove_func:
            ...
            print(
                f"{func_name:>12}: Master={m_calls:>12,}次/{m_time:>8.3f}s  Remove={r_calls:>12,}次/{r_time:>8.3f}s  "
