...
def compare_logs(master_log: str, remove_meta_log: str) -> None:
    ...
    print(f"{'Total Execution Time (s)':<40} {master_time:<25.4f} {remove_time:<25.4f} +{time_diff:.4f} (+{time_pct:.2f}%)")
    ...
    print(f"{'Total Function Calls':<40} {master_calls:<25,} {remove_calls:<25,} +{calls_diff:,} (+{calls_pct:.2f}%)")
    ...
    print(f"{'CPU Time (s)':<40} {master_cpu:<25.4f} {remove_cpu:<25.4f} +{cpu_diff:.4f} (+{cpu_pct:.2f}%)")
    print()
    ...
    getattr_master = master_dict.get('builtins.py(getattr)', {}).get('ncalls', 0) or \
                    master_dict.get('lineseries.py(__getattr__)', {}).get('ncalls', 0)
    getattr_remove = remove_dict.get('builtins.py(getattr)', {}).get('ncalls', 0) or \
                     remove_dict.get('lineseries.py(__getattr__)', {}).get('ncalls', 0)
    
    setattr_master = master_dict.get('builtins.py(setattr)', {}).get('ncalls', 0) or \
                    master_dict.get('lineseries.py(__setattr__)', {}).get('ncalls', 0)
    setattr_remove = remove_dict.get('builtins.py(setattr)', {}).get('ncalls', 0) or \
                     remove_dict.get('lineseries.py(__setattr__)', {}).get('ncalls', 0)
    
    print("\n### Attribute Access Overhead:")
    if hasattr_remove > hasattr_master:
        print(f"  - hasattr() calls increased: {hasattr_master:,} → {hasattr_remove:,} (+{hasattr_remove-hasattr_master:,})")
    if getattr_remove > getattr_master:
        print(f"  - getattr() calls increased: {getattr_master:,} → {getattr_remove:,} (+{getattr_remove-getattr_master:,})")
    if setattr_remove > setattr_master:
        print(f"  - setattr() calls increased: {setattr_master:,} → {setattr_remove:,} (+{setattr_remove-setattr_master:,})")
    
    print("\n### Missing Optimizations:")
    # Check for _once implementation
    once_remove = remove_dict.get('lineiterator.py(_once)', {})
    if once_remove:
        print(f"  - _once() optimization present but may need tuning: {once_remove.get('ncalls', 0):,} calls")
    ...
    for i, change in enumerate(changes[:5], 1):
        if change['master_calls'] > 0:
            print(f"    {i}. {change['function']}: +{change['diff']:,} calls (+{change['pct']:.1f}%)")
        else:
