...
def compare_stats(stats1, stats2, header1, header2):
    ...
    results = {
        "overall": {
            ...
            "time_diff_pct": (
                (header2["execution_time"] - header1["execution_time"])
                / header1["execution_time"]
                * 100
            ),
        },
...
def generate_comparison_report(results, output_file="performance_comparison.md"):
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        ...
        if overall["time_diff"] > 0:
            f.write(
                f"+{overall['time_diff']:.4f}s (+{overall['time_diff_pct']:.2f}%) ⚠️ **SLOWER** |\n\n"
            )
        else:
            f.write(
                f"{overall['time_diff']:.4f}s ({overall['time_diff_pct']:.2f}%) ✅ **FASTER** |\n\n"
            )
        ...
        backtrader_degraded = [
            f for f in results["degraded"] if "backtrader" in f["function"].lower()
        ]
        backtrader_improved = [
            f for f in results["improved"] if "backtrader" in f["function"].lower()
        ]
        ...
        if backtrader_degraded:
            ...
            for module, funcs in sorted(
                modules.items(), key=lambda x: sum(f["cumtime_diff"] for f in x[1]), reverse=True
            ):
                total_diff = sum(f["cumtime_diff"] for f in funcs)
                f.write(f"**{module}.py** (+{total_diff:.4f}s)\n")
                for func in funcs[:5]:
                    f.write(
                        f"  - {func['function'].split('(')[1].split(')')[0] if '(' in func['function'] else 'unknown'}"
                    )
